...
$_POST['comp_level']=7;
$_POST['comp_method']=1;
mysqli_connect ($mysqlihost,$mysqliuser,$mysqlipass,$mysqlibase);
...
define('C_RESULT', 2);
define('C_ERROR', 3);
mysqli_query("/*!40101 SET NAMES '" . CHARSET . "' */") or die("Invalid query: " . mysqli_error());
...
class dumper {
	...
	function backup() {
		...
		mysqli_select_db($db) or trigger_error ("Не удается выбрать базу данных.<BR>" . mysqli_error(), E_USER_ERROR);
		$tables = array();
	...
	function restore(){
		...
		while(($str = $this->fn_read_str($fp)) !== false){
			...
			if ($sql) {
    			...
    			if ($execute) {
            		$q++;
            		mysqli_query($sql) or trigger_error ("Неправильный запрос.<BR>" . mysqli_error(), E_USER_ERROR);
