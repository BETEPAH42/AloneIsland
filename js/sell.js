...
function Sinit_main_layer()
{
	...
	inited = 1;
	$("#ec").fadeOut(1);
	$("#ec").css({left:'50%',top:'50%',width:'220px',height:'200px',display:'block', margin:'-100px 0 0 -110px'});
	d.getElementById('ec').innerHTML = '<div style="margin:10px"><div class=but><center>Одинокие Острова</center> <img src="images/closebox.png" onclick="closesellingform()" style="cursor:pointer;float:right;position:relative; top:-20px; left:30px;"></div><img src="images/DS/hr.png" height=3 width=200><div id=transfer style="display:block;background-color:#EEEEEE;"></div></div>';
	$("#ec").show(300);
...
// форма отправки на подпись
function blank(id,name,whos) 
{	
Sinit_main_layer();
$('#transfer').html('<form action=main.php method=POST><input type=hidden name=blank value='+id+'><b>Подписать "'+name+'" ?</b><br> <br>Подписать у: <INPUT TYPE="text" name=who value='+whos+' maxlength=20 class=laar><br><input type=submit value="OK" class=login style="width:100%"></FORM>');
}
function liver(id,name,w) 
{	
	Sinit_main_layer();
	$('#transfer').html('<form action=main.php method=POST><input type=hidden name=liver value='+id+'><b>Сделать живца из "'+name+'"?</b><input type=hidden name=liver_w value='+w+'><br><input type=submit value="OK" class=login style="width:100%"></FORM>');
       ActionFormUse = 'fornicka';
}

function sostav(id,name,whos) 
{	
Sinit_main_layer();
$('#transfer').html('<b>Подписанные "'+name+'" ?</b><br> <br>Подписанно: '+whos+'<br>');
}
