...
function ins_HP(curh, maxh, curm, maxm, hp_int, ma_int) {
	...
	document.getElementById("hptitle").title = '' + (Math.round((maxh / hp_int) * 1000) / 1000) + ' здоровья за 1 сек.';
	document.getElementById("matitle").title = '' + (Math.round((maxm / ma_int) * 1000) / 1000) + ' маны за 1 сек.';
}
