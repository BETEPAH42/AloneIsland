...
async function PostStatUser() {
	let formData = new FormData();
	formData.append('s1', sila);
	formData.append('s2', lovk);
	formData.append('s3', udacha);
	formData.append('s4', zdorov);
	formData.append('s5', znanya);
	formData.append('s6', power);
	formData.append('free_stats', ups);
	formData.append('uid', GetUidPers());
	let resp = await fetch('../services/UserStats.php', {
		method: 'POST',
		body: formData
	});
