...
const newLocal = "top.frames['ch_list'].location = 'weather.php'";
function start() {
...
function main_refresher() {
	...
	// document.getElementById('TIME').innerHTML = '<b>' + transform_time(HOURS) + ':' + transform_time(MINUTES) + ':' + transform_time(SECONDS) + '</b>';
...
function load(a, b) {
	...
	if (prc < 101)
		$("#load").html("Загрузка...[" + prc + "%]");
	if (prc >= 100) {
		load_int = setTimeout("load(0,1)", 3000);
		$("#load").html("Загрузка...[100%]");
	}
	else
