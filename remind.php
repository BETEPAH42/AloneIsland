...
if (isset($_POST["email"])) {
	...
	$pers = sql::q1("SELECT uid FROM users WHERE email='" . $email . "' and second_pass='" . $spass . "' and flash_pass='" . $fpass . "'");
	if ($pers["uid"] and empty($_POST["cemail"])) {
		...
		$pers = sql::q1("SELECT uid,user FROM users WHERE email='" . $email . "'");
		if ($pers) {
			//$newpass = substr(md5(time()),0,6);
