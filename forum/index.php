...
if (UID != 1){  error_reporting(0);}

include ("../configs/config.php");
include ("../inc/functions.php");
$main_conn = mysqli_connect ($mysqlihost,$mysqliuser,$mysqlipass,$mysqlibase);
mysqli_select_db($mysqlibase, $main_conn);
...
echo '<center style="background-image:url(img/topLine.gif); width:100%; height:45px; padding:0; margin:0; display:block;font-size: 26px;font-weight: bold;cursor:pointer;font-family: \'Trebuchet MS\',Verdana;" onclick="location=\'index.php\'">Одинокие острова ФОРУМ</center>';
...
function main(){ 
  ...
  $razd = '';
  $title = 'Форум Одинокие острова :: Главная';
  ?>
...
<?
  ...
  $time = tme();
  while ($sql = mysqli_fetch_array($tmp1)){
...
function show_cat(){
  if (isset($_GET['id'])){
    ...
    if (($hide[0] != 1) || ($hide[0] == 1 && $rights['hide'] == 1)){
      ...
      $title = 'Форум Одинокие острова:: '.$tmp1['title'];
      ?><script language="javascript">document.title='<? echo $title;?>'</script><? //>
      $time = time();
...
function show_topic(){
  ...
  if (isset($_GET['id'])){
    ...
    if (($hide[0] != 1) || ($rights['hide'] == 1)){
      ...
      $title = 'Форум "Одинокие острова" :: '.$razd['title'].' :: '.$topic['title'];
      ?><script language="javascript">document.title='<? echo $title;?>'</script><? //>
      ...
      while ($msg = mysqli_fetch_array($tmp3)){
        $author = usr_info($msg['author'],$time);
        $razd.=$author.'<'.$time.'msg>';
        $razd.=rep(unhtmlentities($msg['text'])).'<'.$time.'msg>';
        $razd.=$msg['author'].'<'.$time.'msg>';
...
?>
<center class=timef><a class=timef href='/aloneisland/forum/'>AloneIslands</a> Форум.</center>
