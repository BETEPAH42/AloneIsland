...
if ($rights['adm'] == 1) {
  ...
  function adm_razd_form()
  {
    ...
    $mx = SQl::q1('SELECT count(id) as count FROM `forum_razd`');
    $mx = $mx['count'];
    $sql = sql::q('SELECT * FROM `forum_razd` ORDER BY `pos`');
  ...
  function adm_new_razd()
  {
    ...
    $nm = sql::q1('SELECT count(id) as count FROM `forum_razd`');
    $nm = $nm['count'];
    $nm++;
    ...
    if (isset($_POST['title']) && isset($hide)) {
      ...
      sql::q('INSERT INTO `forum_razd`(`id`,`title`,`pos`,`hide`) VALUES(' . $nm . ',"' . htmlspecialchars($_POST['title']) . '",' . $nm . ',' . $hide . ')');
    }
  ...
  function adm_edit_razd_form()
  {
    ...
    if (isset($_GET['id'])) {
      ...
      if ($nm == 1) {
        ...
        $sql = SQL::q1('SELECT * FROM `forum_razd` WHERE `id`=' . $id);
    ...
    if (isset($_GET['id'])) {
      ...
      if ($nm == 1) {
        ...
        if (isset($_POST['title'])) {
          ...
          sql::q('UPDATE `forum_razd` SET `title`="' . htmlspecialchars($_POST['title']) . '", `hide`=' . $hide . ' WHERE `id`=' . $id);
        }
      } else {
  ...
  function adm_del_razd_form()
  {
    ...
    if (isset($_GET['id'])) {
      ...
      if ($nm == 1) {
        $rzd = sql::q1('SELECT `title`,`hide` FROM `forum_razd` WHERE `id`=' . $id);
        ...
        $s_nm = sqlr('SELECT count(id) as count FROM `forum_subrazd` WHERE `cat`=' . $id);
        $s_nm = $s_nm['count'];
        if ($s_nm > 0) {

          $s_razd = sql::q('SELECT * FROM `forum_razd`');

          echo 'Перенести подразделы в: <select name="subrazd" class=items>';

          foreach ($s_razd as $res) {
  ...
  function adm_del_razd()
  {
    if (isset($_GET['id'])) {
      ...
      sql::q('DELETE FROM `forum_razd` WHERE `id`=' . $id);

      $tmp = sql::q('SELECT * FROM `forum_razd` ORDER BY `pos`');

      $i = 1;

      foreach ($tmp as $res) {

        sql::q('UPDATE `forum_razd` SET `pos`=' . $i . ' WHERE `id`=' . $res['id']);
  ...
  function adm_ud_razd($pos)
  {
    if (isset($_GET['id'])) {
      ...
      if ($id != 1 || $id != $mx['count']) {
        $pos1 = sql::q1('SELECT `pos` FROM `forum_razd` WHERE `id`=' . $id);
        ...
        sql::q('UPDATE `forum_razd` SET `pos`=' . $pos1['pos'] . ' WHERE `pos`=' . $pos2);

        sql::q('UPDATE `forum_razd` SET `pos`=' . $pos2 . ' WHERE `id`<>' . $id . ' AND `pos`=' . $pos1['pos']);
      }
    } else {
      show_err('Указан неверный раздел');
    }
  ...
  function adm_subrazd_form()
  {
    ...
    while ($razd = mysqli_fetch_array($tmp)) {
      ...
      while ($subrazd = mysqli_fetch_array($tmp1)) {
        ...
        if ($subrazd['pos'] != 1) {
          echo ' <a href="index.php?act=adm_up_subrazd&id=' . $subrazd['id'] . '"><img src="img/up.png"></a>';
        }

        if ($subrazd['pos'] != $mx) {
          echo ' <a href="index.php?act=adm_down_subrazd&id=' . $subrazd['id'] . '"><img src="img/down.png"></a>';
        }
    ...
    foreach ($rzd as $razd) {
      echo '<option value=' . $razd['id'] . '>' . $razd['title'] . '</option>';
    }
    ...
    if (isset($_GET['id'])) {
      ...
      if ($nm == 1) {
        ...
        if ($sub_num > 0) {
          ...
          while ($subrazd = mysqli_fetch_array($tmp)) {
        ...
        if ($nm == 1) {
          ...
          echo '<br><form name="form" action="index.php?act=adm_edit_users&id=' . $usr[0] . '" method="post">
            <div class=timef>
              ...
              <tr><td>Применять заклинание молчания:</td><td><input name="ans" type="checkbox" value="1" ';
          if ($user['ans'] == 1) {
            echo 'checked';
          }
          echo '>';
          echo '</td></tr><tr><td>Редактировать сообщения:</td><td><input name="edit" type="checkbox" value="1" ';
          if ($user['edit'] == 1) {
            echo 'checked';
          }
          echo '><br>';
          echo '</td></tr><tr><td>Закрывать темы:</td><td><input name="close" type="checkbox" value="1" ';
          if ($user['close'] == 1) {
            echo 'checked';
          }
          echo '><br>';
          echo '</td></tr><tr><td>Прикреплять темы:</td><td><input name="up" type="checkbox" value="1" ';
          if ($user['up'] == 1) {
            echo 'checked';
          }
          echo '><br>';
          echo '</td></tr><tr><td>Создавать разделы:</td><td><input name="create" type="checkbox" value="1" ';
          if ($user['create'] == 1) {
            echo 'checked';
          }
          echo '><br>';
          $adm = sqlr('SELECT count(uid) FROM `forum_admin` WHERE `uid`=' . $usr[0]);
          echo '</td></tr><tr><td>Администрировать форум:</td><td><input name="adm" type="checkbox" value="1" ';
          if ($adm == 1) {
            echo 'checked';
          }
          echo '>';
  ...
  function adm_edit_users()
  {
    if (isset($_GET['id'])) {
      ...
      if (isset($_POST['ans']) && intval($_POST['ans']) == 1) {
        $ans = 1;
      } else {
        $ans = 0;
      }
      ...
      if (isset($_POST['close']) && intval($_POST['close']) == 1) {
        $close = 1;
      } else {
        $close = 0;
      }
      if (isset($_POST['up']) && intval($_POST['up']) == 1) {
        $up = 1;
      } else {
        $up = 0;
      }
      if (isset($_POST['create']) && intval($_POST['create']) == 1) {
        $create = 1;
      } else {
        $create = 0;
      }
