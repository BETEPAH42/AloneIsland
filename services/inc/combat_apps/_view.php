...
foreach ($allapps as $app) {
	...
	$p1 = '';
	$p2 = '';
	$p = sql::q("SELECT sign,invisible,user,level,state,clan_name,fteam,rank_i FROM users WHERE apps_id=" . $app["id"] . "");
	if (
		$app["pl1"] == $app["count1"] and $app["pl2"] == $app["count2"]
		and $app["type"] == 2
	...
	elseif (
		...
		} elseif (!$write_this) sql::q("DELETE FROM app_for_fight WHERE id=" . $app["id"] . "");
		elseif ($app["atime"] <= $UNIXtime) {
			if ($app["type"] == 1) {
				sql::q("DELETE FROM app_for_fight WHERE id=" . $app["id"] . "");
				sql::q("UPDATE users SET apps_id=0,refr=1 WHERE apps_id=" . $app["id"] . "");
			}
			if ($app["type"] == 2) {
				if ($app["pl2"]) {
					$p = sql::q1("SELECT sign,aura,user,level,state,clan_name,fteam FROM users WHERE apps_id=" . $app["id"] . "");
					include("_begin_group.php");
				} else {
					sql::q("DELETE FROM app_for_fight WHERE id=" . $app["id"] . "");
					sql::q("UPDATE users SET apps_id=0,refr=1 WHERE apps_id=" . $app["id"] . "");
			...
			if ($app["type"] == 3) {
				...
				if ($app["pl1"] > 1) {
					$p = sql::q1("SELECT user,rank_i FROM users WHERE apps_id=" . $app["id"] . "");
					include("_begin_haot.php");
				} else {
					sql::q("DELETE FROM app_for_fight WHERE id=" . $app["id"] . "");
					sql::q("UPDATE users SET apps_id=0,refr=1 WHERE apps_id=" . $app["id"] . "");
...
$lb_attack = 0;
if ($cat == 1) {
	$lb = sql::q1("SELECT b_frequency FROM configs")['b_frequency'];
	if ($pers["level"] < 6 and $pers["level"] > 0) {
		if (($pers["lb_attack"] + $lb) <= tme()) {
			$bts = sql::q("SELECT id,user,level FROM bots WHERE level>" . ($pers["level"] - 2) . " and level<" . ($pers["level"] + 2) . " and rank_i<" . ($pers["rank_i"] + 140) . " and special=0 ORDER BY RAND() LIMIT 0,3");
			foreach ($bts as $bt) {
				$s .= '\'50:1:300:';
				$s .= '1:1:1:';
				$s .= '1:1:1:';
	...
	if ($pers["level"] < 2 and $pers["level"] >= 0) {
		if (($pers["lb_attack"] + $lb) <= tme()) {
			$bts = sql::q("SELECT id,user,level FROM bots WHERE level>=0 and level<" . ($pers["level"] + 2) . " and rank_i<" . ($pers["rank_i"] + 140) . " and special=0 ORDER BY RAND() LIMIT 0,3");
			foreach ($bts as $bt) {
				$s .= '\'50:1:300:';
				$s .= '1:1:1:';
				$s .= '1:1:1:';
	...
	/*if ($pers["level"]<=2 and $counter<2)
		{
			$bts = SQL::q('SELECT uid,user,level,ctip FROM users WHERE ctip=-1 and level='.($pers["level"]).' and silence=0 LIMIT 0,'.rand(2,3).'');
			if(!$bts)
				$bts = SQL::q("SELECT uid,user,level,ctip FROM users WHERE level=".($pers["level"])." and block<>'' and rank_i>5 and s6=1 and s5=1 and silence = 0 LIMIT 0,".rand(2,3)."");
			while($bt = mysqli_fetch_array($bts))
			{
				$s .= '\'50:1:120:';
				$s .= '1:1:1:';
				$s .= '1:1:1:';
