...
if (@$_GET["show_t1"] and !$t1["finished"] and $t1["type"] == 0) // Турнир 1 5-10 10x10
{
	...
	if (!mtrunc(T_START + $t1["time"] - tme())) {
		$id = start_t1();
		sql::q("UPDATE quest SET finished=0,time=" . tme() . "+" . T_START . ",type=1,lParam=" . $id . " WHERE id = " . TOUR1);
	} else {
		...
		$users = sql::q1("SELECT * FROM users WHERE tour=1;");
		$bots = sql::q1("SELECT * FROM bots WHERE level=10 ORDER BY RAND() LIMIT 0,20;");
		$can_join = 0;
		echo "Участники:";
		echo "<center>";
...
<br><i>Призы:</i> 10 пергаментов, 100 LN, 10000 опыта";
	...
	if (!mtrunc(T_START2 + $t1["time"] - tme())) {
		$id = start_t1(10, 2);
		sql::q("UPDATE quest SET finished=0,time=" . tme() . ",type=3,lParam=" . $id . " WHERE id = " . TOUR1);
	} else {
		$start = "";
		if ($t1["type"] == 2)
	...
	sql::q("UPDATE quest SET finished=1,time=" . tme() . " WHERE id=" . TOUR1);
