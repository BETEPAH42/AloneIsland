...
if (@$_GET["show_t3"] and !$t1["finished"] and $t1["type"] == 0) // Турнир 1 15-99 10x10
{
	...
	if (!mtrunc(T_START + $t1["time"] - tme())) {
		...
		sql::q("UPDATE quest SET finished=0,time=" . tme() . "+" . T_START . ",type=1,lParam=" . $id . " WHERE id = " . TOUR3);
	} else {
		...
		$users = "SELECT * FROM users WHERE tour=3;";
		...
		for ($i = 0; $i < 20; $i++) {
			...
			if (!$u) {
				...
				if (!$u)
					$u = $u1;
				$info = "<a href=AloneIsland/binfo.php?" . $u["id"] . " target=_blank><img src=images/i.gif></a>";
			} else
				$info = "<a href=AloneIsland/info.php?id=" . $u["uid"] . " target=_blank><img src=images/i.gif></a> <b class=mfb style='color:#009900'>[человек]</b>";
			echo "<tr>";
			echo "<td>";
...
<br><i>Призы:</i> 10 пергаментов, 100 LN, 10000 опыта";
	...
	if (!mtrunc(T_START2 + $t1["time"] - tme())) {
		...
		sql::q("UPDATE quest SET finished=0,time=" . tme() . ",type=3,lParam=" . $id . " WHERE id = " . TOUR3);
	} else {
		$start = "";
		if ($t1["type"] == 2)
			$start = "<b class=mfb>Начало через " . mtrunc(T_START2 + $t1["time"] - tme()) . " сек.</b>";
		$log = sql::q("SELECT time,log FROM fight_log WHERE cfight=" . $t1["lParam"] . " ORDER BY turn DESC LIMIT 0,2");
		$_LOG = '';
		foreach ($lof as $l)
			$_LOG .= str_replace("'", '"', str_replace("%", "<br><font class=timef>" . $l["time"] . "</font> ", "<font class=timef>" . $l["time"] . "</font> " . $l["log"])) . "<hr>";
		$fight_res = sql::q1("SELECT result FROM fights WHERE id=" . $t1["lParam"] . "")['result'];
	...
	sql::q("UPDATE quest SET finished=1,time=" . tme() . " WHERE id=" . TOUR3);
