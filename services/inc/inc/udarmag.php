...
if ($zid and substr_count("19|" . $_pers["zakl"], $zid)) {
	...
	if ($_pers["cma"] >= $zakl["mana"]) {
		...
		if ($promax > rand(0, 100)) {
			...
			if ($zakl["type"] == "push") {
				...
				$subpush_log = '';
				$counter = 0;
				$ps = sql::q("SELECT uid,user,xf,yf,aura,chp,hp,level,sb3,s2,mf2,fteam,sign,bg,bn,bj FROM users WHERE cfight=" . $_pers["cfight"] . " and fteam<>" . $_pers["fteam"] . " and chp>0");
				$bs = sql::q("SELECT id,user,xf,yf,aura,chp,hp,level,s2,mf2,fteam,id_skin,rank_i FROM bots_battle WHERE cfight=" . $_pers["cfight"] . " and fteam<>" . $_pers["fteam"] . " and chp>0");

				$i = 0;
				foreach ($ps as $persvs) {
					$i++;
					if ($i > $zakl["c_c"]) break;
					else {
						...
						if ($persvs["sign"] <> 'none') $sign = '<img src="images/signs/' . $persvs["sign"] . '.gif">';
						else $sign = '';
						...
						if ($persvs[$bpoint] > 0) {
							$hp = 0;
							$steps = 0;
							$block_z = ' <u>(противник заблокировал удар)</u>';
						}
						if ($ylov > rand(1, 100)) {
							$hp = 0;
							$steps = 0;
							$block_z = ' <u>(противник увернулся)</u>';
						}
						$persvs["xf"] += $steps;
						...
						if ($hp > 0) $hp = "(<font class=hp>-" . $hp_true . " HP</font>, " . $shagi . ")";
						else $hp = $block_z;
				...
				foreach ($bs as $persvs) {
					...
					if ($i > $zakl["c_c"]) break;
						...
						sql::q("UPDATE bots_battle SET xf=" . $persvs["xf"] . ",chp=" . $persvs["chp"] . " WHERE id=" . $persvs["id"] . "");
						$hp_alll += abs($steps);
						$hp_all += $hp;
						//Считаем опыт
						if (!$persvs["id_skin"])
			...
			if ($zakl["type"] == "blast" or $zakl["type"] == "blast2") {
				...
				foreach ($ps as $persvs) {
					...
					if ($i > $zakl["c_c"]) break;
						...
						$_pers["exp_in_f"] += experience($hp, $_pers["level"], $persvs["level"], $persvs["uid"]);
						//Закончили опыт
						if ($hp > 0) $hp = "(<font class=hp>-" . $hp_true . " HP</font>)";
						else $hp = $block_z;
						$subpush_log .= "<font class=bnick color=" . $colors[$persvs["fteam"]] . ">" . $persvs["user"] . "</font>" . $hp . ",";
						$counter++;
					}
		...
		$z = 2;
		$no_mana = true;
		$zakl_log = "<font class=bnick color=" . $colors[$_pers["fteam"]] . ">" . $_pers["user"] . '</font> нехватает маны для хода';
	}


	if ($zakl["stype"] <> '')
		sql::q("UPDATE `users` SET `cma`='" . $_pers["cma"] . "',`" . $zakl["stype"] . "`=" . $_pers[$zakl["stype"]] . ",fexp=fexp+" . intval($hp_all) . ",exp_in_f=" . $_pers["exp_in_f"] . " WHERE `uid`='" . $_pers["uid"] . "'; ");
	...
	if ($_pers["chp"] > 0) {
		if (@$_GET["vs_id"]) {
			if ($_GET["vs_id"] > 0)
				$_persvs = sql::q1("SELECT * FROM users WHERE uid=" . intval($_GET["vs_id"]) . " and chp>0");
			else
				$_persvs = sql::q1("SELECT * FROM bots_battle WHERE id=" . intval($_GET["vs_id"]) . "  and chp>0");
		}
