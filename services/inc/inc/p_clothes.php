...
if ($pers["uid"])
	$res = sql::q("SELECT * FROM `wp` WHERE uidp=" . intval($pers["uid"]) . " and weared=1");
else
	$res = sql::q("SELECT * FROM `wp` WHERE uidp=" . intval(-1 * $pers["bid"]) . " and weared=1");
$j = 0;
foreach ($res as $v) {
	...
	if (
		$v["type"] == "orujie" and ($or1["id"] <> $dscr)
		and $or2["image"] = $v["image"]
	) {
		$or2["id"] = $dscr;
		$or2type = $v["stype"];
	}
	if ($v["type"] == "kolco" and $ko1["id"] == "0" and $ko1["image"] = $v["image"] and $_ko1 = true)
		$ko1["id"] = $dscr;
	if (
		$v["type"] == "kolco" and ($ko1["id"] <> $dscr)
		and $ko2["image"] = $v["image"]
	)	$ko2["id"] = $dscr;
	if ($v["type"] == "kam") {
		for ($i = $j; $i < $j + 1; $i++)
			if ($i == 0) {
