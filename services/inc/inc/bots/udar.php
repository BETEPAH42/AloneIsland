...
if ($persvs["chp"] > 0) {
	...
	if ($r > 1) {
		...
		SQL::q("UPDATE `fights` SET `turn`='" . str_replace($botlib . "|", "", $fight["turn"]) . $botlibnew . "|" . "' ,
	 `ltime`=" . time() . " ,
...
if (!empty($_POST['attp']) and strpos(" " . $pers['zakl'], $_POST['attp']) > 0 and $_POST["attp"] <> 'forv') {
	...
	$persvs = $pers;
	$pers = $tpers;
	if ($error == '') $s = " <font class=" . $color . ">" . $pers["user"] . "</font> накладывает <font class=user>" . $zak['name'] . "</font> на <font class=" . $vscolor . ">" . $persvs['user'] . "</font>.";
	else $s = " Персонажу <font class=" . $color . ">" . $pers["user"] . "</font> не хватает маны чтобы наложить <font class=user>" . $zak['name'] . "</font> на <font class=" . $vscolor . ">" . $persvs['user'] . "</font>, или это заклинание уже действует.";
	if ($error == '') SQL::q("UPDATE `users` SET `cma`='" . $pers["cma"] . "' WHERE `uid`='" . $pers["uid"] . "'; ");
	$fall = $s . ";" . $fall;
}
...
if ($kl <> 0) {
	...
	if ($persvs["bot"] == 0)
		$p = sql::q1("SELECT aura FROM users WHERE user='" . $persvs["user"] . "'");
