...
if ($pers["chp"]>0)
{
$sk = sql::q1("SELECT user,id_skin FROM bots_battle WHERE cfight=".$pers["cfight"]." and fteam<>".$pers["fteam"]." and id_skin>0");
$wp = sql::q1("SELECT name,dprice,price,max_durability FROM wp WHERE uidp=".$pers["uid"]." and weared=1 and stype='noji' and dprice=0");
if ($wp and $sk) 
{
	if (@$_GET["skinout"])
	{
		...
		SQL::q("UPDATE bots_battle SET id_skin=0 WHERE cfight=".$pers["cfight"]." and fteam<>".$pers["fteam"]." and id_skin>0");
		if ($wp["dprice"]==0)SQL::q("UPDATE wp SET durability=durability-1 WHERE uidp=".$pers["uid"]." and weared=1 and stype='noji' and dprice=0 LIMIT 1;");
...
FROM users WHERE cfight=" . $pers["cfight"] . " and fteam=2");
	...
	if ($c1b == 1) {
		...
		if ($ghost)
			if (sql::q1("SELECT COUNT(*) as count FROM users WHERE uid=" . $ghost . " and =-1")['count'])
	...
	if ($fight["turn"] <> "finished" and $fight["type"] == "notf" and $t > $pers["waiter"]) {
		...
		foreach ($p_t1 as $tmp) {
			...
			if ($tmp["sign"] <> 'none') $sign = '<img src="images/signs/' . $tmp["sign"] . '.gif" title="' . $tmp["clan_name"] . '">';
			else $sign = '';
			...
			if ($fight["turn"] <> 'n' and $_FT >= 10) {
				if ($LIFE1 > 0) $tmp["victories"]++;
				else $tmp['losses']++;
			}
			if ($_FT < 10) $exp = 0;
...
,`exp`=exp + " . $exp . ",exp_in_f=0,`exp_chat`=" . $exp . ",f_turn=" . $win . " WHERE `uid`='" . $tmp['uid'] . "'");
		...
		if ($fight["turn"] <> "finished" and $fight["type"] == "notf") {
			...
			sql::q("DELETE FROM turns_f WHERE idf='" . $pers["cfight"] . "'");
		}
	}
