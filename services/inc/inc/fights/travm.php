...
if (($rand < $fight["travm"] / 3 and $_persvs["uid"]  or $fight["travm"] == 100) and $_persvs["level"] > 4) {
	...
	$zid = sql::q1("SELECT id FROM auras WHERE special=" . $rand . " ORDER BY RAND()")['id'];
...
if ($_persvs["uid"] and $_pers["uid"] and $_persvs["level"] > 4) {
	if ($_pers["pol"] == "male") $ob = "порыскал в карманах трупа";
	else $ob = "порыскала в карманах трупа";
	...
	if (rand(0, 100) < 4 and $_persvs["level"] >= ($_pers["level"] - 2)) {
		if (($_persvs["level"] / 12 + $_pers["sp10"] / 20) > rand(1, 200)) {
			$v = sql::q1("SELECT * FROM wp WHERE uidp=" . ($_persvs["uid"]) . " and price<100 and dprice=0
	and weared=0 ORDER BY RAND()");
			if ($v["id"] <> '') {
				$res = "Обнаружен и украден «" . $v["name"] . "» !";
				sql::q("UPDATE wp SET uidp=" . $_pers["uid"] . ",user=" . $_pers["user"] . " WHERE id=" . $v . "");
			} else {
			...
			sql::q("UPDATE `users` SET `money`=money-" . $r . " WHERE `uid`='" . $_persvs["uid"] . "'");
		}
