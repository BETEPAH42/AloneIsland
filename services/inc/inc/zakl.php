...
if ($id <> '') {
	$m_str1 = '';
	$m_str2 = '';
	if ($type <> 'all' and $type <> 'other') $m_str1 = "`type`='" . $type . "'";
	elseif ($type == 'other')
	...
	if ($m_str1 == '' and $m_str2 == '') $m_str = "";
	else $m_str = " and " . $m_str;

	$zak = sql::q1("SELECT * FROM `zakl` WHERE `id`='" . $id . "'" . $m_str . ";");
}
if ($zak["type"] == 'aura2' or $zak["type"] == 'push' or $zak["type"] == 'blast2') $proc = '%';
else $proc = '';
$v = $zak;
$key = 's1';
if ($v[$key] > 0) $v[$key] = "+" . $v[$key] . $proc;
$key = 's2';
if ($v[$key] > 0) $v[$key] = "+" . $v[$key] . $proc;
$key = 's3';
if ($v[$key] > 0) $v[$key] = "+" . $v[$key] . $proc;
...
if (@$zak["name"] <> '') {
	...
	if ($zak['t_in_c'] > 0) $zak['time'] = $zak['t_in_c'] . ' ход.';
	else $zak['time'] = tp($zak['time']);
