...
if ($action) # Основной блок действий
{
	...
	if (@$_GET["gotox"] and $go) {
		...
		if ($shagi <= floor(1) and $_GET["gotox"] > 0 and $_GET["gotox"] < $fight["maxx"] and $_GET["gotoy"] >= 0 and $_GET["gotoy"] < $fight["maxy"] and !substr_count($bplace["xy"], "|" . $_GET["gotox"] . "_" . $_GET["gotoy"] . "|")) {
			$check_for_go = sql::q1("SELECT uid FROM users WHERE cfight=" . $pers["cfight"] . " and xf=" . intval($_GET["gotox"]) . " and yf=" . intval($_GET["gotoy"]) . " and chp>0");
			if (!$check_for_go['uid']) {
				$check_for_go = sql::q1("SELECT id FROM bots_battle WHERE cfight=" . $pers["cfight"] . " and xf=" . intval($_GET["gotox"]) . " and yf=" . intval($_GET["gotoy"]) . " and chp>0");
				if (!$check_for_go['id']) {
	...
	if (@$req["gotox"]) {
		...
		if ($shagi <= floor(1) and $req["gotox"] > 0 and $req["gotox"] < $fight["maxx"] and $req["gotoy"] >= 0 and $req["gotoy"] < $fight["maxy"] and !substr_count($bplace["xy"], "|" . $req["gotox"] . "_" . $req["gotoy"] . "|")) {
			$check_for_go = sql::q1("SELECT uid FROM users WHERE cfight=" . $pers["cfight"] . " and xf=" . intval($req["gotox"]) . " and yf=" . intval($req["gotoy"]) . "");
			if (!$check_for_go['uid']) {
				$check_for_go = sql::q1("SELECT id FROM bots_battle WHERE cfight=" . $pers["cfight"] . " and xf=" . intval($req["gotox"]) . " and yf=" . intval($req["gotoy"]) . "");
				if (!$check_for_go['id']) {
	...
	if ($Checker and $persvs["uid"]) {
		$radius = sql::q1("SELECT MAX(radius) as max FROM wp WHERE uidp=" . $persvs["uid"] . " and weared=1 and type='orujie'")['max'];
