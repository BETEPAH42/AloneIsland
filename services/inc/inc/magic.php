...
$zakl = sql::q1("SELECT * FROM `u_blasts` WHERE `id`='" . $zid . "' and uidp=" . $_pers["uid"] . " and tlevel<=" . $_pers["level"] . " and ts6<=" . $_pers["s6"] . " and cur_colldown<=" . tme() . " and cur_turn_colldown<=" . $_pers["f_turn"]);
...
if ($zakl and $zakl["manacost"] <= $_pers["cma"] / $req["magic_koef"]) {
	...
	$ps = sql::q1("SELECT uid,user,xf,yf,chp,hp,level,sb3,s2,mf2,mf4,fteam,sign,bg,bn,bj,bt,is_art,kb,invisible FROM users WHERE cfight=" . $_pers["cfight"] . " and fteam<>" . $_pers["fteam"] . " and chp>0 and uid<>" . intval($_persvs["uid"]));
	$bs = sql::q1("SELECT id,user,xf,yf,chp,hp,level,s2,mf2,fteam,id_skin,rank_i,kb,magic_resistance,dropfrequency,droptype FROM bots_battle WHERE cfight=" . $_pers["cfight"] . " and fteam<>" . $_pers["fteam"] . " and chp>0 and id<>" . intval($_persvs["id"]));
	...
	while ($i < $zakl["targets"]) {
		...
		$shield = sql::q1("SELECT COUNT(*) as count FROM wp WHERE stype='shit' and weared=1 and uidp=" . $persvs["uid"] . "")['count'];
		...
		//Считаем опыт
		$_pers["exp_in_f"] += experience($ydar, $_pers["level"], $persvs["level"], $persvs["uid"], $persvs['rank_i']);
		//Закончили опыт
	...
	while ($i < $zakl["targets"]) {
		...
		$shield = sql::q1("SELECT COUNT(*) as count FROM wp WHERE stype='shit' and weared=1 and uidp=" . (-1 * $persvs["bid"]) . "")['count'];
		if ($shield) $ydar = floor($ydar / 2);
	...
	sql::q("UPDATE `u_blasts` SET cur_colldown=" . tme() . "+colldown, cur_turn_colldown=turn_colldown+" . $_pers["f_turn"] . " WHERE id=" . $zakl["id"]);

	$_pers = catch_user($_pers["uid"]);

	if ($_pers["chp"] > 0) {
		if (@$_GET["vs_id"]) {
			if ($_GET["vs_id"] > 0) $_persvs = sql::q1("SELECT * FROM users WHERE uid=" . intval($_GET["vs_id"]) . " and chp>0");
			else $_persvs = sql::q1("SELECT * FROM bots_battle WHERE id=" . intval($_GET["vs_id"]) . "  and chp>0");
		}
		if (empty($_GET["vs_id"]) or !$_persvs["user"]) {
			$_persvs = sql::q1("SELECT * FROM users WHERE cfight=" . $_pers["cfight"] . " and fteam<>" . $_pers["fteam"] . " and chp>0");
			if (!$_persvs["uid"])
				$_persvs = sql::q1("SELECT * FROM bots_battle WHERE cfight=" . $_pers["cfight"] . " and fteam<>" . $_pers["fteam"] . " and chp>0");
		}
