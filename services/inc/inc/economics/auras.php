...
if (isset($_POST["zakl"]) and isset($_POST["fornickname"])) {
	...
	if ($persto["location"] == $pers["location"]) {
		if ($v["p_type"] >= 10 and $v["p_type"] <= 12) {
			$special = $v["p_type"] - 7;
			$all = sql::q1("SELECT name FROM p_auras WHERE uid=" . $persto["uid"] . " and special=" . $special . " and esttime>" . tme() . "")['name'];
			if ($all) {
				sql::q("UPDATE p_auras SET esttime=0 WHERE uid=" . $persto["uid"] . " and special=" . $special . " and esttime>" . tme() . " LIMIT 1;");
				...
				sql::q("UPDATE wp SET durability=durability-1 WHERE id=" . $v["id"] . "");
...
if (!empty($_POST["potion"])) {
	...
	$zakl = sql::q1("SELECT `index`,name,id,image,durability FROM `wp` WHERE `id`='" . $zakl . "' and durability>0");
	$acount = sql::q1("SELECT COUNT(*) as count FROM p_auras WHERE uid=" . $pers["uid"] . " and special=13")['count'];
	$Ppers = $pers;
	$potions = sql::q("SELECT params FROM p_auras WHERE uid=" . $pers["uid"] . " and special=13");
	...
	elseif ($zakl and $zakl["durability"] > 0) {
		...
		sql::q("UPDATE wp SET durability=durability-1 WHERE id=" . intval($_POST["potion"]) . "");
	}
	SQL::q("COMMIT;");
}

if (@$_POST["teleport"]) {
	$cell = sql::q1("SELECT * FROM nature WHERE x=" . intval($_POST["X"]) . " and y=" . intval($_POST["Y"]) . "");
	$v = sql::q1("SELECT weight,uidp,where_buy,dprice FROM wp WHERE id=" . intval($_POST["teleport"]) . " and uidp=" . $pers["uid"] . "");
	if (isset($cell["name"]) and isset($v["uidp"])) {
		sql::q("UPDATE users SET x=" . intval($_POST["X"]) . " , y=" . intval($_POST["Y"]) . ", curstate=2 , location='out' WHERE uid=" . $pers["uid"] . "");
		sql::q("UPDATE wp SET durability=durability-1 WHERE id=" . intval($_POST["teleport"]) . "");
...
// подпись бланков, нужно условие нахождения на одной клетке или в локации + доклад о подписи для админа
if ($pers["location"] == "lavka") $_WHOP = "Лавочник";
elseif ($pers["location"] == "admhouse") $_WHOP = "Форин";
elseif ($pers["location"] == "woodgetter") $_WHOP = "Лесник";
elseif ($pers["location"] == "melnica") $_WHOP = "Мельник";
else $_WHOP = "";

if (isset($_POST["blank"]) and isset($_POST["who"])) {
	if ($_POST["who"] <> '') {
		$sostav = sql::q("SELECT * FROM wp ORDER by ID ASC");
		foreach ($sostav as $sost) {
			if ($sost["uidp"] == $pers["uid"] and $sost["id_in_w"] == '14547' and $sost['present'] == $_WHOP) {
				$zz = 1;
				break;
			} else $zz = 0;
		}
		if ($zz == 1) $_RETURN .=  "Вы уже подписывали бланк у " . $_POST["who"] . "!";
		if ($zz == 0) {
			$_RETURN .=  "Вы подписали бланк у " . $_POST["who"] . "!";
			sql::q("UPDATE wp SET id_in_w='14547', image='blank2', name='Подписанный клановый бланк', present='" . $_POST["who"] . "'  WHERE id=" . intval($_POST["blank"]) . "");
		}
	} else $_RETURN .=  "Здесь не укого подписывать!";
}
