...
if (@$_POST["dk"]) {
	$dk = abs($_POST["dk"]);
	if ($dk <= $pers["dmoney"]) {
		sql::q("UPDATE users SET money=money+" . ($dk * $ye_to_ln) . ", dmoney=dmoney-" . $dk . " WHERE uid='" . $pers["uid"] . "'");
...
if (@$_POST["dmoney"]) {
	...
	if ($dk <= $pers["dmoney"]) {
		...
		sql::q("UPDATE clans SET dmoney=dmoney+" . $dk . " WHERE sign='" . $pers["sign"] . "'");
		$pers["dmoney"] -= $dk;
		echo "<script>location='main.php';</script>";
	}
}
if (@$_GET["gopers"] == "service") {
	...
	if ($_GET["do"] == "zeroing" and $pers["dmoney"] >= 5) {
		$pers["dmoney"] -= 5;
		$pers["zeroing"]++;
		sql::q("UPDATE users SET zeroing=zeroing+1,dmoney=dmoney-5 WHERE uid=" . $pers["uid"]);
		echo "<script>location='main.php';</script>";
	}
	if ($_GET["do"] == "szeroing" and $pers["dmoney"] >= 20) {
		$pers["dmoney"] -= 20;
		$pers["zeroing"]++;
		sql::q("UPDATE users SET skill_zeroing=skill_zeroing+1,dmoney=dmoney-20 WHERE uid=" . $pers["uid"]);
		echo "<script>location='main.php';</script>";
	}
	if ($_GET["do"] == "fz" and $pers["dmoney"] >= 10) {
		$pers["dmoney"] -= 10;
		sql::q("UPDATE users SET dmoney=dmoney-10,action=-11 WHERE uid=" . $pers["uid"]);
		echo "<script>location='main.php';</script>";
	}
	if ($_GET["do"] == "obr" and $pers["dmoney"] >= 1) {
		$pers["dmoney"]--;
		$pers["obr"] = 0;
		sql::q("UPDATE users SET obr=0,dmoney=dmoney-1 WHERE uid=" . $pers["uid"]);
		echo "<script>location='main.php?gopers=options';</script>";
	}
	if ($_GET["do"] == "tire" and $pers["dmoney"] >= 1) {
		$pers["dmoney"]--;
		$pers["tire"] = 0;
		sql::q("UPDATE users SET tire=0,dmoney=dmoney-1 WHERE uid=" . $pers["uid"]);
		echo "<script>location='main.php?gopers=options';</script>";
	}
	if ($_GET["do"] == "prg" and $pers["dmoney"] >= 5) {
		$pers["dmoney"] -= 5;
		sql::q("UPDATE users SET coins=coins+5,dmoney=dmoney-5 WHERE uid=" . $pers["uid"]);
		echo "<script>location='main.php';</script>";
	}
	if ($_GET["do"] == "bot" and $pers["dmoney"] >= 0.05) {
		$pers["dmoney"] -= 0.05;
		sql::q("UPDATE users SET lb_attack=0,dmoney=dmoney-0.05 WHERE uid=" . $pers["uid"]);
		echo "<script>location='main.php';</script>";
	}
	if ($_GET["do"] == "bot3" and $pers["dmoney"] >= 0.1) {
		$pers["dmoney"] -= 0.1;
		sql::q("UPDATE users SET dmoney=dmoney-0.1 WHERE uid=" . $pers["uid"]);
		$SPECIAL_pers = $pers;
		$SPECIAL_count = 3;
	...
	if ($_GET["do"] == "bot6" and $pers["dmoney"] >= 0.2) {
		$pers["dmoney"] -= 0.2;
		sql::q("UPDATE users SET dmoney=dmoney-0.2 WHERE uid=" . $pers["uid"]);
		$SPECIAL_pers = $pers;
		$SPECIAL_count = 6;
		include("bots/attack.php");
	}
...
if (@$_POST["user"] and $pers["dmoney"] >= 100) {
	$err = 0;
	if (strlen($_POST["user"]) < 3 or strlen($_POST["user"]) > 21) {
		print "Некорректный Логин.(Величина)<hr>";
		$err = 1;
	}
	if (
		...
		print "Некорректный Логин.(Нельзя использовать специальные символы в нике)<hr>";
		$err = 1;
	}
	if ($err == 0) {
		sql::q("UPDATE users SET smuser=LOWER('" . $_POST["user"] . "'),user='" . $_POST["user"] . "',dmoney=dmoney-100 WHERE uid=" . $pers["uid"]);
...
$req = sql::q1("SELECT uid FROM avatar_request WHERE uid=" . $pers["uid"])['uid'];
if (@$_FILES and !$req and $pers["dmoney"] >= 30) {
	if ($_FILES['obr']['type'] == 'image/gif') {
		...
		if ($im) {
			...
			sql::q("INSERT INTO `avatar_request` (`uid`)VALUES ('" . $pers["uid"] . "');");
			set_vars("dmoney=dmoney-30", $pers["uid"]);
			$req = 1;
		}
	}
	if (preg_match('image/?jpeg', $_FILES['obr']['type'])) {
...
echo "<table border=0 class=but2 width=90%>";
if (!$req) {
	echo '<tr>
...
<table border="0" width="90%" cellspacing="5" class="but">
	<tr>
