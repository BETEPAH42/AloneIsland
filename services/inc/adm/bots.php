...
if (@$_GET["deletebot"]) {
	sql::q("DELETE FROM bots WHERE id>=" . $_GET["start"] . " and id<=" . $_GET["finish"] . "");
}
...
if (@$_GET["wear"]) {
	...
	for ($i = intval($p["minlvl"]); $i <= intval($p["maxlvl"]); $i++) {
		...
		$bid = sql::q1("SELECT id FROM bots WHERE user='" . $p["user"] . "' and level=" . $i);
		if ($bid['id'])
			insert_wp($r, -1 * $bid['id'], -1, 1, $p["user"]);
	}
}
...
if (@$_GET["unwear"]) {
	...
	for ($i = intval($p["minlvl"]); $i <= intval($p["maxlvl"]); $i++) {
		$bid = sql::q1("SELECT id FROM bots WHERE user='" . $p["user"] . "' and level=" . $i);
		if ($bid['id'])
			sql::q1("DELETE FROM wp WHERE weared=1 and uidp=" . (-1 * $bid['id']) . " and stype='" . $p["stype"] . "'");
	}
}

if (@$_GET["attack"]) {
	$bid = sql::q1("SELECT id FROM bots WHERE user='" . $_POST["name"] . "' and level=" . intval($_POST["lvl"]) . "");
	begin_fight($pers["user"], "bot=" . $bid['id'] . "|", "Тестирующий бой", 50, 300, 1, 0);
...
if (@$_GET["added"] and $_POST["balance"] != 1) {
	...
	$botlastid = sql::q1("SELECT MAX(id) as max FROM bots");
	$botlastid = floor($botlastid['max'] / 100) * 100 + 100;
	...
	for ($i = $p["minlvl"]; $i <= $p["maxlvl"]; $i++) {
		$summ = sql::q1("SELECT SUM(stats) as sum FROM exp WHERE level<=" . $i) + 20;
		$params = Class_Params($i, $class, "mech", $p["power"] / 100);
		$params["s1"] += 0.3 * $summ['sum'];
		$params["s2"] += 0.2 * $summ['sum'];
		$params["s3"] += 0.2 * $summ['sum'];
		$params["s4"] += 0.3 * $summ['sum'];
		$params["hp"] += $params["s4"] * 5;
		$params["kb"] += $params["s4"];


		sql::q("INSERT INTO `bots` 
...
if (@$_GET["skin"]) {
	$BN = sql::q1("SELECT user FROM bots WHERE id='" . intval($_GET["skin"]) . "'");
	sql::q("UPDATE bots SET id_skin=" . intval($_POST["skin"]) . " WHERE user='" . $BN['user'] . "'");
}
...
if (@$_GET["delete"]) {
	$u = sql::q1("SELECT user FROM bots WHERE id=" . intval($_GET["delete"]) . "");
	sql::q1("DELETE FROM bots WHERE user='" . $u['user'] . "'");
}

$allbots_names = sql::q("SELECT * FROM bots GROUP BY user;");

$JS = '';
$SKIN_SEL = '<option value="0">Нет</option>';
$SKINS = sql::q("SELECT * FROM skins");
...
foreach ($allbots_names as $bn) {
	$lvls = sql::q1("SELECT MAX(level) as maxlvl, MIN(level) as minlvl,MAX(rank_i) as maxrank, MIN(rank_i) as minrank FROM bots WHERE user='" . $bn["user"] . "'");
	echo "<tr>";
	echo "<td class=timef>";
