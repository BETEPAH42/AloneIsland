...
if ($priv["ewp"] == 2) {
	...
	if (@$_GET["copy"]) {
		...
		$part2 = ") VALUES (";
		$v = sql::q("SELECT * FROM weapons WHERE id='" . $_GET["copy"] . "'");
		foreach ($v as $key => $a) {
			if ($key == 'idn') $a = $mid;
			if ($key == 'id') $a = $mid;
			$part1 .= "`" . $key . "`,";
...
<table border=0 width=100% class=inv>
	...
	<tr>
		<td>
			<center>
				<table width=70%>
					<tr>
						<td width=33%><a href=main.php?a=1&set_type=fish class=bg>Рыба</a></td>
						<td width=33%><a href=main.php?a=1&set_type=blank class=bg>Документы</a></td>
						<td width=33%><a href=main.php?a=1&set_type=quest class=bg>Квестовые вещи</a></td>
					</tr>
				</table>
			</center>
		</td>
		...
		<td valign="top" align=center>
			<form method="POST" action='main.php'>
				<font class=time>
					...
					</select>от<input type="text" name="minlevel" size="7" value="<? if ($_FILTER["lavkaminlevel"] <> "") echo $_FILTER["lavkaminlevel"];
																					else echo "0"; ?>" class=laar>до<input type="text" name="maxlevel" size="7" value=<? if ($_FILTER["lavkamaxlevel"] <> "") echo $_FILTER["lavkamaxlevel"];
																																										else echo $pers["level"]; ?> class=laar>Уровня.&nbsp;
					Не дороже <input type="text" name="maxcena" size="7" value="<? if ($_FILTER["lavkamaxcena"] <> "") echo $_FILTER["lavkamaxcena"];
																				else echo "1000"; ?>" class=laar><input type="submit" value="Ок" class=loc></font>
	...
	<td class=time>Кол-во в лавке:" . $v["q_s"] . "</td>";
						...
						elseif ($v["where_buy"] == 3) echo "<td><font color=orange><b>купить на Маяке</b></font></td>";
