...
if (@$_POST["name"] and $_FILES["sign"]["type"] == "image/gif") {
	...
	$idl = intval($idl['max']);
	$idl++;
	sql::q("INSERT INTO `clans` ( `id` , `name` , `glav` , `sign` , `sait` ) VALUES ('" . $idl . "', '" . addslashes($_POST["name"]) . "', '" . addslashes($_POST["glav"]) . "', 'c" . $idl . "', '" . addslashes($_POST["site"]) . "');");
	sql::q("UPDATE users SET sign='c" . $idl . "',rank='<glav>',state='Глава клана' WHERE user='" . addslashes($_POST["glav"]) . "'");
	print "<pre>";
...
if (@$_POST["nick"] and $_FILES["obr"]["type"] == "image/gif") {
	$obr = sql::q1("SELECT MAX(obr) as max FROM users");
	$obr = $obr['max'] + 1;
	$pers = sql::q1("SELECT pol FROM users WHERE user='" . addslashes($_POST["nick"]) . "'");
	...
	sql::q("UPDATE users SET obr='" . $obr . "' WHERE user='" . addslashes($_POST["nick"]) . "'");
...
if (@$_POST["nick2"] and $_FILES["pict"]["type"] == "image/gif") {
	...
	sql::q("UPDATE users SET image_in_info='images/titles/" . $t . ".gif' WHERE user='" . addslashes($_POST["nick2"]) . "'");
...
if (@$_POST["nick3"]) {
	$pers = sql::q1("SELECT uid FROM users WHERE user='" . addslashes($_POST["nick3"]) . "'")['uid'];
	$a = sql::q1("SELECT COUNT(id) as count FROM wp WHERE uidp=" . $pers . " and weared=1 and `type`='" . addslashes($_POST["type"]) . "'")['count'];
	sql::q("UPDATE wp SET image='" . addslashes($_POST["wpname2"]) . "' WHERE uidp=" . $pers . " and weared=1 and `type`='" . addslashes($_POST["type"]) . "'");
		...
		<table border="0" width="100%" cellspacing="0" cellpadding="0">
			...
			<tr>
				<td colspan="2" class="laar" align="center">
					<input class="laar" type="submit" value="Окай">
				</td>
			</tr>
		</table>
	</form>
	...
	<form enctype="multipart/form-data" method=post>
		<table border="0" width="100%" cellspacing="0" cellpadding="0">
			...
			<tr>
				...
				<td width="82%"><select size="1" name="type" class="login">
						<option selected value="shlem">Шлем</option>
						<option value="ojerelie">Кулон</option>
						<option value="orujie">Оружие</option>
						<option value="poyas">Пояс</option>
						<option value="zelie">Зелье/камень</option>
						<option value="sapogi">Сапоги</option>
						<option value="naruchi">Наручи</option>
						<option value="perchatki">Перчатки</option>
						<option value="kolco">Кольцо</option>
						<option value="bronya">Броня</option>
					</select></td>
			</tr>
			<tr>
				<td colspan="2" class="laar" align="center">
