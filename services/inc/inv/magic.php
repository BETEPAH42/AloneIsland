...
if (@$_GET["aura_delete"]) {
	SQL::q("DELETE FROM u_auras WHERE uidp=" . $pers["uid"] . " and id=" . intval($_GET["aura_delete"]));
}

if (@$_GET["autocast"]) {
	$cast = intval($_GET["autocast"]);
	$a = sql::q1("SELECT autocast FROM u_auras WHERE id=" . $cast)['autocast'];
	if ($a) $a = 0;
	else $a = 1;
	sql::q("UPDATE u_auras SET autocast=" . $a . " WHERE id=" . $cast);
	if ($a == 0)
		sql::q("UPDATE p_auras SET autocast=0 WHERE autocast=" . $cast);
	else
...
if ($type == "blast") {
	$blsс = sql::q1("SELECT COUNT(*) as count FROM u_blasts WHERE uidp=" . $pers["uid"] . "")['count'];
	$bls = sql::q("SELECT * FROM u_blasts WHERE uidp=" . $pers["uid"]);
...
if ($type == "aura") {
	...
	$blsс = sql::q1("SELECT COUNT(*) as count FROM u_auras WHERE uidp=" . $pers["uid"] . "")['count'];
	$bls = sql::q("SELECT * FROM u_auras WHERE uidp=" . $pers["uid"] . $q);
