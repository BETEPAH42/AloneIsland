...
<body topmargin="15" leftmargin="15" rightmargin="15" bottommargin="15" class=fightlong style="overflow:hidden;">
	...
	if ($_COOKIE["uid"])
		$pers = sql::q1("SELECT * FROM `users` WHERE `uid`=" . intval($_COOKIE["uid"]) . "");
...
if (!$_SPEECH)
		...
		if (($sp["relation"] > 0 and $R > $sp["relation"]) or ($sp["relation"] < 0 and $R < $sp["relation"]) or $sp["relation"] == 0) {
			...
			if ($sp["showcounts"]) {
				$c = sql::q1("SELECT COUNT(*) as count FROM u_speech WHERE uid=" . $pers["uid"] . " and sid=" . $sp["id"])['count'];
				if ($c)
		...
		*/
			...
			if ($sp["action"] == 10) {
				$a = sql::q1("SELECT name FROM p_auras WHERE uid=" . $pers["uid"] . " and special>=3 and special<=5 and esttime>" . tme() . " LIMIT 1;")['name'];
