...
";
		...
		if (isset($_GET["zero_skill"]) and substr_count(implode("@", $sps) . "@", $cat . "@") and $pers["skill_zeroing"]) {
			$wp_sk = intval(sql::q1("SELECT SUM(`" . $cat . "`) as sum FROM wp WHERE uidp=" . $pers["uid"] . " and weared=1")['sum']);
			$pers[$cat] -= $wp_sk;
		...
		if ($pers["waiter"] < tme()) {
			if (@$_GET["learn"] and substr_count(implode("@", $sps) . "@", $_GET["learn"] . "@")) {
				$cat = $_GET["learn"];
				$wp_sk = intval(sql::q1("SELECT SUM(`" . $cat . "`) as um FROM wp WHERE uidp=" . $pers["uid"] . " and weared=1")['sum']);
				$pers[$cat] -= $wp_sk;
				...
				elseif (intval($pers[$cat] + 10) > (100 + 70 * $pers["level"]))
					...
					set_vars(
						"`" . $cat . "`=`" . $cat . "`+20,money=money-" . intval($pers[$cat]) . ",coins=coins-" . $ps . ",waiter=" . tme() . "+300",
						$pers["uid"]
					);
					$pers["waiter"] = tme() + 300;
...
if (@$_GET["Dlearn"] and substr_count(implode("@", $sps) . "@", $_GET["learn"] . "@")) {
				...
				foreach ($sps as $value) {
					$wp_sk = intval(sql::q1("SELECT SUM(`" . $value . "`) as sum FROM wp WHERE uidp=" . $pers["uid"] . " and weared=1")['sum']);
