<center>
	<font class=hp>
		<?
		if (@$_POST["id"]) {
			$id = sql::q1("SELECT * FROM presents WHERE id='" . intval($_POST["id"]) . "'");
			if (isset($id["id"])) {
				$persto = sql::q1("SELECT uid,user,location FROM users WHERE user='" . addslashes($_POST["towho"]) . "'");
				if (@$persto["uid"] and $pers["money"] > $id["price"]) {
					sql::q("INSERT INTO `presents_gived` ( `uid` , `name` , `image` , `date` , `who` , `anonymous` , `text` ) VALUES ('" . $persto["uid"] . "', '" . $id["name"] . "', '" . $id["image"] . "', '" . time() . "', '" . $pers["user"] . "', '" . intval($_POST["anonymous"]) . "', '" . $_POST["p"] . "');");
					sql::q("UPDATE users SET money=money-" . $id["price"] . " WHERE uid='" . $pers["uid"] . "'");
			...
			<tr>
				<td width=800 colspan=9 class=fightlong height=17>
					<table border="1" width="100%" cellspacing="0" cellpadding="0" bordercolorlight=#C0C0C0 bordercolordark=#FFFFFF>
						<?
						$presents = SQL::q("SELECT * FROM presents WHERE id>86 ORDER BY price ASC");
						$i = 0;
