		...
		if (empty($_GET["learn"]))
			...
			if ($pers["waiter"] < tme()) {
				if (@$_GET["learn"]) {
					...
					$your = sql::q1("SELECT * FROM u_special_dmg WHERE uid=" . $pers["uid"] . " and wod=" . $cat . "");
					$def = sql::q1("SELECT * FROM special_dmg WHERE od=" . $cat . "");
					$z = 0;
					if ($your) {
						...
						if ($money <= $pers["money"] and $ps <= $pers["coins"]) {
							sql::q1("UPDATE _special_dmg SET od=od+10,value=value+" . $def["value"] . " 
			WHERE uid=" . $pers["uid"] . " and wod=" . $cat . "");
						...
						$ps = $def["coins"];
						if ($money <= $pers["money"] and $ps <= $pers["coins"]) {
							SQL::q("INSERT INTO `u_special_dmg` ( `uid` , `od` , `name` , `type` , `value` , `coins` , `wod` )
VALUES (
'" . $pers["uid"] . "', '" . $def["od"] . "', '" . $def["name"] . "', '" . $def["type"] . "', '" . $def["value"] . "', '" . $def["coins"] . "', '" . $def["od"] . "');");
					...
					$spcls = sql::q("SELECT * FROM special_dmg");
					foreach ($spcls as $sp) {
