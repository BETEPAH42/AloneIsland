...
if (substr_count($pers["rank"],"<molch>") or $pers["diler"]=='1' or $pers["priveleged"]);else
{
	...
	if (empty($_POST) and empty($_GET["id"])) {
		...
		$vs = sql::q("SELECT * FROM wp WHERE uidp=" . UID . " and dprice=0 and where_buy=0 and weared=0  and " . $UPGR . " and price>" . MIN_PRICE . " and price<=" . $MAX_PRICE . "");
...
if (empty($_POST) and isset($_GET["id"]) and empty($_GET["param"])) {
		$v = sql::q1("SELECT * FROM wp WHERE uidp=" . UID . " and dprice=0 and where_buy=0 and weared=0  and " . $UPGR . " and price>" . MIN_PRICE . " and price<=" . $MAX_PRICE . " and id='" . intval($_GET["id"]) . "'");
		if (!$v)
...
if (empty($_POST) and isset($_GET["id"]) and isset($_GET["param"]) and empty($_GET["up"])) {
		...
		if (!$v or $v[$param] == 0)
			...
			$mRes = sql::q1("SELECT MAX(image) as max FROM resources")['max'];
			$z = uncrypt(md5($param . $v["stype"]));
			$z = floor($z / 100 + $z % 10000);
			$z = $z % $mRes;
			if ($z == 0) $z = $mRes;
			$r = sql::q1("SELECT * FROM resources WHERE image='" . $z . "'");
			$yours = intval(sql::q1("SELECT SUM(durability) as sum FROM wp WHERE uidp=" . UID . " and type='resources' and image='resources/" . $r["image"] . "'")['sum']);
...
if (empty($_POST) and isset($_GET["id"]) and isset($_GET["param"]) and isset($_GET["up"])) {
		...
		if (!$v or $v[$param] == 0)
			...
			elseif ($chance > rand(1, 100)) {
				$yours = sql::q("SELECT id,durability FROM wp WHERE uidp=" . UID . " and type='resources' and image='resources/" . $r["image"] . "'");
				$tmp = 0;
				foreach ($yours as $yr) {
					$tmp += $yr["durability"];
					if ($tmp < $k)
						sql::q("UPDATE wp SET durability=0 WHERE id=" . $yr["id"] . "");
					else
						sql::q("UPDATE wp SET durability=" . ($tmp - $k) . ",weight=weight-" . $k . " WHERE id=" . $yr["id"] . "");
				}

				$v["name"] .= '(ап)';
				sql::q("UPDATE wp SET `" . $param . "` = `" . $param . "` + " . ($kk * $eq) . ",upgrated=upgrated+1,name='" . $v["name"] . "',price=price+" . UP_COST . "*" . $kk . " WHERE id=" . $v["id"] . "");
				$v[$param] += $kk * $eq;
				$v["price"] += UP_COST * $kk;
				$v["upgrated"]++;
				$vesh = $v;
