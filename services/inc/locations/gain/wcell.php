...
if ($_GET["wcell"] == 'start') {
	...
	if ($have_cells)
		$near_cells = sql::q1("SELECT COUNT(*) as count FROM nature WHERE x>=" . ($x - 1) . " and x<=" . ($x + 1) . " and y>=" . ($y - 1) . " and y<=" . ($y + 1) . " and belong=" . $pers["uid"] . "")['count'];
...
if ($_GET["wcell"] == 'go') {
	...
	if ($have_cells and !$near_cells) echo "Вы не можете завоевать земли вдали от уже завоеванных.";
		...
		$perstowho = sql::q1("SELECT * FROM users WHERE x=" . $x . " and y=" . $y . " and online=1 and gain_time<>0 and uid<>" . $pers["uid"] . " LIMIT 0,1");
		if ($perstowho) {
			if ($perstowho["cfight"] > 10) {
				...
				if ($fight["type"] <> 'f' and $fight["id"]) {
					...
					sql::q("UPDATE `users` SET `curstate`=4 , `cfight`='" . $fight["id"] . "', fteam=" . (($perstowho["fteam"] + 1) % 2) . ",refr=1 WHERE `uid`='" . $pers["uid"] . "'");
...
if ($pers["gain_time"] > 0 and $pers["gain_time"] <= (tme() - 1200)) {
	...
	sql::q("UPDATE nature SET belong=" . $pers["uid"] . " WHERE x=" . $x . " and y=" . $y . "");
	set_vars("gain_time=0", $pers["uid"]);
}
