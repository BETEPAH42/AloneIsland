	...
	if ($z == 1) {
		...
		if (empty($_POST["primid"]) and $pers["waiter"] < tme()) {
			echo "<center class=fightlong><form action=main.php?fishing=on method=post><input type=hidden name=check value='" . md5($lastom_new . "1") . "'>
				<table border=0 class=LinedTable width=70%><tr><td align=center colspan=7><input type=submit class=login value='Ловить' style='width:100%'></td></tr>";
			$res = sql::q("SELECT durability,max_durability,id,image,name FROM wp WHERE uidp=" . $pers["uid"] . " and weared=0 and p_type=1 and durability>0 and type<>'orujie' and sp6=0");
			foreach ($res as $v) {
				$a++;
				echo "<tr><td valing=center width=58><img src='images/weapons/fishing_prim/" . $v["image"] . ".gif'></td><td class=user>" . $v["name"] . "";
				##################################### Нужно группировать и выводить колличество
				//if ($v["name"]=='Перловая каша' and $v["durability"]==$v["max_durability"]) $g++; 
				//if ($g>1)echo "(".$g." шт.)";
				#####################################
				echo "</td><td class=timef><center>[";
				...
				echo "<font color=" . $col . ">" . $v['durability'] . "</font> / 
						...
						<td><center><input type=radio name=primid value=" . $v['id'] . "></center></td></tr>";
			}
			...
			$i = 0;
			while ($i < 3) //количестов выловленного за один заброс
			{
				...
				if (@$v["id"]) {
					...
					if (rand(1, 100) < 5 and round(10.0 / sqrt($pers["sp6"]), 2) > 0) {
						...
						sql::q("UPDATE wp SET durability=durability-1 WHERE uidp=" . $pers["uid"] . " and weared=0 and id=" . $v["id"] . "");
						echo "Вы поймали труселя.<br><img src=images/weapons/fish/p.png><br>Ваше умение \"Рыбак\" выросло на " . round(15.0 / (sqrt($pers["sp6"]) + 1), 2) . "!<br> Мирный опыт +1";
						set_vars("sp6=sp6+" . round(20.0 / (sqrt($pers["sp6"]) + 1), 2) . ", peace_exp=peace_exp+1", $pers["uid"]);
					} elseif (rand(1, 500) < 3) {
						sql::q("UPDATE wp SET durability=durability-1 WHERE uidp=" . $pers["uid"] . " and weared=0 and id=" . $v["id"] . "");
						...
						$fish = sql::q1("SELECT * FROM fish WHERE skill<" . ($pers["sp6"] + $skill_p) . " and place=" . $cell["fishing"] . " and (prim=" . intval(str_replace("fishing_prim_", "", $v["image"])) . " or prim=" . (intval(str_replace("fishing_prim_", "", $v["image"]) + 1) % 8 + 1) . ") ORDER BY RAND()"); //указание на приманку
						if (rand(sqrt($cell["fish_population"]), 150) < ($fish["no_kl"] - sqrt($pers["sp6"] + $skill_p) + 10) or $cell["fish_population"] == 0) {
							echo "Нет клёва.";
							$ii++;
							if ($ii == 3) $ii = 2;
						} elseif ($fish["id"] == 0) {
							if (sql::q1("SELECT COUNT(*) as count FROM fish WHERE skill<" . ($pers["sp6"] + $skill_p) . " and place=" . $cell["fishing"])['count'])
							...
							$vesh = sql::q1("SELECT * FROM wp WHERE id=" . $vesh . "");
							$k = rand(-3, 3);
							...
							sql::q("UPDATE wp SET price=" . $vesh["price"] . ", weight=" . $vesh["weight"] . ",timeout=" . (tme() + 345600) . ",`describe`='" . $l . "',name='" . $vesh["name"] . "',image='" . $vesh["image"] . "'  WHERE id=" . $vesh["id"] . "");
							$vesh["timeout"] = (tme() + 345600);
			...
			set_vars("waiter=" . round(tme() + FISHING_TIME * ($i - $ii)) . ",tire=tire+2*" . $i . "", $pers["uid"]);
			$pers["waiter"] = round(tme() + FISHING_TIME * ($i - $ii));
			//		if($pers["waiter"]>tme())
