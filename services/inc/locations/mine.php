...
$tl = SQL::q1("SELECT * FROM mine WHERE x=" . ($pers["minex"] - 1) . " and y=" . $pers["miney"] . " and mine=" . $MINE_ID . "");
$td = SQL::q1("SELECT * FROM mine WHERE x=" . ($pers["minex"]) . " and y=" . ($pers["miney"] + 1) . " and mine=" . $MINE_ID . "");
$tu = SQL::q1("SELECT * FROM mine WHERE x=" . ($pers["minex"]) . " and y=" . ($pers["miney"] - 1) . " and mine=" . $MINE_ID . "");
...
if ($t % 3000 == 0) {
	$mperses = sql::q("SELECT user,level FROM users WHERE location='mine' and cfight=0 and apps_id=0 and online=1");
	$bots_str = '';
	$users_str = '';
	foreach ($mperses as $mpers) {
...
if (@$_GET["minego"] and $pers["waiter"] <= $t) {
	$res = "SELECT * FROM resources ORDER BY RAND()";
	$r1 = SQL::q1($res);
	$r2 = SQL::q1($res);
	$r3 = SQL::q1($res);
	...
	if ($_GET["minego"] == 'left') {
		...
		if (!$tl["mine"]) sql::q("INSERT INTO `mine` ( `x` , `y` , `time_ready` , `r1id` , `r2id` , `r3id` , `r1k` , `r2k` , `r3k` ,`mine` , `countp` ) VALUES ('" . ($pers["minex"]) . "', '" . ($pers["miney"]) . "', '" . ($t + $timep - $pers["sp7"]) . "','" . $r1["image"] . "', '" . $r2["image"] . "', '" . $r3["image"] . "', '" . $kr1 . "', '" . $kr2 . "', '" . $kr3 . "', '" . $MINE_ID . "', '1');");
		elseif ($tl["time_ready"] > $t)
			sql::q("UPDATE mine SET time_ready=" . ($t + ($tl["time_ready"] - $t) * ($tl["countp"] - 1) / $tl["countp"]) . ",countp=countp+1 WHERE x='" . ($pers["minex"]) . "' and y='" . ($pers["miney"]) . "' and mine=" . $MINE_ID . "");
	}
	if ($_GET["minego"] == 'right') {
		$pers["minex"] = $pers["minex"] + 1;
		$pers["miney"] = $pers["miney"];
		if (!$tr["mine"]) sql::q("INSERT INTO `mine` ( `x` , `y` , `time_ready` , `r1id` , `r2id` , `r3id` , `r1k` , `r2k` , `r3k` ,`mine` , `countp` ) VALUES ('" . ($pers["minex"]) . "', '" . ($pers["miney"]) . "', '" . ($t + $timep - $pers["sp7"]) . "',	'" . $r1["image"] . "', '" . $r2["image"] . "', '" . $r3["image"] . "', '" . $kr1 . "', '" . $kr2 . "', '" . $kr3 . "', '" . $MINE_ID . "', '1');");
		elseif ($tr["time_ready"] > $t)
			sql::q("UPDATE mine SET time_ready=" . ($t + ($tr["time_ready"] - $t) * ($tr["countp"] - 1) / $tr["countp"]) . ",countp=countp+1 WHERE x='" . ($pers["minex"]) . "' and y='" . ($pers["miney"]) . "' and mine=" . $MINE_ID . "");
	}
	if ($_GET["minego"] == 'up') {
		$pers["minex"] = $pers["minex"];
		$pers["miney"] = $pers["miney"] - 1;
		if (!$tu["mine"]) sql::q("INSERT INTO `mine` ( `x` , `y` , `time_ready` , `r1id` , `r2id` , `r3id` , `r1k` , `r2k` , `r3k`, `mine` , `countp` ) VALUES ('" . ($pers["minex"]) . "', '" . ($pers["miney"]) . "', '" . ($t + $timep - $pers["sp7"]) . "','" . $r1["image"] . "', '" . $r2["image"] . "', '" . $r3["image"] . "', '" . $kr1 . "', '" . $kr2 . "', '" . $kr3 . "', '" . $MINE_ID . "', '1'	);");
		elseif ($tu["time_ready"] > $t)
			sql::q("UPDATE mine SET time_ready=" . ($t + ($tu["time_ready"] - $t) * ($tu["countp"] - 1) / $tu["countp"]) . ",countp=countp+1 WHERE x='" . ($pers["minex"]) . "' and y='" . ($pers["miney"]) . "' and mine=" . $MINE_ID . "");
	}
	if ($_GET["minego"] == 'down') {
		$pers["minex"] = $pers["minex"];
		$pers["miney"] = $pers["miney"] + 1;
		if (!$td["mine"]) sql::q("INSERT INTO `mine` 
		...
		elseif ($td["time_ready"] > $t)
			sql::q("UPDATE mine SET time_ready=" . ($t + ($td["time_ready"] - $t) * ($td["countp"] - 1) / $td["countp"]) . ",countp=countp+1 WHERE x='" . ($pers["minex"]) . "' and y='" . ($pers["miney"]) . "' and mine=" . $MINE_ID . "");
	}
...
$tunnel = sql::q1("SELECT * FROM mine WHERE x=" . $pers["minex"] . " and y=" . $pers["miney"] . " and mine=" . $MINE_ID . "");
############################################
/*
if (!$tunnel["r1k"] and !$tunnel["r2k"] and !$tunnel["r3k"] and $t%20==0)
SQL::q("UPDATE mine SET r1k=r1k+".rand(1,30).",r2k=r2k+".rand(2,15).",r3k=r3k+".rand(15,40)." WHERE x=".$tunnel["x"]." and y=".$tunnel["y"]." and mine=".$tunnel["mine"]."");
*/
############################################
$inst = sql::q1("SELECT id,udmin,udmax,durability,price FROM wp WHERE uidp=" . $pers["uid"] . " and weared=1 and p_type=5");
if (!$inst["id"]) $no_make = 1;
if (@$_GET["beginr"] and !$no_make and $pers["waiter"] < $t and $pers["tire"] < 100) {
	include("inc/resource.php");
	$inst = sql::q1("SELECT id,udmin,udmax,durability FROM wp WHERE uidp=" . $pers["uid"] . " and weared=1 and p_type=5");
}
############################################	
...
if (empty($tr["mine"]) or $tr["time_ready"] > $t) $cltr = 'class=fader onclick="go_confirm(\'right\')"';
else $cltr = 'onclick="location=\'main.php?minego=right\'"';
if (empty($tl["mine"]) or $tl["time_ready"] > $t) $cltl = 'class=fader onclick="go_confirm(\'left\')"';
else $cltl = 'onclick="location=\'main.php?minego=left\'"';
if (empty($tu["mine"]) or $tu["time_ready"] > $t) $cltu = 'class=fader onclick="go_confirm(\'up\')"';
else $cltu = 'onclick="location=\'main.php?minego=up\'"';
if (empty($td["mine"]) or $td["time_ready"] > $t) $cltd = 'class=fader onclick="go_confirm(\'down\')"';
else $cltd = 'onclick="location=\'main.php?minego=down\'"';

$x = $pers["minex"];
$y = $pers["miney"];

$cells_around = sql::q("SELECT x,y,time_ready FROM mine WHERE x>=" . ($pers["minex"] - 3) . " and x<=" . ($pers["minex"] + 3) . " and y>=" . ($pers["miney"] - 2) . " and y<=" . ($pers["miney"] + 2) . " and mine=" . $MINE_ID . "");

$maked_str = array();
foreach ($cells_around as $cc)
	if ($cc["time_ready"] < $t)
...
if ($tunnel["r1id"])
	$r1 = sql::q1("SELECT * FROM resources WHERE image='" . $tunnel["r1id"] . "'");
$r1["k"] = $tunnel["r1k"];
if ($tunnel["r2id"])
	$r2 = sql::q1("SELECT * FROM resources WHERE image='" . $tunnel["r2id"] . "'");
$r2["k"] = $tunnel["r2k"];
if ($tunnel["r3id"])
	$r3 = sql::q1("SELECT * FROM resources WHERE image='" . $tunnel["r3id"] . "'");
$r3["k"] = $tunnel["r3k"];
...
if ($r2["image"] and ($r2["price"] < 2 or $r2["price"] < $pers["sp7"] / 38) and mtrunc($r2["k"])) {
	if (!$no_make) $begin = '<input class=laar type=button value="[начать добычу]" onclick="location=\'main.php?beginr=' . $r2["image"] . '\'">';
	else $begin = '';
	$resources .= '<table border=0 width=100%>';
	$resources .= '<tr>';
...
if ($r3["image"] and ($r3["price"] < 2 or $r3["price"] < $pers["sp7"] / 38) and mtrunc($r3["k"])) {
	if (!$no_make) $begin = '<input class=laar type=button value="[начать добычу]" onclick="location=\'main.php?beginr=' . $r3["image"] . '\'">';
	else $begin = '';
	$resources .= '<table border=0 width=100%>';
	$resources .= '<tr>';
...
$_r = sql::q("SELECT * FROM wp WHERE type='ruda' and uidp='" . $pers["uid"] . "'");
foreach ($_r as $v) {
	$resources .= "<tr>";
...
<table border="0" width="100%" cellspacing="2" cellpadding="2">
	<tr>
		...
		<td class=fightlong align=center width=30% valign=top><b><i>Обнаруженные ресурсы:</i></b><br>
			<?
			if ($pers["tire"] < 100) echo $resources;
			else echo "Вы слишком устали.";
			?>
		</td>
	</tr>
</table>
