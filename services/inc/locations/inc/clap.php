...
if (substr_count($pers["rank"], "<glav>")) {
	if (empty($_POST) and empty($_GET["buy"])) {
		...
		$r = sql::q("SELECT name,id FROM wp WHERE uidp='" . $pers["uid"] . "' and where_buy=1 and id_in_w='' and clan_sign='" . $pers["sign"] . "' and weared=0");
		$i = 0;
		foreach ($r as $p) {
			$i++;
		...
		$vesh = sql::q1("SELECT * FROM wp WHERE id='" . $_POST["id"] . "' and uidp='" . $pers["uid"] . "' and where_buy=1 and id_in_w='' and clan_sign='" . $pers["sign"] . "' and weared=0");
...
' . $dop . '
	...
	<tr>
		...
		$lastid = sql::q1("SELECT MAX(id) FROM wp");
		$lastid = 1 + $lastid[0];
		$vesh = sql::q1("SELECT * FROM wp WHERE id='" . $_POST["idd"] . "' and uidp='" . $pers["uid"] . "' and where_buy=1 and id_in_w='' and clan_sign='" . $pers["sign"] . "'");
		$type  = $vesh["stype"];
		$oldp = $vesh["dprice"];
		$oldid = $vesh["id"];
		$dprice = 30;
		...
		sql::q("INSERT INTO `wp` (`id`,`name`,`s1`,`s2`,`s3`,`s5`,`s6`,`mf1`,`mf2`,`mf3`,`mf4`,`mf5`,`kb`,`hp`,`ma`,`udmin`,`udmax`,`describe`,`dprice`,`image`,`type`,`where_buy`,`stype`,`weight`,`max_durability`,durability,`index`,`slots`,`radius`) VALUES ('" . $lastid . "','" . $vesh["name"] . "','" . $p["s1"] . "','" . $p["s2"] . "','" . $p["s3"] . "','" . $p["s5"] . "','" . $p["s6"] . "','" . $p["mf1"] . "','" . $p["mf2"] . "','" . $p["mf3"] . "','" . $p["mf4"] . "','" . $p["mf5"] . "','" . $p["kb"] . "','" . $p["hp"] . "','" . $p["ma"] . "','" . $p["udmin"] . "','" . $p["udmax"] . "','Индивидуальный артефакт','" . $dprice . "','" . $vesh["image"] . "','" . $vesh['type'] . "',1,'" . $vesh["stype"] . "','1','1','1','','" . $p["slots"] . "','" . $p["radius"] . "');");
		...
		$vesh = sql::q1("SELECT * FROM wp WHERE id='" . $_GET["buy"] . "'");
		$old = sql::q1("SELECT * FROM wp WHERE id='" . $_GET["old"] . "' and weared=0 and uidp=" . $pers["uid"] . "");
		$clan = sql::q1("SELECT * FROM `clans` WHERE `sign`='" . $pers['sign'] . "'");
		if ($old["image"] == $vesh["image"]) {
			if ($old["dprice"] and $clan["dmoney"] >= abs($vesh["dprice"] - $old["dprice"]) and $vesh["uidp"] == 0) {
				SQL::q("UPDATE wp SET uidp=" . $pers["uid"] . ",durability=1,clan_sign='" . $pers["sign"] . "',clan_name='" . $clan["name"] . "',present='" . $pers["user"] . "' WHERE id='" . $_GET["buy"] . "'");
				sql::q("DELETE FROM wp WHERE id='" . $old["id"] . "'");
				sql::q("UPDATE clans SET dmoney=dmoney-" . abs($vesh["dprice"] - $old["dprice"]) . " WHERE `sign`='" . $pers['sign'] . "'");
