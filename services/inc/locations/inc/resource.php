...
if (($r == $tunnel["r1id"] and $k = $tunnel["r1k"]) or
	...
	$inst2 = sql::q1("SELECT id FROM wp WHERE uidp=" . $pers["uid"] . " and weared=1 and p_type=13");
	$r_get .= "<i class=user>Добыча</i><br>";
	$r = sql::q1("SELECT * FROM resources WHERE image='" . $r . "'");
	$r["k"] = $k;
	...
	$za = 0;
	if (rand(1, 500) == 1) {
		$v = sql::q1("SELECT * FROM weapons WHERE type='rune' and price/10<" . $pers["sp7"] . " and dprice=0 ORDER BY RAND()");
		if ($v) {
			insert_wp($v["id"], $pers["uid"], -1, 0, $pers["user"]);
			$r_get .= "Разрывая клоки земли, вы откопали руну!<hr>";
			$pers["waiter"] = $t + $timed;
			sql::q("UPDATE wp SET durability=durability-" . ($kk / 2 + 1) . " WHERE id='" . $inst["id"] . "'");
	...
	if ($za == 0) {
		if (rand(sqrt($pers["sp12"]), $pers["sp12"] * 2) > $r["price"]) {
			...
			$pers["waiter"] = $t + $timed;
			sql::q("UPDATE wp SET durability=durability-" . round($mdur, 2) . " WHERE id='" . $inst["id"] . "'");
			set_vars("sp12=sp12+" . round((10 / ($pers["sp12"] + 1)), 3) . ",sp7=sp7+" . round((8 / ($pers["sp7"] + 1)), 3) . ",peace_exp=peace_exp+3
		,waiter=" . $pers["waiter"] . ",tire=tire+1", $pers["uid"]);
			$rr = sql::q1("SELECT * FROM wp WHERE uidp='" . $pers["uid"] . "' and id_in_w='res.." . $r["image"] . "'");
			if ($rr["id"])
				sql::q("UPDATE wp SET price=price+" . $kk * $r["price"] . ",weight=weight+" . $kk . ",durability=durability+" . $kk . ",max_durability=max_durability+" . $kk . " WHERE id=" . $rr["id"] . "");
			else {
				sql::q("INSERT INTO `wp` ( `id` , `uidp` , `user` , `weared` ,`id_in_w`, `price` , `dprice` , `image` , `index` , `type` , `stype` , `name` , `describe` , `weight` , `where_buy` , `max_durability` , `durability` ,`p_type`) VALUES (0, '" . $pers["uid"] . "','" . $pers["user"] . "', '0','res.." . $r["image"] . "','" . $kk * $r["price"] . "', '0', 'resources/" . $r["image"] . "', '0', 'ruda', 'resources', '" . $r["name"] . "', '', '" . $kk . "', '0', '" . $kk . "', '" . $kk . "','7');");
			}
			if ($r["image"] == $tunnel["r1id"]) {
				sql::q("UPDATE mine SET r1k=r1k-" . $kk . " WHERE x=" . $tunnel["x"] . " and y=" . $tunnel["y"] . " and mine=" . $tunnel["mine"] . "");
				$tunnel["r1k"] -= $kk;
			}
			if ($r["image"] == $tunnel["r2id"]) {
				sql::q("UPDATE mine SET r2k=r2k-" . $kk . " WHERE x=" . $tunnel["x"] . " and y=" . $tunnel["y"] . " and mine=" . $tunnel["mine"] . "");
				$tunnel["r2k"] -= $kk;
			}
			if ($r["image"] == $tunnel["r3id"]) {
				sql::q("UPDATE mine SET r3k=r3k-" . $kk . " WHERE x=" . $tunnel["x"] . " and y=" . $tunnel["y"] . " and mine=" . $tunnel["mine"] . "");
				$tunnel["r3k"] -= $kk;
			}
		} else {
		...
		,waiter=" . $pers["waiter"] . ",tire=tire+8", $pers["uid"]);
			if ($r["image"] == $tunnel["r1id"]) {
				sql::q("UPDATE mine SET r1k=r1k-" . $kk . " WHERE x=" . $tunnel["x"] . " and 
		y=" . $tunnel["y"] . " and mine=" . $tunnel["mine"] . "");
				$tunnel["r1k"] -= $kk;
			}
			if ($r["image"] == $tunnel["r2id"]) {
				sql::q("UPDATE mine SET r2k=r2k-" . $kk . " WHERE x=" . $tunnel["x"] . " and 
		y=" . $tunnel["y"] . " and mine=" . $tunnel["mine"] . "");
				$tunnel["r2k"] -= $kk;
			}
			if ($r["image"] == $tunnel["r3id"]) {
				sql::q("UPDATE mine SET r3k=r3k-" . $kk . " WHERE x=" . $tunnel["x"] . " and 
		y=" . $tunnel["y"] . " and mine=" . $tunnel["mine"] . "");
				$tunnel["r3k"] -= $kk;
			}
		}
