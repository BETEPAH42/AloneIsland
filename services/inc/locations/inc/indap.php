...
if (empty($_POST) and empty($_GET["buy"])) {
	...
	$vesh = sql::q1("SELECT * FROM wp WHERE id='" . $_POST["id"] . "' and uidp='" . $pers["uid"] . "' and where_buy=1 and id_in_w='' and clan_sign='' and weared=0");
...
' . $dop . '
	...
	$lastid++;
	$vesh = sql::q1("SELECT * FROM wp WHERE id='" . $_POST["idd"] . "' and uidp='" . $pers["uid"] . "' and where_buy=1 and id_in_w='' and clan_sign=''");
	$type  = $vesh["stype"];
	$oldp = $vesh["dprice"];
	$oldid = $vesh["id"];
	$dprice = 30;
	...
	sql::q("INSERT INTO `wp` (`id`,`name`,`s1`,`s2`,`s3`,`s5`,`s6`,`mf1`,`mf2`,`mf3`,`mf4`,`mf5`,`kb`,`hp`,`ma`,`udmin`,`udmax`,`describe`,`dprice`,`image`,`type`,`where_buy`,`stype`,`weight`,`max_durability`,durability,`index`,`slots`,`radius`,`tlevel`) VALUES ('" . $lastid . "','" . $vesh["name"] . "','" . $p["s1"] . "','" . $p["s2"] . "','" . $p["s3"] . "','" . $p["s5"] . "','" . $p["s6"] . "','" . $p["mf1"] . "','" . $p["mf2"] . "','" . $p["mf3"] . "','" . $p["mf4"] . "','" . $p["mf5"] . "','" . $p["kb"] . "','" . $p["hp"] . "','" . $p["ma"] . "','" . $p["udmin"] . "','" . $p["udmax"] . "','Индивидуальный артефакт','" . $dprice . "','" . $vesh["image"] . "','" . $vesh['type'] . "',1,'" . $vesh["stype"] . "','1','1','1','','" . $p["slots"] . "','" . $p["radius"] . "',8);");
	...
	$old = sql::q1("SELECT * FROM wp WHERE id='" . $_GET["old"] . "'");
	if ($old["image"] == $vesh["image"]) {
		if ($old["dprice"] and $pers["dmoney"] >= abs($vesh["dprice"] - $old["dprice"]) and $vesh["uidp"] == 0) {
			sql::q("UPDATE wp SET uidp=" . $pers["uid"] . ",durability=1 WHERE id='" . $_GET["buy"] . "'");
			sql::q("DELETE FROM wp WHERE id='" . $old["id"] . "' and weared=0 and uidp=" . $pers["uid"] . "");
