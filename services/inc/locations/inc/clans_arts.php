<div class=return_win><input type="button" value="Назад" class="inv_but" onclick="location='main.php'" style="width:120">
	<input type="button" value="Улучшить" class="inv_but" onclick="location='main.php?c=clap'" style="width:120">
</div><?
...
' . $dop . '
	...
	<tr>
		...
		<input type="submit" value="Готово" class="inv_but"></td>
				...
				sql::q("INSERT INTO `wp` (`id`,`name`,`s1`,`s2`,`s3`,`s5`,`s6`,`mf1`,`mf2`,`mf3`,`mf4`,`mf5`,`kb`,`hp`,`ma`,`udmin`,`udmax`,`describe`,`dprice`,`image`,`type`,`where_buy`,`stype`,`weight`,`max_durability`,durability,`index`,`slots`,`radius`) VALUES 
...
");
		...
		<option value="8">8</option>
				...
				$vesh = sql::q1("SELECT * FROM wp WHERE id='" . $_GET["id"] . "'");
				$k = intval($_GET["k"]);
				$price = $vesh["dprice"] * $k;
				$clan = sql::q1("SELECT * FROM `clans` WHERE `sign`='" . $pers['sign'] . "'");
				if ($vesh["dprice"] and $clan["dmoney"] >= $price and $vesh["uidp"] == 0 and $k > 2 and $k < 9) {
					SQL::q("UPDATE clans SET dmoney=dmoney-" . $price . " WHERE sign='" . $pers["sign"] . "'");
					for ($i = 0; $i < $k; $i++) {
						$v = insert_wp_new($pers["uid"], "id='" . $_GET["id"] . "'");
						sql::q("UPDATE wp SET uidp=" . $pers["uid"] . ",durability=1,clan_sign='" . $pers["sign"] . "',clan_name='" . $clan["name"] . "',present='" . $pers["user"] . "' WHERE id='" . $v["id"] . "'");
					}
