...
$tmp = SQL::q1("SELECT COUNT(*) as count FROM p_auras WHERE uid=" . UID . " and special=5 and esttime>" . tme() . "");
$_TRVM = ($tmp['count']) ? 0 : 0; //1:0 запрет выхода из закладки персонаж или рюкзак при тяжёлой травме 
$tmp = SQL::q1("SELECT esttime FROM p_auras WHERE uid=" . UID . " and special=14 and esttime>" . tme());
$_MINE = mtrunc($tmp['esttime'] - tme());
$tmp = SQL::q1("SELECT esttime FROM p_auras WHERE uid=" . UID . " and special=15 and esttime>" . tme());
$_UMINE = mtrunc($tmp['esttime'] - tme());
$tmp = SQL::q1("SELECT esttime FROM p_auras WHERE uid=" . UID . " and special=17 and esttime>" . tme());
$_STUN = mtrunc($tmp['esttime'] - tme());
# праздники 
$GOOD_DAY = 0;
...
$world = SQL::q1("SELECT weather,weatherchange FROM world", 0);
