...
$cells_around = sql::q("SELECT x,y,winnable,buildable,belong,name FROM nature WHERE x>=" . ($x - 3) . " and x<=" . ($x + 3) . " and 
...
// debag($_GET["go_nature"]);
// if (@$_GET["go_nature"]) $p = "Переход на <i>" . $go_n . "</i>.";
// else $p = "";
$y = $pers["y"];
$x = $pers["x"];

/*
$buildings_around = sql::q("SELECT x,y,type FROM buildings WHERE x>=".($x-3)." and x<=".($x+3)." and 
y>=".($y-2)." and y<=".($y+2)."");
$bd_str = '';
foreach ($buildings_around as $cc)
{
