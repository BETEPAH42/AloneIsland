...
function molch($persto, $perswho, $duration, $reason)
{
	...
	" . $persto["uid"] . ", " . time() . ", '" . $perswho["user"] . "', '1', '" . $reason . "', '" . ($duration * 60) . "'
		...
		SQL::q("UPDATE p_auras SET esttime=0 WHERE uid=" . $persto["uid"] . " and special=1");
		set_vars("silence=0", $persto["uid"]);
		SQL::q("INSERT INTO `puns` ( `uid` , `date` , `who` , `type` ) 
...
function block($persto, $perswho, $duration, $reason)
{
	if ($duration <> 2) {
		...
		SQL::q("INSERT INTO `puns` ( `uid` , `date` , `who` , `type` , `reason` , `duration` ) VALUES (" . $persto["uid"] . ", " . time() . ", '" . $perswho["user"] . "', '2', '" . $reason . "', '0');");
	} else {
		...
		SQL::q("UPDATE puns SET duration=" . (time() - $persto["lastom"]) . " WHERE duration=0 and type=2 and uid=" . $persto["uid"] . "");
		SQL::q("INSERT INTO `puns` ( `uid` , `date` , `who` , `type` , `reason` , `duration` ) 
	VALUES (
...
function blocki($persto, $perswho, $reason)
{
	...
	sql::q("UPDATE `chars` SET about='Заблокировано. Причина: " . $reason . "' WHERE `uid`=" . $persto["uid"]);
	sql::q("INSERT INTO `puns` ( `uid` , `date` , `who` , `type` , `reason` , `duration` ) 
	VALUES (
...
function diler($persto, $perswho, $count)
{
	...
	if ($count > 0) {
		...
		SQL::q("INSERT INTO `dtransfer` ( `uid` , `uidwho` , `date` , `summ` ) 
VALUES (
