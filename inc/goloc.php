	...
	if ($pers["cfight"] == 0) {
		...
		elseif ($pers["tour"] != 0)
			...
			if ($pers["location"] == 'out') {
				$cell = sql::q1("SELECT * FROM nature WHERE x=" . $pers["x"] . " and y=" . $pers["y"] . "");
				if ($_GET["goloc"] <> $cell["go_id"]) $Deny = 1;
			} else {
				$Deny = 1;
				$loc1 = sql::q1("SELECT go_id FROM locations WHERE id='" . $_GET["goloc"] . "'");
				$loc2 = sql::q1("SELECT go_id FROM locations WHERE id='" . $pers["location"] . "'");
			...
			if (!$Deny) {
				...
				$t = time();
				SQL::q("UPDATE `users` SET `location`='" . $pers["location"] . "',`curstate`=2 WHERE `uid`=" . UID . "");
				$pers["curstate"] = 2;
			}
		}
	}
