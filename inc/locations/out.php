...
if ($pers["sp8"] > 1000) {
	define("TLOOK_TIME", 6);
	define("TREE_COUNT", 3);
} // поправить время осмотра местности
else {
	define("TLOOK_TIME", 10);
	define("TREE_COUNT", 2);
} // и здесь
define("TRUB_TIME", 30);
define("GAIN_COST", 50);
...
<div id="outer" style="display:none;background: grey; position:absolute;z-index:356;top:60px;top: 85px;width: 50%;left: 25%;border-radius:6px;min-height:40px;">
	...
	if ($_POST["teleport"] and $cell["teleport"]) {
		...
		$TP = sql::q1("SELECT COUNT(*) as count FROM nature WHERE x=" . intval($xtp) . " and y=" . intval($ytp))['count'];
...
<script>
	...
	var aY = <? echo $pers["y"]; ?>;
	view_nature();
	// setTimeout(document.getElementById(" outer").style.display = "none", 6000);
</script>