...
";
		...
		if (isset($_GET["buy"]) and isset($_GET["kolvo"]) and $_GET["kolvo"] > 0 and $_GET["kolvo"] < 100) {
			...
			if ($v["where_buy"] == 0 and $v["q_s"] > 0) {
				if ($pers["money"] < ($v["price"] * $kolvo))
					...
					sql::q("UPDATE `users` SET `money`=money-" . $v["price"] * $kolvo . " WHERE `uid`='" . $pers["uid"] . "' ;");
		...
		if ($_FILTER["lavkatype"] == 'potion' and @$_GET["put_lot2"]) {
			if (@$_GET["put_lot"]) {
				...
				$RES = sql::q1("SELECT * FROM `wp` WHERE uidp=" . UID . " and stype='potion' and weared=0 and in_bank=0 and id=" . $id);
				if ($price > $RES["price"] * 10)
					$price = $RES["price"] * 10;
				if ($RES)
					sql::q("UPDATE wp SET tempprice=" . $price . ",in_bank=2,user='" . $pers["user"] . "' WHERE id=" . $id);
			}
			$RES = sql::q("SELECT * FROM `wp` WHERE uidp=" . UID . " and stype='potion' and weared=0 and in_bank=0");
		...
		if ($_FILTER["lavkatype"] == 'potion' and @$_GET["buy_potion"]) {
			...
			$RES = sql::q1("SELECT * FROM `wp` WHERE in_bank=2 and id=" . $id);
			if ($RES) {
				if ($RES["tempprice"] > $pers["money"]) {
					...
					sql::q("UPDATE wp SET uidp=" . UID . ", user='" . $pers["user"] . "', in_bank=0, tempprice=0 WHERE id=" . $id);
	...
	<tr>
		...
		?>
		<table border=0 width=98% cellspacing=0 cellspadding=0 style="height:16px;">
			<tr>
				<td style="background-image: url('images/DS/graybg_left.png'); background-position:bottom left; height:16px; width:12px;"></td>
				<td style="background-image: url('images/DS/graybg.png');" align=center nowrap>
					<form method="POST" action='main.php'>
						<font class=Llvl>
							...
							</select> | <b class=about>Уровень от<input type="text" name="minlevel" size="7" value="<? if ($_FILTER["lavkaminlevel"] <> "") echo $_FILTER["lavkaminlevel"];
																													else echo "0"; ?>" class=but2>до<input type="text" name="maxlevel" size="7" value=<? if ($_FILTER["lavkamaxlevel"] <> "") echo $_FILTER["lavkamaxlevel"];
																																																		else echo $pers["level"]; ?> class=but2></b>
							| Не дороже <input type="text" name="maxcena" size="7" value="<? if ($_FILTER["lavkamaxcena"] <> "") echo $_FILTER["lavkamaxcena"];
																							else echo "100000"; ?>" class=but2><b>LN</b> | <input type="submit" value="Ок" class=but style='width:80px;height:16px;cursor:pointer;'></font>
				</td>
				<td style="background-image: url('images/DS/graybg_right.png'); background-position:bottom right; height:16px; width:12px;"></td>
			</tr>
		</table>
		<script>
			show_imgs_sell();
		</script>
		</form><? echo "<center class=lUser>У вас с собой <b>" . round($pers["money"], 2) . " LN</b></center>"; ?>
		<hr>
		</td>
		</tr>
		<tr>
			<td valign="top">
				<table border=2 width=98% cellspacing=2 cellpadding=2 bordercolorlight=#C0C0C0 bordercolordark=#FFFFFF>
					...
					$enures = sql::q("SELECT * FROM `weapons` WHERE `tlevel`>='" . $_FILTER["lavkaminlevel"] . "' and `tlevel`<='" . $_FILTER["lavkamaxlevel"] . "' and `price`<='" . $_FILTER["lavkamaxcena"] . "' and " . $stype . " and `where_buy`='0' ORDER BY `" . $_FILTER["lavkasort"] . "` ASC");
					...
					foreach ($enures as $v) {
						if ($v["material"] == '') {
							...
							elseif ($v["tlevel"] < 10)
								...
								$r = rand(3, 5);
							$resources = sql::q("SELECT * FROM resources ORDER BY RAND() LIMIT 0," . $r);
							...
							sql::q("UPDATE weapons SET material='" . $v["material"] . "',material_show='" . $v["material_show"] . "' WHERE id='" . $v["id"] . "'");
						}
						echo "<tr><td align=left class=weapons_box>";
					...
					if ($_FILTER["lavkatype"] == 'potion') {
						...
						$RES = sql::q("SELECT * FROM `wp` WHERE in_bank=2");
						echo "<table border=0 width=760 cellspacing=0 cellpadding=0 class=but>";
						foreach ($RES as $v) {
							echo "<tr><td align=left class=weapons_box>";
