<center><?
		...
		if (@$_GET["lb"]) {
			$a = sql::q1("SELECT id,price,dprice,name FROM blasts WHERE
...
";
		...
		switch ($_FILTER["h_zn_show"]) {
			...
			case '2':
				...
				break;
			case 'a':
				$stype = intval($_FILTER["h_zn_show"]);
				break;
			default:
				$stype = 'all';
				break;
		...
		if ($type == "blast") {
			...
			$bls = sql::q("SELECT * FROM blasts WHERE " . $q);
			...
			foreach ($bls as $bl) {
				$cnt++;
				$tmp = sql::q1("SELECT id FROM u_blasts WHERE uidp=" . $pers["uid"] . " and id_in_w=" . $bl["id"] . "")['id'];
				if ($tmp) continue;
				vblast($bl, $pers);
				echo "<tr>";
				echo "<td class=but colspan=3>";
				if (
					$bl["dprice"] <= $pers["dmoney"] and $bl["price"] <= $pers["money"]
		...
		if ($type == "aura") {
			$q = 'type=' . intval($stype) . ' and where_buy<2';
			$bls = sql::q("SELECT * FROM auras WHERE " . $q);
			echo "<font class=title>АУРЫ</font>";
			echo "<table border=0 width=100% cellspacing=0 cellspadding=0 style='border-left-style: solid; border-width: 1px; border-color:silver'>";
			$cnt = 0;
			foreach ($bla as $bl) {
				$tmp = sql::q1("SELECT id FROM u_auras WHERE uidp=" . $pers["uid"] . " and id_in_w=" . $bl["id"] . "")['id'];
				if ($tmp) continue;
				$cnt++;
				vaura($bl, $pers, 0);
				echo "<tr>";
