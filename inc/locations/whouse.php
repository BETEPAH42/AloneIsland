          ...
          <table border="0" width=100%>
            <tr>
              <td class=but2 width=25%>
                <a class=bg href='main.php?act=add_form&set_type=add_form'>Выставить лот на продажу</a>
              </td>
              <td class=but2 width=25%>
                <a class=bg href='main.php?act=torgi&set_type=torgi'>Просмотр торгов</a>
              </td>
              <td class=but2 width=25%>
                <a class=bg href='main.php?act=my_lots&set_type=lots'>Ваши лоты</a>
              </td>
              <td class=but2 width=25%>
                <a class=bg href='main.php?act=my_up_lots&set_type=uplots'>Ваши текущие ставки</a>
              </td>
            </tr>
          </table>
        </td>
...
?>
  ...
  $num = sql::q1('SELECT count(*) as count FROM `wp` WHERE `auction`<> 1 AND `stype`="' . $stype . '" AND in_bank=0 AND clan_sign=\'\' AND`uidp`=' . UID)['count'];
  if ($num < 1) {
    show_err('У вас нет предметов этого типа', 'e');
  } else {
    echo '<div align=center><table width=800>';
    $sql = sql::q('SELECT * FROM `wp` WHERE `uidp`= ' . UID . ' AND `stype`="' . $stype . '" AND weared=0 AND `auction` <> 1');
    foreach ($sql as $res) {
...
function add_lot()
{
  ...
  if (isset($_GET['lot_id'])) {
    ...
    $res = sql::q1('SELECT `uidp`,`stype` FROM `wp` WHERE `id`=' . $lot_id . " AND in_bank=0 AND clan_sign='' AND uidp=" . UID);
    if (UID == $res['uidp']) {
      ...
      if ($time > 0) {
        ...
        if ($price > 0) {
          global $pers;
          $usrm = sql::q1('SELECT `money` FROM `users` WHERE `uid`=' . UID);
          $money = $usrm['money'] - (100 + $price * 0.3);
          if ($money > 0) {
            sql::q('UPDATE `wp` SET `auction`= 1 WHERE `id`=' . $lot_id . ' AND `uidp`=' . UID);
            sql::q('INSERT INTO `auction` VALUES(' . $lot_id . ',' . $time . ',' . $price . ',' . UID . ',' . UID . ',"' . $res['stype'] . '")');
            sql::q('UPDATE `users` SET `money`=' . $money . ' WHERE `uid`=' . UID);
...
function set_lot()
{
  ...
  if (isset($_GET['lot_id'])) {
    $lot_id = intval($_GET['lot_id']);
    $res = sql::q1('SELECT `uidp` FROM `wp` WHERE `id`=' . $lot_id);
    if (UID == $res[0]) {
      ...
      $vesh = sql::q1('SELECT * FROM `wp` WHERE `id`=' . $lot_id . ' AND weared=0 AND `uidp`=' . UID);
      $lavka = 0;
      include("./inc/inc/weapon.php");
...
function my_lots()
{
  $num = sql::q1('SELECT count(*) as count FROM `auction` WHERE `owner`=' . UID)['count'];
  if ($num > 0) {
    $sql = sql::q('SELECT * FROM `wp` WHERE `uidp`=' . UID . ' AND `auction`=1');
    echo '<div align=center><table width=800>';
    foreach ($sql as $res) {
      echo '<tr><td  class=weapons_box>';
      $vesh = $res;
      $lavka = 0;
      include("./inc/inc/weapon.php");
      $zapr = sql::q1('SELECT `wanner`,`price`,`time` FROM `auction` WHERE `id`=' . $res['id']);
      $wanner = sql::q1('SELECT `user` FROM `users` WHERE uid=' . $zapr['wanner']);
      echo '<table><tr><td>';
      if ($zapr['wanner'] != UID) {
...
function my_up_lots()
{
  $num = sql::q1('SELECT count(*) as count FROM `auction` WHERE `wanner`=' . UID)['count'];
  if ($num > 0) {
    $sql = sql::q('SELECT `id` FROM `auction` WHERE `wanner`=' . UID . ' AND `wanner`<>`owner`');
    echo '<div align=center><table width=800>';
    foreach ($sql as $r) {
      $zapr = sql::q1('SELECT * FROM `wp` WHERE id=' . $r['id']);
      echo '<tr><td  class=weapons_box>';
      $vesh = $zapr;
      $lavka = 0;
      include("./inc/inc/weapon.php");
      $zapr = sql::q1('SELECT `owner`,`price`,`time` FROM `auction` WHERE `id`=' . $zapr['id']);
      $owner = sql::q1('SELECT `user` FROM `users` WHERE uid=' . $zapr['owner']);
      echo '<table><tr><td>';
...
{
  $cur_time = time();
  $num = sql::q1('SELECT count(*) as count FROM `auction` WHERE time < ' . $cur_time)['count'];
  if ($num > 0) {
    $sql = sql::q('SELECT * FROM `auction` WHERE time < ' . $cur_time);
    foreach ($sql as $zapr) {
      if ($zapr['wanner'] != $zapr['owner']) {
        $us = sql::q1('SELECT `user` FROM `users` WHERE `uid`=' . $zapr['wanner']);
        sql::q('UPDATE `wp` SET `auction`="", `uidp`=' . $zapr['wanner'] . ', `user`="' . $us['user'] . '" WHERE `id`=' . $zapr['id']);
        sql::q('DELETE FROM `auction` WHERE `id`=' . $zapr['id']);
        $m = sql::q1('SELECT `money` FROM `users` WHERE `uid`=' . $zapr['owner']);
        $m = $m['money'] + $zapr['price'];
        sql::q('UPDATE `users` SET `money`=' . $m . ' WHERE `uid`=' . $zapr['owner']);
      } else {
        sql::q('UPDATE `wp` SET `auction`="", `uidp`=' . $zapr['wanner'] . ' WHERE `id`=' . $zapr['id']);
        sql::q('DELETE FROM `auction` WHERE `id`=' . $zapr['id']);
      }
    }
  }
...
{
  $lot_id = intval($_GET['lot_id']);
  $sql = sql::q1('SELECT * FROM `auction` WHERE `id`=' . $lot_id);
  $wan_m = sql::q1('SELECT `money` FROM `users` WHERE `uid`=' . UID);
  $up_m = $wan_m['money'] - ($sql['price'] * 1.1);
  if ($up_m > 0) {
    if (UID != $sql['owner']) {
      sql::q('UPDATE `users` SET `money`=' . $up_m . ' WHERE `uid`=' . UID);
      if ($sql['owner'] != $sql['wanner']) {
        $wan_m = sql::q1('SELECT `money` FROM `users` WHERE `uid`=' . $sql['wanner']);
        $wan_m = $wan_m['money'] + $sql['price'];
        sql::q('UPDATE `users` SET `money`=' . $wan_m . ' WHERE `uid`=' . $sql['wanner']);
      }
      show_err('Ставка сделана', 'n');
      $price = $sql['price'] * 1.1;
      sql::q('UPDATE `auction` SET `price`=' . $price . ', `wanner`=' . UID . ' WHERE `id`=' . $sql['id']);
    }
  } else {
...
<? //>
  ...
  $num = sql::q1('SELECT count(*) as count FROM `auction`')['count'];
  if ($num > 0) {
    echo '<div align=center><table width=800>';
    $sql = sql::q('SELECT * FROM `wp` WHERE `auction` = 1');
    foreach ($sql as $res) {
      echo '<tr><td class=weapons_box>';
      $vesh = $res;
      $lavka = 0;
      include("./inc/inc/weapon.php");
      $zp = sql::q1('SELECT * FROM auction WHERE id=' . $res['id']);
      $own = sql::q1('SELECT `user` FROM `users` WHERE `uid`=' . $zp['owner']);
      $wan = sql::q1('SELECT `user` FROM `users` WHERE `uid`=' . $zp['wanner']);
