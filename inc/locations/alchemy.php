<center><?
		if (
			...
			SQL::q("UPDATE users SET alchemy_d=alchemy_d+30, money=money-100 WHERE uid=" . $pers["uid"]);
			echo "<font class=hp>Вы удачно купили дистиллятор.</font>";
		}
		if (
			@$_GET["buy_alchemy"] and $_GET["buy_alchemy"] == 2
			and $pers["s5"] > 29 and $pers["money"] > 299
		) {
			SQL::q("UPDATE users SET alchemy_d=alchemy_d+100, money=money-300 WHERE uid=" . $pers["uid"]);
			echo "<font class=hp>Вы удачно купили дистиллятор.</font>";
		}
		if (
			@$_GET["buy_alchemy"] and $_GET["buy_alchemy"] == 3
			and $pers["s5"] > 17 and $pers["money"] > 119
		) {
			SQL::q("UPDATE users SET alchemy_m=alchemy_m+100, money=money-120 WHERE uid=" . $pers["uid"]);
			echo "<font class=hp>Вы удачно купили ступку.</font>";
		}
		if (
			@$_GET["buy_alchemy"] and $_GET["buy_alchemy"] == 4
			and $pers["money"] >= 4 * abs(intval($_GET["count"])) and mtrunc($_GET["count"])
		) {
			SQL::q("UPDATE users SET alchemy_b=alchemy_b+" . abs(intval($_GET["count"])) . ", money=money-" . abs(intval($_GET["count"])) . "*4 WHERE uid=" . $pers["uid"]);
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
	<tr>
		<td width="54" class="lbutton">
			<img border="0" src="images/weapons/potions/d.gif" width="54" height="100">
		</td>
	...
	<tr>
		<td width="54" class="lbutton">
			<img border="0" src="images/weapons/potions/m.gif" width="70" height="70">
		</td>
...
<div class=but>
	<p>Помощь: Дистиллятор нужен для приготовления зелий из растительных
		...
		<b>Если у вас имеется рабочий дистиллятор ,ступка и пустые ёмкости ,то вы можете видеть активную кнопку в вашем инвентаре ("Алхимия").</b>
	</p>
</div>