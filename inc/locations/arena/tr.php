...
if (empty($_GET["id"])) {
	...
	foreach ($bts as $b) {
		$TXT .= "<tr>";
		$TXT .= "<td class=user>" . $b["user"] . "[<b class=lvl>" . $b["level"] . "</b>]<img src=../ onclick=\"javascript:window.open('binfo.php?" . $b["id"] . "','_blank');\" style=\"cursor:point\"></td>";
		if (($pers["lb_attack"] + $lb["b_frequency"]) < tme())
	...
	$b = sql::q1("SELECT id,level,user FROM bots WHERE id=" . intval($_GET["id"]) . "");
...
while($idf<11)
{
$idf = sql::qi ("INSERT INTO `fights` (`oruj`,`travm`,`timeout`,`ltime`,players,bplace) 
...
if ($yf<6)
{
 	$p = sql::q1("SELECT user,level,sign,rank_i,chp,hp,cma,ma,sm6,sm7,lastom,uid,aura FROM `users` WHERE `user`='".$tmp."'");
