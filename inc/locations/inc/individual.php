...
' . $dop . '
	...
	<tr>
		...
		<input type="submit" value="Готово" class="inv_but"></td>
			...
			$lastid = sql::q1("SELECT MAX(id) as max FROM wp");
			$lastid = 1 + $lastid['max'];
			$dprice = 30;
			$p = $_POST;
			$type = "orujie";
			...
			sql::q("INSERT INTO `wp` (`id`,`name`,`s1`,`s2`,`s3`,`s5`,`s6`,`mf1`,`mf2`,`mf3`,`mf4`,`mf5`,`kb`,`hp`,`ma`,`udmin`,`udmax`,`describe`,`dprice`,`image`,`type`,`where_buy`,`stype`,`weight`,`max_durability`,durability,`index`,`slots`,`tlevel`) VALUES 
