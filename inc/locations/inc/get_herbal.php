...
if (isset($_GET["get_herbal"]) and $pers["action"] == 1) {
	...
	if ($res["image"] and $w["id"] and md5($pers["city"] . $res["name"]) == $_GET["code"]) {
		...
		sql::q("UPDATE wp SET durability=durability-1 WHERE id=" . $w["id"] . "");
		set_vars("peace_exp=peace_exp+1", $pers["uid"]);
		$lastid = sql::q1("SELECT MAX(id) as max FROM wp")['max'];
		$lastid++;
		sql::q("INSERT INTO `wp` ( `id` , `uidp` , `user` , `weared` ,`id_in_w`, `price` , `dprice` , `image` , `index` , `type` , `stype` , `name` , `describe` , `weight` , `where_buy` , `max_durability` , `durability` ,`p_type`, `timeout`) VALUES ('" . $lastid . "', '" . $pers["uid"] . "', '" . $pers["user"] . "', '0','','1', '0', 'herbals/" . intval($_GET["get_herbal"]) . "', '', 'herbal', 'herbal', '" . $res["name"] . "', '', '1', '0', '1', '1','200'," . (time() + 1200000) . ");");
		sql::q("UPDATE herbals_cell SET time=" . time() . " WHERE x_y='" . $cell["x"] . "_" . $cell["y"] . "' and image=" . intval($_GET["get_herbal"]) . " and time<" . (time() - $herbal_grow) . " LIMIT 1;");
	} elseif (empty($w["id"]))
