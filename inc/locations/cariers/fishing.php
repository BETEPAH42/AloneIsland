...
if (@$_GET["fishing"]) {
	...
	*/
		...
		##########################################
		echo "<center class=but2><b>{$w} {$p}</b> (У Вас: " . round($pers["sp6"] + $skill_p, 2) . " навыка)<br>{$rer}</center>";
	}
			...
			while ($i < 1) //количестов выловленного за один заброс, будет зависеть от вида удочки
			{
				...
				if (@$v["id"]) {
					...
					else 
					{
						...
						{   
							$vesh = insert_wp_fish($fish["id"], $pers['uid']);
							//проверяем
							SQL::q("UPDATE wp SET durability=durability-1 WHERE uidp=" . $pers["uid"] . " and weared=0 and id=" . $v["id"] . "");
							$durability = 1;
							...
							echo "<b class=green>Вы поймали рыбу на {$v["name"]}!</b><br><i class=timef> " . $vesh["describe"] . "</i><br> Мирный опыт +5";
