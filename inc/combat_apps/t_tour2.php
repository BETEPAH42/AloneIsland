...
$t1 = sql::q1("SELECT * FROM quest WHERE id = " . TOUR2 . "");
...
if (@$_GET["show_t2"] and !$t1["finished"] and $t1["type"] == 0) // Турнир 1 10-15 10x10
{
	...
	if (!mtrunc(T_START + $t1["time"] - tme())) {
		...
		sql::q("UPDATE quest SET finished=0,time=" . tme() . "+" . T_START . ",type=1,lParam=" . $id . " WHERE id = " . TOUR2);
	} else {
		...
		$users = "SELECT * FROM users WHERE tour=2;";
...
<br><i>Призы:</i> 10 пергаментов, 100 LN, 10000 опыта";
	...
	if (!mtrunc(T_START2 + $t1["time"] - tme())) {
		...
		sql::q("UPDATE quest SET finished=0,time=" . tme() . ",type=3,lParam=" . $id . " WHERE id = " . TOUR2);
	} else {
		$start = "";
		if ($t1["type"] == 2)
	...
	sql::q("UPDATE quest SET finished=1,time=" . tme() . " WHERE id=" . TOUR2);
