...
if ($yapp["atime"] <= time() and $yapp["type"] == 1) {
	...
	sql::q("UPDATE users SET apps_id=0,fteam=0 WHERE apps_id=" . $yapp["id"] . "");
	$yapp["uid"] = 0;
	$yapp["type"] = 0;
}
if ($yapp["uid"] <> $pers["uid"] and $yapp["type"] == 1 and @$_GET["refusem"]) {
	$pers["apps_id"] = 0;
	sql::q("UPDATE app_for_fight SET pl2=0,atime=" . (time() + 300) . " WHERE id=" . $yapp["id"] . "");
...
if ($yapp["uid"] == $pers["uid"] and $yapp["type"] == 1 and @$_GET["refuse"]) {
	$pers["apps_id"] = 0;
	sql::q("UPDATE app_for_fight SET pl2=0,atime=" . (time() + 300) . " WHERE id=" . $yapp["id"] . "");
	sql::q("UPDATE users SET apps_id=0,fteam=0,refr=1 WHERE apps_id=" . $yapp["id"] . " and fteam=2");
}
if ($yapp["uid"] == $pers["uid"] and $yapp["type"] == 1 and @$_GET["get_back"]) {
	$pers["apps_id"] = 0;
	sql::q("DELETE FROM app_for_fight WHERE id=" . $yapp["id"] . "");
	sql::q("UPDATE users SET apps_id=0,fteam=0,refr=1 WHERE apps_id=" . $yapp["id"] . "");
	$yapp["uid"] = 0;
	$yapp["type"] = 0;
}
if ($yapp["uid"] == $pers["uid"] and $yapp["type"] == 1 and $yapp["pl2"] == 1 and @$_GET["begin"]) {
	sql::q("DELETE FROM app_for_fight WHERE id=" . $yapp["id"] . "");
	$persvs = sql::q1("SELECT user FROM users WHERE apps_id=" . $yapp["id"] . " and fteam=2");
	sql::q("UPDATE users SET apps_id=0,fteam=0,refr=1 WHERE apps_id=" . $yapp["id"] . "");
