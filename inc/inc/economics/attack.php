...
if (isset($_POST['napad']) and $pers['cfight'] == 0 and $_POST['fornickname'] <> $pers["user"]) {
	...
	if ($v["type"] == 'napad') {
		if ($v["index"] == 100) $travma = 100;
		else $travma = 30;
		$za = intval($_POST['za']);
		$perstowho = sql::q1("SELECT * FROM `users` WHERE `user`='" . $_POST['fornickname'] . "' and online=1");
		if ($perstowho["cfight"] > 10)
			$fight = sql::q1("SELECT * FROM `fights` WHERE `id`='" . $perstowho["cfight"] . "'");
		$k = 0;
		if ($fight["closed"])
		...
		if ((($pers["location"] == $perstowho["location"] and $perstowho["location"] != 'out') or
			...
			if ($perstowho["cfight"] > 10 && $fight["type"] != 'f') {
				...
				if ($fight["type"] <> 'f' and $fight["id"]) {
					...
					sql::q("UPDATE `fights` SET players=players+1 WHERE id=" . $fight["id"] . "");
					if (($za == 1 and $perstowho["fteam"] == 1) or ($za == 0 and $perstowho["fteam"] == 2)) {
						sql::q("UPDATE `users` SET `curstate`=4 , `cfight`='" . $fight["id"] . "', fteam=1,refr=1 WHERE `uid`='" . $pers["uid"] . "'");
						$fteam = 1;
					} else {
						sql::q("UPDATE `users` SET `curstate`=4 , `cfight`='" . $fight["id"] . "', fteam=2,refr=1 WHERE `uid`='" . $pers["uid"] . "'");
						$fteam = 2;
					}
					if ($fight["bplace"]) {
						...
						while ($xf > 0 and $xf < 15) {
							...
							$bcount = sql::q1("SELECT COUNT(*) as count FROM users WHERE cfight=" . $fight["id"] . " and chp>0 and xf=" . $xf . " and yf=" . $yf)['count'];
							$bcount += sql::q1("SELECT COUNT(*)as count FROM bots_battle WHERE cfight=" . $fight["id"] . " and chp>0 and xf=" . $xf . " and yf=" . $yf)['count'];

							if (!substr_count($bplace["xy"], "|" . $xf . "_" . $yf . "|") and $bcount == 0)
								break;
						}
						sql::q("UPDATE `users` SET `yf`=" . $yf . " , `xf`='" . $xf . "' WHERE `uid`='" . $pers["uid"] . "'");
					}
				...
				if ($perstowho["sign"] == 'c2' and date("H") % 2 == 0) {
