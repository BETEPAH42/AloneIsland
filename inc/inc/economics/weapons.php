...
if (isset($_GET["lavkasdat"]) and $pers["punishment"] < $time and strpos(" " . $pers["location"], "lavka")) {
	$v = sql::q1("SELECT * FROM wp WHERE weared=0 and id=" . intval($_GET["lavkasdat"]) . " and uidp=" . UID . " and where_buy<>1");
	...
	if (@$v["id"] and ($v["clan_name"] == "" or $pers["clan_state"] == 'g')) {
		...
		sql::q("UPDATE `weapons` SET `q_s`=q_s+1 WHERE `id`='" . $v["id_in_w"] . "'");
		sql::q("DELETE FROM wp WHERE id=" . $v["id"] . "");
		set_vars("`money`='" . $pers["money"] . "', sp9=sp9+1/(sp9+1),`weight_of_w`=weight_of_w-" . ($v["weight"]), $pers["uid"]);
		$pers["weight_of_w"] -= $v["weight"];
		$_RETURN .= "<center>Вещь удачно сдана в лавку.<br>(Комиссия <b>" . round($v["price"] - $koef * $koef_cur * $v["price"] * ($v["durability"] + 1) / ($v["max_durability"] + 1), 2) . "</b> LN)</center>";
	}
	unset($v);
}
###############
...
if (isset($_GET["bank"]) and $pers["punishment"] < $time and strpos(" " . $pers["location"], "bank") and $pers["money"] >= round(($v["price"] * 0.1), 2)) {
	$v = sql::q1("SELECT id_in_w,price,durability,max_durability,weight,id FROM wp WHERE weared=0 and id=" . intval($_GET["bank"]) . " and uidp=" . UID . " and where_buy<>1 and in_bank=0");

	if (@$v["id"]) {
		$pers["money"] -= round($v["price"] * 0.1, 2);
		sql::q("UPDATE wp SET in_bank=1 WHERE id=" . $v["id"] . "");
		set_vars("`money`='" . $pers["money"] . "',`weight_of_w`=weight_of_w-" . ($v["weight"]), $pers["uid"]);
		$pers["weight_of_w"] -= $v["weight"];
		$_RETURN .= "<center>Вещь удачно сдана в банк на хранение.<br>Комиссия <b>" . round($v["price"] * 0.1, 2) . "</b> LN)</center>";
	}
	unset($v);
}
...
if (isset($_GET["getbank"]) and $pers["punishment"] < $time and strpos(" " . $pers["location"], "bank")) {
	$v = sql::q1("SELECT id_in_w,price,durability,max_durability,weight,id FROM wp 
	WHERE weared=0 and id=" . intval($_GET["getbank"]) . " and uidp=" . UID . " and where_buy<>1 and in_bank=1");

	if (@$v["id"]) {
		sql::q("UPDATE wp SET in_bank=0 WHERE id=" . $v["id"] . "");
...
if (@$_GET["give"] == 'allfish' and $pers["punishment"] < $time and strpos(" " . $pers["location"], "lavka")) {
	...
	if ($koef > 0.99) $koef = 0.99;
	if ($_ECONOMIST) $koef = 0.99;
	$price = sql::q1("SELECT SUM(price) as sum FROM wp WHERE uidp=" . $pers["uid"] . " and type='fish' and weared=0")['sum'];
	if ($price) {
		$pers["money"] += $koef * $price;
		sql::q("DELETE FROM wp WHERE uidp=" . $pers["uid"] . " and type='fish' and weared=0");
...
//Сдаём  все шкуры
###############
if (@$_GET["give"] == 'allskin' and $pers["punishment"] < $time and strpos(" " . $pers["location"], "lavka")) {
	$koef = 1;
	if ($pers["level"] > 4) $koef = 0.8;
	if ($koef < 1) $koef += $pers["sp9"] / 1000;
	if ($koef > 0.99) $koef = 0.99;
	if ($_ECONOMIST) $koef = 0.99;
	$price = sql::q1("SELECT SUM(price) as sum FROM wp WHERE uidp=" . $pers["uid"] . " and type='skin' and weared=0")['sum'];
	if ($price) {
		$pers["money"] += $koef * $price;
		sql::q("DELETE FROM wp WHERE uidp=" . $pers["uid"] . " and type='skin' and weared=0");
		set_vars("sp9=sp9+1/(sp9+1),money=money+" . ($koef * $price), $pers["uid"]);
		$_RETURN .= "Вся рыба удачно сдана в лавку. (Выручка <b>" . round($koef * $price, 2) . "</b> LN)";
	}
}
...
//Сдаём  всю траву
###############
if (@$_GET["give"] == 'allherbals' and $pers["punishment"] < $time and strpos(" " . $pers["location"], "lavka")) {
	$koef = 1;
	if ($pers["level"] > 4) $koef = 0.8;
	if ($koef < 1) $koef += $pers["sp9"] / 1000;
	if ($koef > 0.99) $koef = 0.99;
	if ($_ECONOMIST) $koef = 0.99;
	$price = sql::q1("SELECT SUM(price) FROM wp WHERE uidp=" . $pers["uid"] . " and type='herbal' and weared=0")['sum'];
	if ($price) {
		$pers["money"] += $koef * $price;
		sql::q("DELETE FROM wp WHERE uidp=" . $pers["uid"] . " and type='herbal' and weared=0");
		set_vars("sp9=sp9+1/(sp9+1),money=money+" . ($koef * $price), $pers["uid"]);
		$_RETURN .= "<center>Вся трава удачно сдана в лавку.<br>(Выручка <b>" . round($koef * $price, 2) . "</b> LN)</center>";
	}
}
###############
###############
...
if (@$_GET["give"] == 'alltrees' and $pers["punishment"] < $time and strpos(" " . $pers["location"], "lavka")) {
	...
	if ($koef > 0.99) $koef = 0.99;
	if ($_ECONOMIST) $koef = 0.99;
	$price = sql::q1("SELECT SUM(price) as sum FROM wp WHERE uidp=" . $pers["uid"] . " and type='trees' and weared=0")['sum'];
	$pers["money"] += $koef * $price;
	sql::q("DELETE FROM wp WHERE uidp=" . $pers["uid"] . " and type='trees' and weared=0");
	set_vars("sp9=sp9+1/(sp9+1),money=" . $pers["money"], $pers["uid"]);
	$_RETURN .= "<center>Все деревья удачно сданы в лавку. <br>(Выручка составила <b>" . round($koef * $price, 2) . "</b> LN)</center>";
}
###############
###############
//Сдаём  всю руду
###############
if (@$_GET["give"] == 'allruda' and $pers["punishment"] < $time and strpos(" " . $pers["location"], "lavka")) {
	$koef = 1;
	if ($pers["level"] > 4) $koef = 0.8;
	if ($koef < 1) $koef += $pers["sp9"] / 1000;
	if ($koef > 0.99) $koef = 0.99;
	if ($_ECONOMIST) $koef = 0.99;
	$price = sql::q1("SELECT SUM(price) as sum FROM wp WHERE uidp=" . $pers["uid"] . " and type='ruda' and weared=0")['sum'];
	$pers["money"] += $koef * $price;
	sql::q("DELETE FROM wp WHERE uidp=" . $pers["uid"] . " and type='ruda' and weared=0");
	set_vars("sp9=sp9+1/(sp9+1),money=" . $pers["money"], $pers["uid"]);
	$_RETURN .= "Вся руда удачно сдана в лавку. (Выручка <b>" . round($koef * $price, 2) . "</b> LN)";
}
###############
...
######################
if (@$_GET["dhousesdat"]) {
	$v = sql::q1("SELECT durability, max_durability, weight, dprice, id FROM wp WHERE uidp=" . $pers["uid"] . " and id=" . intval($_GET["dhousesdat"]) . " and weared=0 and timeout=0");
	if ($v["dprice"] > 5) {
		sql::q("DELETE FROM wp WHERE id=" . $v["id"] . "");
...
if (@$_GET["dchousesdat"] and $status == 'g') {
	$v = sql::q1("SELECT durability,max_durability,weight,dprice,id FROM wp 
WHERE uidp=" . $pers["uid"] . " and id=" . intval($_GET["dchousesdat"]) . " and weared=0 and timeout=0 and clan_sign='" . $pers["sign"] . "'");
	if (@$v["dprice"]) {
		sql::q("DELETE FROM wp WHERE id=" . $v["id"] . "");
		sql::q("UPDATE `clans` SET `dmoney`=dmoney+" . floor(($v["dprice"] * DD_CLAN_KOEF) * (($v["durability"] + 1) / ($v["max_durability"] + 1))) . " WHERE `sign`='" . $pers["sign"] . "'");
		set_vars("weight_of_w=weight_of_w-" . ($v["weight"]), $pers["uid"]);
		$pers["weight_of_w"] -= $v["weight"];
...
if (@$_GET["to_clan"] and $pers["clan_tr"] and $pers["punishment"] < $time) {
	$v = sql::q1("SELECT id,name,price,dprice FROM wp WHERE id=" . intval($_GET["to_clan"]) . " and uidp=" . $pers["uid"] . " and weared=0 and where_buy=0 and clan_name=''");
	if (@$v["id"]) {
		$clan = sql::q1("SELECT * FROM `clans` WHERE `sign`='" . $pers['sign'] . "'");
		if ($clan["treasury"] < ($clan["maxtreasury"] + 30)) {
			sql::q("UPDATE wp SET clan_sign='" . $pers["sign"] . "' , clan_name='" . $clan["name"] . "', present='" . $pers["user"] . "' WHERE id=" . $v["id"] . " and uidp=" . $pers["uid"]);
			sql::q("UPDATE clans SET treasury=treasury+1 WHERE sign='" . $pers["sign"] . "'");
		}
...
if (@$_GET["sell"] == 'yes' and $pers["punishment"] < $time) {
	$t = time() - 1;
	$sale = sql::q1("SELECT * FROM `salings` WHERE `id`=" . intval($_GET["hash"]) . "");
	$persto = sql::q1("SELECT uid,money,user,lastip FROM `users` WHERE `uid`=" . intval($sale["uidp"]) . "");
	$v = sql::q1("SELECT price,name,weight,`id` FROM `wp` WHERE `id`='" . $sale["idw"] . "' and uidp=" . $sale["uidp"] . " and weared=0");
	if ($v["id"]) {
		if ($pers["money"] > $sale["price"]) {
			...
			sql::q("UPDATE wp SET uidp=" . $pers["uid"] . " WHERE id=" . $sale["idw"] . "");
...
if (isset($_POST["fornickname"]) and intval(@$_POST["forprice"]) > 0 and $_POST["fornickname"] <> $pers["user"] and $pers["punishment"] < $time) {
	...
	$v = sql::q1("SELECT name,id,where_buy FROM wp 
WHERE uidp=" . $pers["uid"] . " and weared=0 and id=" . intval($_POST["id"]) . " and where_buy=0");

	if (@$v["id"]) {
		$persto = sql::q1("SELECT uid,location,x,y FROM `users` WHERE `smuser` = LOWER('" . $_POST["fornickname"] . "')");
		if ($pers["location"] == $persto["location"]) {
			$idf = sql::qi("INSERT INTO `salings` (`id`,`idw`,`uidp`,`price`, `uidwho`) VALUES (0,'" . $v["id"] . "','" . $pers["uid"] . "','" . $_POST["forprice"] . "'," . $persto["uid"] . ") ");
			$m = "saling#" . $idf;
...
if (intval(@$_POST["forprice"]) == 0 and isset($_POST["fornickname"]) and empty($_POST["money"]) and $_POST["fornickname"] <> $pers['user'] and isset($_GET["ids"]) and $pers["punishment"] < $time) {
	...
	$persto = sql::q1("SELECT uid,money,user,lastip,location,x,y FROM `users` WHERE `smuser` = LOWER('" . $_POST["fornickname"] . "')");
	foreach ($ids as $id) {
		if (!$id) continue;
		$v = sql::q1("SELECT weight,price,name,id FROM `wp` WHERE uidp=" . $pers["uid"] . " and weared=0 and id=" . intval($id) . " and where_buy=0");

		if ($pers["location"] == $persto["location"] and $v) {
			sql::q("UPDATE wp SET uidp=" . $persto["uid"] . " WHERE id=" . $id . "");
			set_vars("weight_of_w=weight_of_w-" . ($v["weight"]), $pers["uid"]);
			$pers["weight_of_w"] -= $v["weight"];
...
if (@$_GET["giveallH"] and isset($_POST["fornickname"]) and $_POST["fornickname"] <> $pers['user'] and $pers["punishment"] < $time) {
	$persto = sql::q1("SELECT uid,money,user,lastip,location,x,y FROM `users` WHERE `smuser` = LOWER('" . $_POST["fornickname"] . "')");
	$herbals = sql::q("SELECT weight,price,name,id FROM `wp` WHERE uidp=" . $pers["uid"] . " and weared=0 and type='herbal'");
	if ($pers["location"] == $persto["location"]) {
		foreach ($herbals as $v) {
			sql::q("UPDATE wp SET uidp=" . $persto["uid"] . " WHERE id=" . $v["id"] . "");
			set_vars("weight_of_w=weight_of_w-" . ($v["weight"]), $pers["uid"]);
			$pers["weight_of_w"] -= $v["weight"];
			$m = $pers["user"] . "|" . $v["name"] . "|" . " " . "|" . $v["id"] . "|";
			say_to_chat('s', $m, 1, $_POST["fornickname"], '*', 0);
...
##########################
if (isset($_GET["drop"])) {
	$v = sql::q1("SELECT weight,clan_sign,dprice,price FROM wp WHERE id=" . intval($_GET["drop"]) . " and dprice=0 and (where_buy<>1 and where_buy<>1 or p_type=13) and uidp=" . $pers["uid"] . " and weared=0");
	if ($v["clan_sign"] == "" or ($v["clan_sign"] <> "" and $v["price"] < 1400 and $v["dprice"] < 1 and $status == 'g')) {
		if ($v["clan_sign"]) sql::q("UPDATE clans SET treasury=treasury-1 WHERE sign='" . $pers["sign"] . "'");
		set_vars("weight_of_w=weight_of_w-" . intval($v["weight"]), $pers["uid"]);
		$pers["weight_of_w"] -= $v["weight"];
		sql::q("DELETE FROM wp WHERE id=" . intval($_GET["drop"]));
