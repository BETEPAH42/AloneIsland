...
if ($r <= round($pers["sm5"] / 3 + 3 + $pers["od_b"])) {
	...
	foreach ($temp1 as $tmp) {
		if (strpos(" |" . $tmp, 'bot=') > 0) {
			$p = explode("=", $tmp);
			$tmp = sql::q1("SELECT hp FROM `bots` WHERE `id`='" . $p[1] . "'");
			$tmp["chp"] = $p[2];
			$xy = explode("_", $p[4]);
			$tmp["xf"] = $xy[0];
			$tmp["yf"] = $xy[1];
			...
			$tmp = sql::q1("SELECT user,chp,level,sign,hp,uid,xf,yf,cma,ma FROM `users` 
		WHERE `user`='" . $tmp . "'");
		if (floor($tmp["chp"]) > 0)
	...
	foreach ($temp2 as $tmp) {
		if (strpos(" " . $tmp, 'bot=') > 0) {
			...
			$tmp = sql::q1("SELECT user,chp,level,sign,hp,uid,xf,yf,cma,ma FROM `users` WHERE `user`='" . $tmp . "'");
		if (floor($tmp["chp"]) > 0)
			$go_no_p .= $tmp["xf"] . "_" . $tmp["yf"] . "|";
	}
	#################################################################################	 
	...
	if ($persvs["bot"]) $go = 1;
	if ($_GET["gotox"] and $go == 0) sql::q("INSERT INTO `turns_f` ( `idf` , `uid1` , `uid2` , `turn` ) VALUES (" . $pers["cfight"] . ", " . $pers["uid"] . ", " . $persvs["uid"] . ", 'gotox=" . $_GET["gotox"] . ";gotoy=" . $_GET["gotoy"] . ";');");
	if ($_POST["vs"] and $go == 0) {
		$str = '';
		foreach ($_POST as $key => $v) $str .= $key . "=" . $v . ";";
		sql::q("INSERT INTO `turns_f` ( `idf` , `uid1` , `uid2` , `turn` ) VALUES (" . $pers["cfight"] . ", " . $pers["uid"] . ", " . $persvs["uid"] . ", '" . $str . "');");
	}
	...
	if ($_GET["gotox"] and $go) {
		...
		if ($shagi <= $sha and $_GET["gotox"] > 0 and $_GET["gotox"] < 15 and $_GET["gotoy"] >= 0 and $_GET["gotoy"] < 5 and !substr_count($bplace["xy"], "|" . $_GET["gotox"] . "_" . $_GET["gotoy"] . "|")) {
			...
			$go_no_p = str_replace(
				"|" . $pers["xf"] . "_" . $pers["yf"] . "|",
				"|" . intval($_GET["gotox"]) . "_" . intval($_GET["gotoy"]) . "|",
				$go_no_p
			);
			$pers["xf"] = intval($_GET["gotox"]);
			$pers["yf"] = intval($_GET["gotoy"]);
			sql::q("UPDATE `users` SET `xf`='" . $pers["xf"] . "' ,`yf`=" . $pers["yf"] . ", `bg`=0 , `bj`=0 , `bn`=0 WHERE `uid`='" . $pers["uid"] . "' ;");
		}
	}
	...
	if ($req["gotox"]) {
		...
		if ($shagi <= $sha and $req["gotox"] > 0 and $req["gotox"] < 15 and $req["gotoy"] >= 0 and $req["gotoy"] < 5 and !substr_count($bplace["xy"], "|" . $req["gotox"] . "_" . $req["gotoy"] . "|")) {
			...
			$fall = $s . $fall;
			$go_no_p = str_replace(
				"|" . $persvs["xf"] . "_" . $persvs["yf"] . "|",
				"|" . intval($req["gotox"]) . "_" . intval($req["gotoy"]) . "|",
				$go_no_p
			);
			$persvs["xf"] = intval($req["gotox"]);
			$persvs["yf"] = intval($req["gotoy"]);
			sql::q("UPDATE `users` SET `xf`='" . $persvs["xf"] . "' ,`yf`=" . $persvs["yf"] . ", `bg`=0 , `bj`=0 , `bn`=0 WHERE `uid`='" . $can["uid1"] . "' ;");
		}
	}
	...
	if ($kl <> 0) {
		...
		sql::q("UPDATE `fights` SET `turn`='" . $fight["turn"] . "' , `all`='" . addslashes($fight["all"]) . "' , `ltime`='" . time() . "' WHERE `id`='" . $fight["id"] . "' ;");
