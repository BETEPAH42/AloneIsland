...
if ($_GET["d"]) {
	SQL::q("UPDATE wp SET p_type=201 WHERE weared=0 and uidp=" . $pers["uid"] . " and id=" . intval($_GET["d"]) . " and p_type=200");
	SQL::q("UPDATE users SET alchemy_m=alchemy_m-1 WHERE uid=" . $pers["uid"] . "");
	$pers["alchemy_m"]--;
}

if ($_GET["g"]) {
	SQL::q("UPDATE wp SET p_type=200 WHERE weared=0 and uidp=" . $pers["uid"] . " and id=" . intval($_GET["g"]) . " and p_type=201");
}

if ($_GET["alchemy_go"] == 1 and $cell_type == 6) {
	if ($pers["sp11"] > 1200) $pers["sp11"] = 1200;
	$rcount = SQL::q1("SELECT COUNT(image) as count FROM `wp` WHERE `uidp`=" . $pers["uid"] . " and weared=0 and p_type=201")['count'];
	$all = "SELECT image,name FROM `wp` WHERE `uidp`=" . $pers["uid"] . " and weared=0 and p_type=201 ORDER BY image ASC";
	$id1 = SQL::q1($all);
	$id2 = SQL::q1($all);
	$id3 = SQL::q1($all);
	$id4 = SQL::q1($all);
	...
	if ($id1['image']) {
		$material .= $id1['image'] . "|";
		$material_show .= $id1['name'] . ", ";
	}
	if ($id2['image']) {
		$material .= $id2['image'] . "|";
		$material_show .= $id2['name'] . ", ";
	}
	if ($id3['image']) {
		$material .= $id3['image'] . "|";
		$material_show .= $id3['name'] . ", ";
	}
	if ($id4['image']) {
		$material .= $id4['image'] . "|";
	...
	$id1 = str_replace("herbals/", "", $id1['image']);
	$id2 = str_replace("herbals/", "", $id2['image']);
	$id3 = str_replace("herbals/", "", $id3['image']);
	$id4 = str_replace("herbals/", "", $id4['image']);
	$rsumm = $id1 + $id2 + $id3 + $id4;
	foreach (SQL::q($all) as $tmp) {
	...
	if ($rcount > 1) {
		if ($z <> 100)
			$z = sql::q1("SELECT image,name,param FROM potions WHERE image=" . $z . "");
		else
			$z = sql::q1("SELECT image,name,param FROM potions WHERE image%30=" . ($rsumm % 30));
	}
	...
	if ($z["image"]) {
		...
		SQL::q("INSERT INTO `wp` ( `id` , `uidp` , `weared` , `price` ,  `image` , `index` , `type` , `stype` , `name` , `weight` , `where_buy` , `max_durability` , `durability` ,`p_type`, `describe`, `material` , `material_show`) VALUES ('" . $lastid . "', '" . $pers["uid"] . "', '0','" . $price . "', 'potions/" . $z["image"] . "', '" . $time . "|" . $z["param"] . "|" . $a . "', 'potion', 'potion', '" . $z["name"] . "', '1', '0', '" . (1 + $pers["sp11"] / 200) . "', '" . (1 + $pers["sp11"] / 200) . "','0','" . $sk . "<br>Время действия " . tp($time) . "','" . $material . "','" . $material_show . "');");
		...
		$vesh = SQL::q1("SELECT * FROM wp WHERE id=" . $lastid);
	...
	SQL::q("UPDATE users SET sp11=sp11+1/(sp11+1), alchemy_b=alchemy_b-1, alchemy_d=alchemy_d-1,alchemy_m=alchemy_m-1 WHERE uid=" . $pers["uid"] . "");
	SQL::q("DELETE FROM wp WHERE p_type=201 and weared=0 and uidp=" . $pers["uid"] . "");
}
?>
...
<?
$count_r = 0;
$res = "SELECT name,id,image FROM `wp` WHERE `uidp`=" . $pers["uid"] . " and weared=0 and p_type=201";
...
$res = sql::q("SELECT * FROM `wp` WHERE `uidp`=" . $pers["uid"] . " and weared=0 and p_type=200");
