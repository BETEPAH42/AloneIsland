...
<script>
	...
	if (@$_POST["molch"] and $m == 1 and $_POST["molch"] <> 'sn') {
		...
		SQL::q("UPDATE `users` SET aura='" . $pers["aura"] . "molch" . "|" . ($_POST["molch"] * 60 + time()) . "|molch.gif|Заклинание молчания|@" . "' WHERE `uid`=" . $pers["uid"] . "");
		SQL::q("UPDATE chars SET molch='" . $chars["molch"] . $lt . "|" . $timemolch . "|" . $_SESSION["user"] . "@' WHERE uid='" . $pers["uid"] . "'");
	}

	if (@$_GET["bug"]) {
		echo 'show_message("BUG OFF ::: % <b>' . $pers["user"] . '</b>");';
		SQL::q("UPDATE `users` SET cfight=0 , curstate=0 WHERE `uid`=" . $pers["uid"] . "");
	}

	if (@$_GET["logclear"]) {
		echo 'show_message("Log clear ::  <b>' . $pers["user"] . '</b>");';
		SQL::q("UPDATE `chars` SET transfers='',sales='',ips='' WHERE `uid`=" . $pers["uid"] . "");
	}
	...
	if (@$_POST["molch"] and $m == 1 and $_POST["molch"] == 'sn') {
		...
		SQL::q("UPDATE `users` SET aura='" . $pers["aura"] . "' WHERE `uid`=" . $pers["uid"] . "");
		SQL::q("UPDATE chars SET molch='" . $chars["molch"] . $lt . "|" . '0' . "|" . $_SESSION["user"] . "@' WHERE uid='" . $pers["uid"] . "'");
	}
	...
	if (@$_POST["prison"] and $p == 1 and $_POST["prisontime"] <> 'vip') {
		...
		SQL::q("UPDATE `users` SET location='prison',prison='" . $_POST["prisontime"] . "|" . htmlspecialchars($_POST["prison"]) . "' WHERE `uid`=" . $pers["uid"] . "");
		SQL::q("UPDATE chars SET blocks='" . $chars["blocks"] . $lt . "|" . $_POST["prisontime"] . "|(" . htmlspecialchars($_POST["prison"]) . ")|" . $_SESSION["user"] . "@' WHERE uid='" . $pers["uid"] . "'");
	}
	...
	if ($p == 1 and $_POST["prisontime"] == 'vip') {
		...
		SQL::q("UPDATE `users` SET prison='' WHERE `uid`=" . $pers["uid"] . "");
		SQL::q("UPDATE chars SET blocks='" . $chars["blocks"] . $lt . "|" . '0' . "||" . $_SESSION["user"] . "@' WHERE uid='" . $pers["uid"] . "'");
	}
	...
	if (@$_POST["block"] and $b == 1 and $_POST["blockt"] <> '2') {
		...
		SQL::q("UPDATE `users` SET block='" . htmlspecialchars($_POST["block"]) . "' WHERE `uid`=" . $pers["uid"] . "");
		SQL::q("UPDATE chars SET blocks='" . $chars["blocks"] . $lt . "|" . '1' . "|(" . htmlspecialchars($_POST["block"]) . ")|" . $_SESSION["user"] . "@' WHERE uid='" . $pers["uid"] . "'");
	}
	...
	if ($b == 1 and $_POST["blockt"] == '2') {
		...
		SQL::q("UPDATE `users` SET block='' WHERE `uid`=" . $pers["uid"] . "");
		SQL::q("UPDATE chars SET blocks='" . $chars["blocks"] . $lt . "|" . '2' . "||" . $_SESSION["user"] . "@' WHERE uid='" . $pers["uid"] . "'");
	}
	...
	if (@$_POST["blocki"] and $i == 1 and $_POST["blockit"] <> '2') {
		...
		SQL::q("UPDATE `chars` SET about='Заблокировано. Причина: " . htmlspecialchars($_POST["blocki"]) . "' WHERE `uid`=" . $pers["uid"] . "");
	}

	if (@$_POST["pometka"] and $w == 1) {
		SQL::q("UPDATE chars SET zametki='" . $chars["zametki"] . $lt . "|" . htmlspecialchars($_POST["pometka"]) . "|" . $_SESSION["user"] . "@' WHERE uid='" . $pers["uid"] . "'");
	...
	if (@$_POST["v_num"] and $d == 1) {
		...
		SQL::q("UPDATE configs SET dollars=dollars+" . $dk);
		SQL::q("UPDATE configs SET dialers=CONCAT('<u>" . $_SESSION["user"] . "</u> >> <i>" . $pers["user"] . "</i> <b>" . $dk . " y.e.</b><br>',dialers)");
		SQL::q("UPDATE users SET dmoney=dmoney+" . $dk . " WHERE uid=" . $pers["uid"] . "");
		...
		SQL::q("UPDATE users SET dmoney=dmoney+" . round($dk * 0.03) . " WHERE uid=" . $pers["referal_uid"]);
	}
	?>
