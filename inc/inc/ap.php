...
if (@$_GET["gopers"] == "obnyl" or @$_GET["fz"]) {
	include('inc/inc/obnyl.php');
	unset($_GET["gopers"]);
}
...
while (($pers["exp"] + $pers["peace_exp"]) >= $level["exp"] and $level["exp"] > 0) {
	...
	$level = sql::q1("SELECT * FROM `exp` WHERE `level`=" . ($level["level"] + 1));
	$i++;
}
...
if ($i > 0) {
	...
	$pers["money"] += $money;
	$pers["coins"] += $coins;
	sql::q("UPDATE `users` SET level=level+" . $levels . ", free_stats=free_stats+" . $free_stats . ",free_f_skills=free_f_skills + " . $free_f_skills . ", free_m_skills=free_m_skills+" . $free_m_skills . ",money=money+" . $money . ",coins=coins+" . $coins . " WHERE `uid`='" . $pers["uid"] . "'");
}
...
if (!$zeroyed and $i > 0) {
	...
	if ($pers["level"] == 3 and $pers["referal_nick"]) {
		sql::q("UPDATE users SET referal_rcounter=referal_rcounter+1 WHERE uid=" . $pers["referal_uid"]);
	}
	...
	if ($pers["level"] % 5 == 0 and $pers["level"] <> 0 and $pers["referal_nick"]) {
		SQL::q("UPDATE users SET money=money+50 WHERE uid=" . $pers["referal_uid"] . "");
	...
	if ($pers["level"] == 15 and $pers["referal_nick"]) {
		sql::q("UPDATE users SET money=money+200 WHERE uid=" . $pers["referal_uid"] . "");
...
//Получение Звания
if (@$_GET["eff"] == 1) {
	$zvan = sql::q1("SELECT * FROM `zvanya` WHERE `id`='" . ($pers["zvan"] + 1) . "' ");
	if ($zvan["cena"] <= ($pers["victories"] - $pers["losses"]) and $zvan["cena"] <> 0) {
		$pers["zvan"] = $zvan["id"];
		sql::q("UPDATE `users` SET `zvan`=" . $pers["zvan"] . " WHERE `uid`=" . $pers["uid"] . " ;");
...
if ($pers["curstate"] <> 4 and $pers["cfight"] < 5) {
	...
	sql::q("UPDATE `users` SET " . hp_ma_up(
		$pers["chp"],
		$pers["hp"],
		$pers["cma"],
		$pers["ma"],
