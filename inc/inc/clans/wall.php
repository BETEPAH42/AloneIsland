	...
	<div style="width:500px" class=but>
		...
		<script>
			...
			function pr_r(WHO, LVL, SIGN, DATE, text) {
				if (SIGN != 'none') SIGN = '<img src=images/signs/' + SIGN + '.gif>';
				else SIGN = '';
		...
		#Добавить отзыв:
		if (@$_POST["report"]) {
			sql::q("INSERT INTO `reports_for_clans` ( `csign` , `lvl` , `sign` , `date` , `who` , `text` ) VALUES ('" . $pers["sign"] . "', '" . $pers["level"] . "', '" . $pers["sign"] . "', '" . time() . "' , '" . $pers["user"] . "', '" . str_replace("'", "", $_POST["report"]) . "');");
		}
		# Отзывы
		echo "<script>";
		if (empty($_GET["all_reports"]))
			$rep = sql::q("SELECT * FROM reports_for_clans WHERE csign='" . $pers["sign"] . "' ORDER BY date DESC LIMIT 7;");
		else
			$rep = sql::q("SELECT * FROM reports_for_clans WHERE csign='" . $pers["sign"] . "' ORDER BY date DESC");
		...
		foreach ($rep as $r) {
			$k++;
			echo "pr_r('" . $r["who"] . "'," . $r["lvl"] . ",'" . $r["sign"] . "','" . date("d.m.Y H:i", $r["date"]) . "','" . str_replace('', '<br>', $r["text"]) . "');";
		}
