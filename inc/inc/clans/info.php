...
<div class=inv>
	<center>
		<table border=0 cellspacing=0 width=600 class=but>
			<tr>
				<td width=120><a href=main.php?action=addon&gopers=clan&<? echo tme(); ?> class=blocked>Обновить</a></td>
				<td width=120><a href=main.php?clan=w&action=addon&gopers=clan class=blocked>Казна</a></td>
				<td width=120><a href=main.php?clan=wall&action=addon&gopers=clan class=blocked>Стена</a></td>
				<td width=120><a href=main.php?clan=tips&action=addon&gopers=clan class=blocked>Подсказки</a></td>
				<td width=120><a href=main.php?clan=zvan&action=addon&gopers=clan class=blocked>Должности</a></td>
			</tr>
		</table>
		<?
		if ($pers["clan_state"] == 'g' or $pers["clan_state"] == 'z' or $pers["sign"] == 'c2') {
		?>
			<table border=0 cellspacing=0 width=600 class=but>
				<tr>
					<td width=120><a href=main.php?clan=glava&action=addon&gopers=clan class=blocked>Управление личным составом</a></td>
					<td width=120><a href=main.php?clan=table&action=addon&gopers=clan class=blocked>Таблица опыта</a></td>
				</tr>
			</table>
	</center>
<?
		}
		$clan = sql::q1("SELECT * FROM `clans` WHERE `sign`='" . $pers['sign'] . "'");
		...
		if (@$_GET["well"]) {
			...
			if ($clan["time_well"] <= time()) {
				...
				sql::q("UPDATE clans SET time_well=" . (time() + 600) . " WHERE `sign`='" . $pers['sign'] . "'");
			}
		}
		#######################################################
		if (@$_GET["up_um"]) {
			...
			if ($clan["freestats"] > 0) {
				if ($_GET["up_um"] == 1)
					sql::q("UPDATE clans SET maxtreasury=maxtreasury+15,freestats=freestats-1 WHERE `sign`='" . $pers['sign'] . "'");
				if ($_GET["up_um"] == 2)
					sql::q("UPDATE clans SET maxpl=maxpl+3,freestats=freestats-1 WHERE `sign`='" . $pers['sign'] . "'");
				if ($_GET["up_um"] == 3)
					sql::q("UPDATE clans SET well=well+10,freestats=freestats-1 WHERE `sign`='" . $pers['sign'] . "'");
			}
		}
		#######################################################
		//доступ к казне
		if (@$_GET["do_inv"]) {
			...
			if ($_GET["do_inv"] == 1) {
				report("*** Доступ удачно выдан персонажу " . $_GET["pers"] . "! ***");
				sql::q("UPDATE users SET clan_tr ='1' WHERE `user`='" . $_GET["pers"] . "'");
			}
			if ($_GET["do_inv"] == 2) {
				report("*** Доступ удачно закрыт персонажу " . $_GET["pers"] . "! ***");
				sql::q("UPDATE users SET clan_tr ='0' WHERE `user`='" . $_GET["pers"] . "'");
			}
		}
		...
		if (@$_GET["zero"]) {
			$clan = sql::q1("SELECT * FROM `clans` WHERE `sign`='" . $pers['sign'] . "'");
			if ($clan["money"] >= 500)
				sql::q("UPDATE clans SET maxtreasury=0,maxpl=0,well=0,freestats=level,money=money-500 WHERE `sign`='" . $pers['sign'] . "'");
		}
		#######################################################
		if (@$_GET["set_params"] and ($status == 'g' or $status == 'z')) {
			$bp = sql::q1("SELECT uid,clan_state FROM `users` WHERE `smuser`=LOWER('" . $_GET["set_params"] . "') and `sign`='" . $pers['sign'] . "'");
			if (($bp["clan_state"] == 'g' and $status == 'g') or $bp["clan_state"] != 'g') {
				if (@$bp['uid']) {
					if ($bp["clan_state"] == 'g' or !$_POST["clan_state"])
						sql::q("UPDATE `users` SET `state`='" . $_POST["state"] . "', clan_tr='" . $_POST["clan_tr"] . "' WHERE `uid`='" . $bp['uid'] . "'");
					else
						sql::q("UPDATE `users` SET `state`='" . $_POST["state"] . "', `clan_state`='" . $_POST["clan_state"] . "', clan_tr='" . $_POST["clan_tr"] . "' WHERE `uid`='" . $bp['uid'] . "'");
				}
			}
		}
		#######################################################
		if (@$_POST["money"]) {
			...
			sql::q("UPDATE clans SET money=money+" . $m . " WHERE sign='" . $pers["sign"] . "'");
			report("Деньги удачно записаны на счёт клана.");
		}
		#######################################################
		...
		if (@$_POST["site"]) {
			...
			$site = str_replace("/", "", $site);
			$site = str_replace("\\", "", $site);
			sql::q("UPDATE clans SET sait='" . $site . "' WHERE sign='" . $pers["sign"] . "'");
			$clan["sait"] = $site;
		}
		#######################################################
		...
		echo " <center><table border=0 cellspacing=0 width=500 class=but><tr><td width=250>" . $cmoney . "</td><td width=250>" . $cdmoney . "</td></tr><tr><td class=but id=money colspan=3 align=center></td></tr></table></center>";

		if (isset($clan)) $c = 1;
		if ($pers["user"] == $clan["glav"] and $status != 'g') {
			$pers["clan_status"] = "g";
			sql::q("UPDATE users SET clan_state='g' WHERE uid=" . $pers["uid"] . "");
			$status = 'g';
		}
		...
		if ($c == 1 and ($status == 'g' or $status == 'z' or $status == 'k') and isset($_POST["go_in"]) and $allpers < ($clan["maxpl"] + 10)) {
			$go_in = sql::q1("SELECT sign,uid,user FROM `users` WHERE `user`='" . $_POST["go_in"] . "'");
			if (@$go_in['uid'] and $go_in["sign"] == "none") {
				$pers["money"] -= 200;
				sql::q("UPDATE `users` SET `sign`='" . $clan["sign"] . "' , `state`='Солдат' , `sklon`='" . $clan["sklon"] . "',
		`rank`='' WHERE `uid`='" . $go_in['uid'] . "'");
				sql::q("UPDATE `users` SET `money`='" . $pers["money"] . "' WHERE `uid`='" . $pers['uid'] . "'");
				report("<font color=gren>Персонаж <b>" . $go_in['user'] . "</b> принят в клан! С вашего счёта списано 200 LN.</font>");
			} else
		...
		#######################################################
		//   Выгоняем из клана
		if (($status == 'g' or $status == 'z' or $status == 'k') and isset($_POST["go_out"])) {
			$go_out = sql::q1("SELECT sign,uid,user,sklon FROM `users` 
	WHERE `user`='" . $_POST["go_out"] . "'");
			if (@$go_out['uid'] and $go_out["sign"] == $clan["sign"]) {
				sql::q("UPDATE `users` SET `sign`='none' , `state`='' , `sklon`='none',	`rank`='' WHERE `uid`='" . $go_out['uid'] . "'");
				report("Персонаж <b>" . $go_out['user'] . "</b> выгнан в клана!");
			} else
				report("Нет такого персонажа или персонаж уже находится в клане.");
			unset($go_out);
		}
		#######################################################
		// меняем права соклановцев 
		if ($status == 'g' and isset($_POST["editclan"])) {
			$who = sql::q1("SELECT rank,uid FROM `users` WHERE `user`='" . $_POST["editclan"] . "'");
			if ($who["uid"]) {
				$who["rank"] = '';
				...
				sql::q("UPDATE `users` SET `state`='" . $_POST["state"] . "',`rank`='" . $who["rank"] . "' WHERE `uid`='" . $who['uid'] . "'");
			}
		}
		unset($who);
		######################################################
		// Передаём права Главы клана
		if ($status == 'g' and isset($_POST["do_glav"])) {
			$p = sql::q1("SELECT uid,sign,user FROM users WHERE smuser=LOWER('" . $_POST["do_glav"] . "')");
			if (@$p["uid"] and $p["sign"] == $pers["sign"] and $p["user"] <> $pers["user"])
				sql::q("UPDATE clans SET glav='" . $p["user"] . "' WHERE sign='" . $pers["sign"] . "'");
			sql::q("UPDATE users SET state='',clan_state='b' WHERE uid='" . $pers["uid"] . "'");
			sql::q("UPDATE users SET state='',clan_state='g' WHERE uid='" . $p["uid"] . "'");
			$status = 'b';
		}
		######################################################
...
<a href='http://" . $clan['sait'] . "' target=_blank class=bold>" . $clan['sait'] . "</a>" . $ch_site . "</td></tr>
				...
				$sostav = sql::q("SELECT user,rank,online,location,state,level,aura,uid,rank_i,clan_state,lastom,silence,clan_tr FROM `users` 
WHERE `sign`='" . $clan['sign'] . "' ORDER BY `clan_state` ASC");
				...
				foreach ($sostav as $perssost) {
					...
					$allpers++;
					echo "<tr><td>";
					echo "<img src='images/pr.gif' onclick=\"javascript:top.say_private('" . $perssost["user"] . "')\" style='cursor:pointer' height=16><font class=user  " . $onclick . ">";
					...
					if ($perssost["clan_tr"]) {
						echo "<td class=timef>Казна <i class=green>вкл</i></td><td>";
						if ($pers["clan_state"] == 'g' or $pers["clan_state"] == 'z')
							echo "<a href=main.php?action=addon&gopers=clan&do_inv=2&pers=" . $perssost["user"] . " class=blocked><i class=hp>Закрыть доступ</i></a>";
					} else {
						echo "<td class=timef>Казна <i class=hp>выкл</i></td><td>";
						if ($pers["clan_state"] == 'g' or $pers["clan_state"] == 'z')  echo "<a href=main.php?action=addon&gopers=clan&do_inv=1&pers=" . $perssost["user"] . " class=blocked><i class=green>Открыть доступ</i></a>";
					}
					echo "</td></tr>";
				}
				echo "</table></center>";
				...
				if ((
					...
					sql::q("UPDATE clans SET level=level+1,freestats=freestats+1 WHERE sign='" . $pers["sign"] . "'");
					$clan["level"]++;
...
<input type="submit" value="Принять [200 LN]" class=inv_but></p>
		...
		<td bgcolor="#F0F0F0" class="td"><form method="POST" action=main.php?gopers=clan&action=addon>
<p align="right">
<input name=go_out size=100 class=laar style="float: left"> 
<input type="submit" value="Выгнать из клана!" class=inv_but></p>
		</form></td>
	</tr>';
					if ($status == 'g')
						echo '	<tr>
...
<input type="submit" value="Сделать главой клана" class=inv_but></p>
		...
		if ($c == 1) {
			$who = sql::q1("SELECT sign,state,rank,user FROM `users` WHERE `smuser`=LOWER('" . $_GET["who"] . "')");
...
<form method="POST" action=main.php?gopers=clan&action=addon>';
				if ($pers["user"] == 'BETEPAH') {
					...
					echo "<tr><td>Молчания</td>";
					if ($m == 1) echo '<td><input type="checkbox" name=m value=1 CHECKED></td></tr>';
					else echo '<input type="checkbox" name=m value=1></td></tr>';
					echo "<tr><td>Тюрьма</td>";
					if ($p == 1) echo '<td><input type="checkbox" name=p value=1 CHECKED></td></tr>';
					else echo '<td><input type="checkbox" name=p value=1></td></tr>';
					echo "<tr><td>Блокирование</td>";
					if ($b == 1) echo '<td><input type="checkbox" name=b value=1 CHECKED></td></tr>';
					else echo '<td><input type="checkbox" name=b value=1></td></tr>';
					echo "<tr><td>Оставлять заметки</td>";
					if ($w == 1) echo '<td><input type="checkbox" name=w value=1 CHECKED></td></tr>';
					else echo '<td><input type="checkbox" name=w value=1></td></tr>';
					echo "<tr><td>Блокирование информации</td>";
					if ($i == 1) echo '<td><input type="checkbox" name=i value=1 CHECKED></td></tr>';
					else echo '<td><input type="checkbox" name=i value=1></td>';
					echo "<tr><td>Кара</td>";
					if ($u == 1) echo '<td><input type="checkbox" name=u value=1 CHECKED></td></tr>';
					else echo '<td><input type="checkbox" name=u value=1></td></tr>';
					echo "</table></center>";
				}
		...
		elseif (@$_GET["clan"] == "zvan") include("zvan.php");
		elseif (@$_GET["clan"] == "glava") include("glava.php");
		elseif (@$_GET["clan"] == "table") include("tableO.php");
		elseif ($clan["sign"] == 'c2') {
			echo '<table border="0" width="100%" style="border-style: solid; border-width: 1px; border-color: #777777" cellspacing="1">
	<tr>
	...
	<tr>
		...
		<form method="POST" action=main.php?gopers=clan&action=addon>
			...
			if (@$_POST["mass"]) {
				say_to_chat("<font color=red>Верховный совет</font>", $_POST["mass"], 0, '', '*', 0);
			}
		}
		if ($status == 'g')
