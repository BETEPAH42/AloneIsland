...
if (@$_GET["use"] and $pers["cfight"] > 10 and $pers["chp"]) {
	...
	if ($v["durability"] > 0) {
		...
		SQL::q("UPDATE wp SET durability=durability-1 WHERE id=" . intval($_GET["use"]) . " and uidp='" . $pers["uid"] . "'");
	}
}
...
if (!empty($_GET["wear"]) and $_GET["wear"] <> 'none' and !$pers["cfight"] and !$pers["apss_id"])
	dress_weapon(intval($_GET["wear"]), true);
...
if ($t % 10 == 0) {
	$all_weight = intval(SQL::q1("SELECT SUM(weight) as ww FROM `wp` WHERE uidp=" . $pers["uid"] . "  and in_bank=0")['ww']);
	if ($all_weight <> $pers["weight_of_w"])
		SQL::q("UPDATE `users` SET `weight_of_w`=" . ($all_weight) . " WHERE `uid`='" . $pers["uid"] . "';");
}
...
if (@$_GET["rune_join"]) {
	if ($weared_slots) {
		...
		if ($pers["sp5"] > $rune["tsp5"]) {
			...
			SQL::q("UPDATE wp SET `" . $sk[2] . "`=`" . $sk[2] . "`+" . $sk[1] . ",`slots`=`slots`-1, `weight`=`weight`+1, `describe`='Установлена " . $rune["name"] . "', `price`=`price`+" . round(sqrt($rune["price"]), 2) . ", `name`='" . $weared_name . "[Р]' WHERE id=" . $weared_id . "");
			if ($sk[2] == "udmax")
				SQL::q("UPDATE wp SET `udmin`=`udmin`+1 WHERE id=" . $weared_id . "");
			SQL::q("DELETE FROM wp WHERE id=" . intval($_GET["rune_join"]) . ";");
