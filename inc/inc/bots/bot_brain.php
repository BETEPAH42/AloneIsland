...
if ($r > 1) // Если мы стоим далеко - стоит подойти
{
	for ($i = 0; $i < 3 and $goed == 0; $i++) {
		...
		foreach ($chk as $c)
			if ($c[0])
				$bplace["xy"] .= $c[1] . '_' . $c[2] . '|';
		$chk = sql::q("SELECT id,xf,yf FROM bots_battle WHERE cfight=" . $pers["cfight"] . " and (xf>=" . ($x - 1) . " and xf<=" . ($x + 1) . ") and (yf>=" . ($y - 1) . " and yf<=" . ($y + 1) . ") and chp>0");
		foreach ($chk as $c)
			if ($c[0])
				$bplace["xy"] .= $c[1] . '_' . $c[2] . '|';
		$lambda = 999;
		...
		/*if ($x==0) $x=1;
$check_for_go = sql::q1("SELECT uid FROM users WHERE cfight=".$pers["cfight"]." and xf=".$x." and yf=".$y." and chp>0");
if (!$check_for_go[0] and !substr_count($bplace["xy"],"|".$x."_".$y."|"))
		{
		$check_for_go = sql::q1("SELECT id FROM bots_battle WHERE cfight=".$pers["cfight"]." and xf=".$x." and yf=".$y." and chp>0");
		if (!$check_for_go[0])
				{
...
*/
		sql::q("UPDATE bots_battle SET xf=" . $x . " , yf=" . $y . " WHERE id=" . $pers["id"]);
		$goed = 1;
		$pers["xf"] = $x;
		$pers["yf"] = $y;
	}
