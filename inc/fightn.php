...
foreach ($cans as $c)
	$uids .= '<' . $c["uid2"] . '>';
if ($_GET["fstate"] == 1 and $pers["fstate"] <> 1 and $pers["fstate"] = 1)
...
?><table border=0 width=100% cellspacing=0 cellpadding=0>
	<tr>
		<td width=250 valign=top align=center><?= show_pers_in_f($pers, 3); ?></td>
		<td id=fight valign=top bgcolor=#CDCBCC></td>
		<td width=250 valign=top id="pers_vs" align=center>&nbsp;</td>
	</tr>
</table>
<script>
	<?
	...
	foreach ($temp1 as $tmp) {
		if ($tmp) {
			if (strpos(" |" . $tmp, 'bot=') > 0) {
				$p = explode("=", $tmp);
				$tmp = SQL::q1("SELECT hp,ma FROM `bots` WHERE `id`='" . $p[1] . "'");
				$tmp["chp"] = $p[2];
				$xy = explode("_", $p[4]);
				$tmp["xf"] = $xy[0];
				$tmp["yf"] = $xy[1];
				...
				$tmp = sql::q1("SELECT user,chp,level,sign,hp,uid,xf,yf,cma,ma,aura,uid FROM `users` WHERE `user`='" . $tmp . "'");
			if ($team == 2) {
	...
	foreach ($turns as $t)
		...
		elseif ($p = sql::q1("SELECT uid,user,aura FROM users WHERE user='" . $t . "'"))
	...
	if (($fight["ltime"] < time() - $fight["timeout"]) and !empty($_GET["boytime"])) {
		if ($_GET["boytime"] == 'zavboy') {
			...
			$uid = $persvs['uid'];
			$travm = $fight['travm'];
			$tmp = sql::q1("SELECT chp, uid, user, pol FROM `users` WHERE `user`='" . $tmp . "'");
			if ($tmp["chp"] > 0) {
				if ($tmp["pol"] == "female") $m = 'а';
				else $m = '';
				...
				sql::q("UPDATE `users` SET `chp`=0,`losses`=losses+1 WHERE `uid`='" . $tmp['uid'] . "'");
			}
			sql::q("UPDATE `fights` SET `all`='" . addslashes($fight["all"]) . "',`turn`='" . $fight["turn"] . "' WHERE `id`='" . $fight["id"] . "'");
			$persvs['uid'] = $uid;
			$fight['travm'] = $travm;
			$turns = explode("|", $fight["turn"]);
		}
	...
	if ($life1 == 0 or $life2 == 0 or $fight["turn"] == "finish" or $fight["turn"] == "finished" or $fight["turn"] == "timeout" or $fight["turn"] == "nicya" or count($turns) < 2) include('inc/inc/finish.php');
		...
		foreach ($idall as $id)
			if ($id <> "") {
				$zak = sql::q1("SELECT image,name,`id`,`type`,`describe`,`mana`,`time`,`t_in_c` FROM `zakl` WHERE `id`='" . $id . "' and ts6<=" . $pers["s6"] . ";");
				if ($zak["t_in_c"] > 0) $l = $zak["t_in_c"] . " ход.";
				else $l = tp($zak["time"]);
				if ($zak["type"] <> 'blast' and $zak["type"] <> 'blast2' and $zak["type"] <> 'push') $l = " || Время действия: " . $l;
				else $l = '';
		...
		if ($pers["posfight"] < 20 and $pers["posfight"] > -10) $otoity = 0;
		else $otoity = 1;
		$otoity = 1;
...
<script>
	...
	mod_st_fin();
	if (!$persvs["bot"]) $pvs = '|||||||||' . $persvs["uid"];
	else $pvs = '|||' . $persvs["chp"] . '|||||||' . $persvs["id"];
	echo "info_p('" . $pvs . "');";
	?>
</script>