...
$frs = sql::q("SELECT user,online,location,state,level,uid,rank_i,sign,ds FROM `users` WHERE referal_uid=" . UID . " ORDER BY `" . $sort . "` DESC LIMIT " . ($c * 30 + 1) . ",30");
$count = 1;
foreach ($frs as $perssost) {
	...
	echo "<img src='../' onclick=\"javascript:window.open('info.php?p=" . $perssost["user"] . "','_blank')\" style=cursor:pointer>";
	echo "</td>";
