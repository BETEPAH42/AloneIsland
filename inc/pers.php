...
<div id=inf_from_php style='display:none;'>
	...
	if (@$_GET["gopers"] == 'battle' and $pers["free_stats"] < 5 and $pers["level"] < 3 and $pers["chp"] / $pers["hp"] > 0.75) {
		...
		$bts = sql::q1("SELECT uid FROM users WHERE level=" . ($pers["level"]) . " and block<>'' and rank_i>5 and silence = 0 and lb_attack<" . tme() . " and s6=1 and s5=1 LIMIT 0,1")['uid'];
	...
	if ($t < $pers["waiter"]) {
		echo "<center class=items><b>" . $pers["user"] . "</b></center><hr><div id=waiter class=items align=center></div>";
	}
	...
	if (@$_POST["newchatcolor"] or @$_POST["selectob"] <> "") {
		if (@$_POST["selectob"] <> 0 and $pers["obr"] == 0 and $_POST["selectob"] < 12) {
			SQL::q("UPDATE `users` SET `obr`='" . $_POST["selectob"] . "' WHERE `uid` = " . UID);
		...
		if (@$_POST["newchatcolor"]) {
			...
			SQL::q("UPDATE `users` SET `options`='" . $pers["options"] . "' WHERE `uid` = " . UID);
	...
	if (@$_POST["pass"] and md5($_POST["pass"]) == $pers["pass"] and $_POST["newpass"] == $_POST["newpass2"] and $pers["noaction"] < time()) {
		set_vars("`pass` = '" . (md5($_POST["newpass"])) . "',noaction=UNIX_TIMESTAMP()+86400", $_COOKIE['uid']);
		$pers["noaction"] = time() + 86400;
		echo "<font class=red> Ваш Пароль изменён! <br></font>";
		SQL::q("INSERT INTO `puns` ( `uid` , `date` , `who` , `type` , `reason` , `duration` ) VALUES (" . $pers["uid"] . ", " . time() . ", '" . show_ip() . "', '7', 'главный пароль', '0');");
		$_GET["gopers"] = "parol";
	...
	if (@$_POST["snewpass"] and @$_POST["snewpass"] == $_POST["snewpass2"] and $pers["noaction"] < time()) {
		set_vars("`second_pass` = '" . (md5($_POST["snewpass"])) . "',noaction=UNIX_TIMESTAMP()+86400", $_COOKIE['uid']);
		$pers["noaction"] = time() + 86400;
		echo "<font class=red> Ваш ВТОРОЙ Пароль изменён! <br></font>";
		$_GET["gopers"] = "parol";
		SQL::q("INSERT INTO `puns` ( `uid` , `date` , `who` , `type` , `reason` , `duration` ) VALUES (" . $pers["uid"] . ", " . time() . ", '" . show_ip() . "', '7', 'второй пароль', '0');");
	...
	if (@$_POST["set_flash"] == 2 or @$_POST["set_flash"] == 1 and $pers["noaction"] < time()) {
		SQL::q("INSERT INTO `puns` ( `uid` , `date` , `who` , `type` , `reason` , `duration` ) VALUES (" . $pers["uid"] . ", " . time() . ", '" . show_ip() . "', '7', 'цифровой пароль', '0');");
		set_vars("flash_pass=" . $pers["flash_pass"] . ",noaction=UNIX_TIMESTAMP()+86400", $_COOKIE['uid']);
		$pers["noaction"] = time() + 86400;
	}

	if (!empty($_POST["name"]) or !empty($_POST["city"]) or !empty($_POST["country"]) or !empty($_POST["ovas"])) {
		$chars = SQL::q1("SELECT about FROM chars WHERE uid= " . $pers["uid"] . "");
		...
		$chars["about"] = str_replace("\\", "", $chars["about"]);
		SQL::q("UPDATE `users` SET `name` = '" . $pers["name"] . "' ,`city` = '" . $pers["city"] . "' ,`country` = '" . $pers["country"] . "'  WHERE `uid` = " . $pers["uid"] . " ;");
		SQL::q("UPDATE `chars` SET `about` = '" . $chars["about"] . "'  WHERE `uid` = " . $pers["uid"] . " ;");
		unset($chars);
	}
...
http://AloneIsland/into.php?id=" . $pers["uid"] . " </font> <br>С уважением, Администрация Alone Islands.</font><br>
</p>
";
	...
	if (empty($_GET["gopers"])) {
		...
		if ($pers["free_stats"] < 5 and $pers["level"] < 3  and $pers["chp"] / $pers["hp"] > 0.75) {
			$bts = SQL::q1("SELECT uid FROM users WHERE level=" . ($pers["level"]) . " and block<>'' and rank_i>5 and silence = 0 and lb_attack<" . tme() . " and s6=1 and s5=1 LIMIT 0,1");
			if ($bts['uid'])
		...
		if ($pers["kindness"] > 0)
			...
			$kindness_class = "hp";
		echo "<center class=dark><b>Мораль:</b> <b class=" . $kindness_class . ">" . kind_stat($pers["kindness"]) . "</b></center><br>";
		echo "<center class=dark><b>Профессия:</b> <b class=green>" . prof_pers($pers["prof_osn"]) . " [" . $pers["prof_osnLVL"] . "]</b></center>";
...
";
		...
		if (substr_count($pers["aura"], "doctor") > 0)
			include('inc/inc/characters/doctor.php');
		if (@$_GET["deny"]) {
			$pupil = SQL::q1("SELECT * FROM users WHERE uid = " . $pers["instructor"]);
			SQL::q("UPDATE users SET instructor=0 WHERE uid = " . $pers["uid"]);
	...
	if (@$_GET["gopers"] == "quests") {
		include("hero/quests.php");
	}
...
";
		...
		echo "<div class=return_win>Вы сможете изменить любой пароль через " . tp($pers["noaction"] - time()) . "<br><font class=bnick color=#990000>ТЕКУЩИЙ ЦИФРОВОЙ ПАРОЛЬ: <b>" . $pers["flash_pass"] . "</b></font></div>";
		...
		for ($aa = 0; $aa < 8; $aa += 2)
			for ($bb = 0; $bb < 8; $bb += 2)
				for ($cc = 0; $cc < 8; $cc += 2) if ($opt[5] == dechex($aa) . dechex($aa) . dechex($bb) . dechex($bb) . dechex($cc) . dechex($cc)) echo "<OPTION style='BACKGROUND: #" . dechex($aa) . dechex($aa) . dechex($bb) . dechex($bb) . dechex($cc) . dechex($cc) . "' value=" . dechex($aa) . dechex($aa) . dechex($bb) . dechex($bb) . dechex($cc) . dechex($cc) . " SELECTED></OPTION>";
...
<option  ' . $v6 . ' value="0">Нет</option>
		...
		/*
Настройка тактического боя
*/
		if ($pers["boi"] == "tact")
			$v2 = 'selected';
		else $v1 = 'selected';
		echo '<br>Отображение боя:<BR><form action="main.php?gopers=options" method=post>
	<table border="0" width="100%" cellspacing="0" cellpadding="0" style="font-size: 11px">
	<tr><td>Тактический бой</td>
	<td align="right">
	<select size="1" name="boi" style="width: 120" class="laar">
	<option ' . $v1 . ' value="clask">Классический</option>
	<option ' . $v2 . ' value="tact">Тактический</option>
	</select></td></tr></table>
	<input type=submit class=laar value=Сохранить  style="width:100%;cursor:pointer">
	</form>';
		if ($_POST["boi"]) {
			echo "<center>Если вы видите эту надпись, то Вы поменяли способ ведения боя, но пока отображается иной способ, ничего не делате все изменения внесены в базу (со временем ошибочку устраним)</center>";
			set_vars("boi='" . $_POST["boi"] . "'", UID);
		}
	} elseif (@$_GET["gopers"] == "service") include("hero/service.php");
	if (@$_GET["gopers"] == "quest") include("hero/quests.php");
	// if (@$_GET["gopers"]=='sms')
	// {
...
<?
$as = sql::q("SELECT * FROM p_auras WHERE uid=" . $pers["uid"] . "");
$txt = '';
foreach ($as as $a) {
