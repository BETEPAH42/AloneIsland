...
if (@$_POST["friend_nick"]) {
	...
	if ($p) {
		sql::q("INSERT INTO `friendship` ( `uid1` , `uid2` , `type` ) VALUES ('" . UID . "', '" . $p["uid"] . "', '1');");
...
if (@$_GET["get_friendship"]) {
	sql::q("UPDATE friendship SET type=0 WHERE uid1=" . intval($_GET["get_friendship"]));
}

if (@$_GET["cans_friendship"]) {
	sql::q("DELETE FROM friendship WHERE uid1=" . intval($_GET["cans_friendship"]) . " and uid2=" . intval($pers["uid"]));
	sql::q("DELETE FROM friendship WHERE uid2=" . intval($_GET["cans_friendship"]) . " and uid1=" . intval($pers["uid"]));
}
?>
	...
	<tr>
		<td align=center>
			<table border="0" width="80%" cellspacing="0" cellpadding="0" class=LinedTable>
				...
				$q = '';
				$q2 = '';
				$fl = sql::q("SELECT * FROM friendship WHERE uid1=" . UID . " or uid2=" . UID);
				foreach ($fl as $f) {
					if ($f["type"] == 0) {
						if ($f["uid1"] == UID)
							$q .= 'uid=' . $f["uid2"] . " or ";
				...
				#1
				if ($q) {
					$frs = sql::q("SELECT user,online,location,state,level,aura,uid,rank_i,sign FROM `users` WHERE " . $q . " ORDER BY `rank_i` DESC");
					foreach ($frs as $perssost) {
	...
	<tr>
		<td align=center>
			<table border="0" width="80%" cellspacing="0" cellpadding="0" class=LinedTable>
				<?
				if ($q2) {
					$frs = sql::q("SELECT user,online,location,state,level,aura,uid,rank_i,sign FROM `users` WHERE " . $q2 . " ORDER BY `rank_i` DESC");
					foreach ($frs as $perssost) {
						echo "<tr><td class=user>";
						echo "<img src='images/pr.gif' onclick=\"javascript:top.say_private('" . $perssost["user"] . "')\" style=cursor:pointer> &nbsp;&nbsp;";
						echo "<img src='images/signs/" . $perssost["sign"] . ".gif' title='" . $perssost["state"] . "'>";
