<div class=inv>
	...
	if ($priv["emagic"] > 1) {
		if (isset($_GET["delete"])) {
			if (SQL::q("DELETE FROM blasts WHERE id='" . $_GET["delete"] . "'")) echo "<center class=return_win>Удалено!</center>";
		}
		if (isset($_GET["adelete"])) {
			if (SQL::q("DELETE FROM auras WHERE id='" . $_GET["adelete"] . "'")) echo "<center class=return_win>Удалено!</center>";
		}
		if (@$_GET["new_wp"]) {
			$mid = sql::q1("SELECT MAX(id) as max FROM blasts", 0)['max'] + 1;
			sql::q("INSERT INTO blasts(`id`,`name`,`image`) VALUES ('" . $mid . "','Новая магия " . $mid . "',1)");
			echo "<center class=return_win>Добавлено.</center>";
		}
		if (@$_GET["new_ap"]) {
			$mid = sql::q1("SELECT MAX(id) as max FROM auras", 0)['max'] + 1;
			sql::q("INSERT INTO auras(`id`,`name`,`image`) VALUES ('" . $mid . "','Новая магия " . $mid . "',1)");
			echo "<center class=return_win>Добавлено.</center>";
		}
		...
		if (@$_GET["dall"] and @$_GET["c"] != 2) {
			sql::q("DELETE FROM u_blasts WHERE id_in_w='" . intval($_GET["dall"]) . "'");
			echo "<center class=return_win>Удалено у всех</center>";
			sql::q("UPDATE blasts SET learnall=0 WHERE id='" . intval($_GET["dall"]) . "'");
		}
		if (@$_GET["dall"] and @$_GET["c"] == 2) {
			sql::q("DELETE FROM u_auras WHERE id_in_w='" . intval($_GET["dall"]) . "'");
			sql::q("UPDATE auras SET learnall=0 WHERE id='" . intval($_GET["dall"]) . "'");
			echo "<center class=return_win>Удалено у всех</center>";
		}

		if (@$_GET["lall"] and @$_GET["c"] != 2) {
			sql::q("UPDATE blasts SET learnall=1 WHERE id='" . intval($_GET["lall"]) . "'");
			echo "<center class=return_win>Обучат все как только перезайдут в игру</center>";
		}
		if (@$_GET["lall"] and @$_GET["c"] == 2) {
			sql::q("UPDATE auras SET learnall=1 WHERE id='" . intval($_GET["lall"]) . "'");
			echo "<center class=return_win>Обучат все как только перезайдут в игру</center>";
		}
	}
	if ($priv["emagic"] > 0) {
