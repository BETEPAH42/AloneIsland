...
if (@$_GET["delete"] and $_GET["delete"] <> $pers["uid"]) {
	sql::q("UPDATE users SET priveleged=0 WHERE uid=" . intval($_GET["delete"]));
	sql::q("DELETE FROM priveleges WHERE uid=" . intval($_GET["delete"]));
}
if (@$_GET["edit"]) {
	$p = sql::q1("SELECT * FROM priveleges WHERE uid=" . intval($_GET["edit"]) . "");
...
if (@$_GET["sedit"] and @$_POST) {
	...
	sql::q("UPDATE priveleges SET " . $q . " WHERE uid=" . intval($_GET["sedit"]));
}
if (@$_POST["go_in"]) {
	$p = sql::q1("SELECT uid FROM users WHERE user='" . $_POST["go_in"] . "'", 0)['uid'];
	if ($p) {
		sql::q("INSERT INTO `priveleges` ( `uid` , `emap` , `ewp` , `emagic` , `ebots` , `eusers` , `emain` , `emedia` , `status` ) VALUES ('" . $p . "', '0', '0', '0', '0', '0', '0', '0', 'Министр');");
		sql::q("UPDATE users SET priveleged=1 WHERE uid=" . $p . "");
	}
}

echo "Состав:<br>";
$m = sql::q("SELECT uid,user,level,sign,state FROM users WHERE priveleged=1");
...
foreach ($m as $p) {
	echo "<tr>";
	$prv = sql::q1("SELECT * FROM priveleges WHERE uid=" . $p["uid"]);
	echo "<td>";
