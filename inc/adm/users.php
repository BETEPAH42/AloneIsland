<table width=10%>
	<tr>
		<td><a class=bga href=main.php?go=administration>Назад в меню</a></td>
	</tr>
</table>
<center class=loc>Приветствие.<br>Введите ник персонажа для редактирования:<form action=main.php method=post><input type=text name=nick class=laar><input type=submit value=ОК class=login></form><br><a href=main.php?zeroingall=1 class=timef>Выдать всем обнуления</a><br><a href=main.php?szeroingall=1 class=timef>Выдать всем обнуления умения</a><br><a href=main.php?zeroall=1 class=timef>Принудительно обнулить всех</a><br><a href=main.php?fullzeroall=1 class=timef>Принудительно полностью обнулить всех</a>
	<a href=main.php?myself_edit=1 class=but>Редактировать себя</a>
	<hr>
...
if (!$priv["eusers"]) exit;
if (@$_GET["weather"]) {
	if (SQL::q("UPDATE world SET weather=" . intval($_GET["weather"]) . ",weatherchange=0")) echo "Погода установлена";
}
if (@$_GET["zeroingall"]) {
	if (SQL::q("UPDATE users SET zeroing=zeroing+1")) echo "Обнуления успешно выданы";
}
if (@$_GET["szeroingall"]) {
	if (SQL::q("UPDATE users SET skill_zeroing=skill_zeroing+1")) echo "Обнуления успешно выданы";
}
if (@$_GET["zeroall"]) {
	if (SQL::q("UPDATE users SET action=-10")) echo "Все успешно обнулены.";
}
if (@$_GET["fullzeroall"]) {
	if (SQL::q("UPDATE users SET action=-11")) echo "Все успешно полностью обнулены.";
}
if (@$_GET["zero"]) {
	if (SQL::q("UPDATE users SET action=-10 WHERE user='" . $_POST["zeronick"] . "'")) echo "Персонаж успешно обнулен.";
}
if (@$_GET["fullzero"]) {
	if (SQL::q("UPDATE users SET action=-11 WHERE user='" . $_POST["fullzeronick"] . "'")) echo "Персонаж успешно полностью обнулен.";
}
if (@$_POST["nick"] or @$_GET["myself_edit"]) {
	if ($_GET["myself_edit"])
		$_POST["nick"] = $pers["user"];
	$p = sql::q1("SELECT * FROM users WHERE user='" . $_POST["nick"] . "'");
	echo "<p class=inv>";
...
if (@$_GET["edit"] and $priv["eusers"] > 1) {
	...
	if (sql::q("UPDATE users SET " . $q . " WHERE uid=" . intval($_GET["edit"]) . ""))
