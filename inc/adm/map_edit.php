...
if ($priv["emap"] == 1) {
	if (@$_GET["res"]) {
		sql::q("UPDATE nature SET wood=" . intval($_POST["wood"]) . ",herbal=" . intval($_POST["herbal"]) . ",fishing=" . intval($_POST["fishing"]) . ",agriculture=" . intval($_POST["agriculture"]) . " WHERE x=" . ($x) . " and y=" . ($y) . "");
	}
	if (@$_GET["bts"]) {
		sql::q("UPDATE nature SET bot=" . intval($_POST["bid"]) . ",blvlmin=" . intval($_POST["minlvl"]) . ",blvlmax=" . intval($_POST["maxlvl"]) . " WHERE x=" . ($x) . " and y=" . ($y) . "");
		$buser = SQL::q1("SELECT user FROM bots WHERE id=" . intval($_POST["bid"]))['user'];
		$b1 = sql::q1("SELECT id FROM bots WHERE user='" . $buser . "' and level=" . intval($_POST["minlvl"]))['id'];
		if (!$b1)
			$b1 = sql::q1("SELECT MIN(id) as min FROM bots WHERE user='" . $buser . "'")['min'];
		$b2 = sql::q1("SELECT id FROM bots WHERE user='" . $buser . "' and level=" . intval($_POST["maxlvl"]))['id'];
		if (!$b2)
			$b2 = sql::q1("SELECT MAX(id) as min FROM bots WHERE user='" . $buser . "'")['min'];
		sql::q("INSERT INTO nature_bots(`x` ,`y` ,`idmin` ,`idmax` ,`frq` )
VALUES ('" . $x . "', '" . $y . "', '" . $b1 . "', '" . $b2 . "', '" . intval($_POST["frq"]) . "')");
	}
	if (@$_GET["delete"]) {
		sql::q("DELETE FROM nature WHERE x=" . ($x) . " and y=" . ($y) . "");
	}
	if (@$_GET["bd"]) {
		sql::q("DELETE FROM nature_bots WHERE x=" . ($x) . " and y=" . ($y) . " and idmin=" . intval($_GET["idmin"]) . " and idmax=" . intval($_GET["idmax"]) . "");
		sql::q("UPDATE nature SET bot=0,blvlmin=0,blvlmax=0 WHERE x=" . ($x) . " and y=" . ($y) . "");
	}
	if (@$_GET["add"]) {
		sql::q("INSERT INTO nature (x,y) VALUES (" . $x . "," . $y . ")");
	}
	...
	if (@$_GET["type"]) {
		sql::q("UPDATE nature SET type=" . intval($_GET["type"]) . " WHERE x=" . ($x) . " and y=" . ($y) . "");
	}
	if (@$_POST["name"]) {
		sql::q("UPDATE nature SET name='" . $_POST["name"] . "' WHERE x=" . ($x) . " and y=" . ($y) . "");
	}
	if (@$_GET["act"] == 'emp') {
		sql::q("UPDATE nature SET
	...
	if (@$_GET["goidd"]) {
		sql::q("UPDATE nature SET go_id='" . $_GET["go_id"] . "' WHERE x=" . ($x) . " and y=" . ($y) . "");
	}
}

$cells_around = sql::q("SELECT x,y,type,wood,fishing,herbal,agriculture,go_id,bot,blvlmin,blvlmax FROM nature WHERE x>=" . ($x - 5) . " and x<=" . ($x + 5) . " and 
y>=" . ($y - 4) . " and y<=" . ($y + 4) . "");

$maked_str = '';
foreach ($cells_around as $cc) {
	$bimg = sql::q1("SELECT obr FROM bots WHERE id=" . intval($cc["bot"]))['obr'];
...
$bts = sql::q("SELECT * FROM nature_bots WHERE x=" . ($x) . " and y=" . ($y) . "");
$bots = "<table border=0 class=but>";
foreach ($bts as $b) {
	$b1 = sql::q1("SELECT user,level,id FROM bots WHERE id=" . $b["idmin"]);
	$b2 = sql::q1("SELECT user,level,id FROM bots WHERE id=" . $b["idmax"]);
	$bots .= "<tr>";
	$bots .= "<td class=but2>";
...
$bots .= "</table>";
$buser = sql::q1("SELECT user FROM bots WHERE id='" . $cell["bot"] . "'");
...
if ($resources == "") $resources = "На данной территории ресурсы не обнаружены.";
...
<table border="1" width="100%" cellspacing="0" cellpadding="0" bordercolorlight=#C0C0C0 bordercolordark=#FFFFFF>
	<tr>
		...
		<td align=center width=50% class=loc valign=top>
			<?
			echo '<script>show_nature(' . $x . ',' . $y . ');</script>';
			?>
		</td>
		<td class=loc align=center width=25% valign=top>
			<?
			...
			if ($out["name"])
				echo "<font class=items>Вход в " . $out["name"] . "</font><br>";
			if ($cell)
				echo "<a href=\"javascript:shm_cellname('" . $cell["name"] . "')\" class=timef><b>[" . $cell["name"] . "]</b></a><br>";
			...
			if ($cell['last_trees_change'])
				echo "Деревья вырастут через " . tp($cell["last_trees_change"] - time()) . "<br>";;
			echo "<script>";
			echo "var bn=[";
			$allbots_names = sql::q("SELECT user,id,obr,pol,MIN(level) as minlvl,MAX(level) as maxlvl FROM bots GROUP BY user;");
			foreach ($allbots_names as $bn) {
