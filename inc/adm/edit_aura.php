...
if (@$_POST and $priv["emagic"] == 2) {
	...
	if (sql::q("UPDATE auras SET " . $q . " WHERE id='" . intval($_GET["edit"]) . "'")) echo "<center class=return_win><b>" . $p["name"] . " удачно изменен!</b></center>";
}
?>
...
<script>
	<?
	$v = sql::q1("SELECT * FROM auras WHERE id='" . $_GET["edit"] . "'");
	$params = '';
	$names = '';
