...
?>
<center width=100% class=but><br><b>Возможности министра</b><br>
Должность: Создатель мира назначил вас на должность <b><?= $priv["status"] ?></b><br>
<?= $online ?><p><?= $abb ?></p>
<div class='container_menu_admin container-fluid'>
	<? foreach($menu->menus as $item):?>
		<a class="element_admin_menu" href="<?=$item['url']?>">
			<?=$item['name']?>
		</a>
	<? endforeach;?>
	<div class="element_admin_menu">
...
<?php
	...
	// $req = SQL::q1("SELECT COUNT(*) as count FROM avatar_request");
	// $links .= "<li><a class=bg href=main.php?go=ava_req>Одобрить образ[<b class=user>" . $req['count'] . "</b>]</a></li>";
