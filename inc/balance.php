...
function _MakeItem($level, $class, $stype, $power = 1, $tpower = 1)
{
	...
	$result = array();
	$summ = sql::q1("SELECT SUM(stats) as sum FROM exp WHERE level<=" . intval($level))['sum'] + 20;
...
{
	global $stypes;
	$mid = sql::q1("SELECT MAX(idn) as sum FROM weapons", 0)['sum'] + 1;
	if ($mid < 500) $mid += 500;
	for ($i = 3; $i <= 15; $i++) {
		foreach ($stypes as $stype) {
			for ($j = 1; $j <= 6; $j++) {
		...
		foreach($stypes as $stype)
		{
			for($j=1;$j<=6;$j++)
				...
				//	$image = rand(100,500);
					...
					if($r["ts6"])
					$name_image = sql::q1("SELECT name FROM weapons WHERE stype='".$stype."' and ts6>0 and price<".$r["price"]." ORDER BY RAND()");
					else
					$name_image = sql::q1("SELECT name FROM weapons WHERE stype='".$stype."' and ts6=0 and price<".$r["price"]." ORDER BY RAND()");
