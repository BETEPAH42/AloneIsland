<table border="0" width="100%" cellspacing="0" cellpadding="0">
	<tr>
		...
		<td background="images/design/borders/b1/tbg.gif" align="center" width=100%>
			<img border="0" src="images/design/borders/b1/t.gif" width="380" height="114">
		</td>
		<td width="99">
			<img border="0" src="images/design/borders/b1/rt.gif" width="99" height="116">
		</td>
	</tr>
	<tr>
		<td width="96" background="images/design/borders/b1/l.gif">&nbsp;</td>
		<td width=100% align=center>
			<div id=message class=hp>
				<?
				...
				if (isset($_POST["travm"]) and isset($_POST["timeout"]) and $_SESSION["a_type"] == 'duel') {
					...
					if (
						$_POST["travm"] <> '10' and $_POST["travm"] <> '30' and $_POST["travm"] <> '50'
						and $_POST["travm"] <> '80'
					)	$_POST["travm"] = 10;
				...
				if (@$_GET["z"] == "begin" and $pers["cfight"] == 1) {
					$z = sql::q1("SELECT * FROM zayavki WHERE `name`='" . $pers["user"] . "'");
					if ($z["vsname"]) {
						begin_fight($z["name"], $z["vsname"], "Дуэль на арене", $z["travm"], $z["timeout"], $z["oruj"]);
						sql::q("DELETE FROM zayavki WHERE `name`='" . $pers["user"] . "'");
						echo "Начинаем бой. Ждите.";
					}
				}

				if (@$_POST["tobot"] and $pers["cfight"] == 0 and $pers["last_bot_time"] + $pers["level"] * 200 < time()) {
					$bot = sql::q1("SELECT * FROM bots WHERE obr='bear' and level>" . ($pers["level"] - 2) . " and level<" . ($pers["level"] + 2) . " and id=" . intval($_POST["tobot"]));
					if (@$bot["user"]) {
						$travm = 0;
				...
				if (@$_POST["towhozay"] and $pers["cfight"] == 0 and $_SESSION["a_type"] == 'duel') {
					$z = sql::q1("SELECT * FROM zayavki WHERE `name`='" . $_POST["towhozay"] . "'");
					if ($z["oruj"] == 1 or ($z["oruj"] == 0 and str_replace("none|", "", $pers["wears"]) == '')) {
						if (sql::q("UPDATE zayavki SET `vsname`='" . $pers["user"] . "' WHERE `name`='" . $_POST["towhozay"] . "'"))
							sql::q("UPDATE `users` SET cfight=2 WHERE uid='" . $pers["uid"] . "';");
						sql::q("UPDATE `users` SET refr=1 WHERE user='" . $_POST["towhozay"] . "'");
						$pers["cfight"] = 2;
					}
				}
				...
				if (@$_POST["towhozay_g"] and $pers["cfight"] == 0 and $_SESSION["a_type"] == 'group') {
					...
					$z = sql::q1("SELECT * FROM zayavki WHERE time='" . $twz[0] . "'");
					$l = 0;
					...
					if ($pers["level"] >= $z["minlvl" . $twz[1]] and $pers["level"] <= $z["maxlvl" . $twz[1]] and $l == 1)
						if ($z["oruj"] == 1 or ($z["oruj"] == 0 and str_replace("none|", "", $pers["wears"]) == '')) {
							if ($twz[1] == '1')
								if (sql::q("UPDATE zayavki SET `name`=CONCAT(name,'" . $pers["user"] . "|') WHERE `time`='" . $twz[0] . "'"));
							if ($twz[1] == '2')
								if (sql::q("UPDATE zayavki SET `vsname`=CONCAT(vsname,'" . $pers["user"] . "|') WHERE `time`='" . $twz[0] . "'"));
							sql::q("UPDATE `users` SET cfight=4 WHERE uid='" . $pers["uid"] . "';");
							$pers["cfight"] = 4;
						}
				}
	...
	<input type=hidden name=a_type value=duel>
			...
			';
				...
				function show_z_duel($n, $nvs, $orj, $tr, $timeout)
				{
					global $pers;
					$n = str_replace("|", "", $n);
					$n = sql::q1("SELECT user,level,sign,state,aura FROM users WHERE user='" . $n . "'");
					$nvs = str_replace("|", "", $nvs);
					$nvs = sql::q1("SELECT user,level,sign,state,aura FROM users WHERE user='" . $nvs . "'");
...
<tr>
	...
	';
				...
				function show_z_group($n, $nvs, $orj, $tr, $timeout, $time, $k1, $l1m, $l1n, $k2, $l2m, $l2n, $wait)
				{
					...
					$s1 = '';
					foreach ($n as $e) {
						$e = sql::q1("SELECT user,level,sign,state,aura FROM users WHERE user='" . $e . "'");
						if ($e[0] <> '') {
					...
					$nvs = explode("|", $nvs);
					foreach ($nvs as $e) {
						$e = sql::q1("SELECT user,level,sign,state FROM users WHERE user='" . $e . "'");
						if ($e[0] <> '') {
							if (substr_count($e["aura"], "invisible"))
					...
					if ($pers["level"] >= $l1m and $pers["level"] <= $l1n and substr_count($n_s, "|") < $k1) $dis_1 = '';
					else $dis_1 = 'DISABLED';
					if ($pers["level"] >= $l2m and $pers["level"] <= $l2n and substr_count($nvs_s, "|") < $k2) $dis_2 = '';
					else $dis_2 = 'DISABLED';
	...
	';
				...
				function show_z()
				{
					if ($_SESSION["a_type"] == 'duel') {
						$resault = sql::q("SELECT * FROM zayavki WHERE type='duel'");
						$i = 0;
						echo "<form method=post action=main.php>";
						foreach ($resault as $z) {
							if ($z["time"] < time() - 600) {
								sql::q("DELETE FROM zayavki WHERE name='" . $z["name"] . "'");
								sql::q("UPDATE users SET cfight=0 WHERE user='" . $z["name"] . "' or user='" . $z["vsname"] . "'");
							}
							$i++;
					...
					if ($_SESSION["a_type"] == 'group') {
						$resault = sql::q("SELECT * FROM zayavki WHERE type='group'");
						$i = 0;
						...
						foreach ($resault as $z) {
							if ($z["wait"] - (time() - $z["time"]) < 0 or (substr_count($z["name"], "|") == $z["mpl1"] and
								...
								sql::q("DELETE FROM zayavki WHERE time='" . $z["time"] . "'");
							} else {
								$i++;
		...
		" . tp($pers["last_bot_time"] + $pers["level"] * 300 - time()) . "</font>";
							...
							echo "<form method=post action=main.php>";
							$bot1 = sql::q1("SELECT * FROM bots WHERE obr='bear' and level=" . ($pers["level"] - 1));
							$bot2 = sql::q1("SELECT * FROM bots WHERE obr='bear' and level=" . ($pers["level"] + 0));
							$bot3 = sql::q1("SELECT * FROM bots WHERE obr='bear' and level=" . ($pers["level"] + 1));
							if (@$bot1["user"])
								echo '<tr>
						<td width="17">
				...
				function take_z()
				{
					...
					if ($_SESSION["a_type"] == "duel") {
						...
						if ($pers["cfight"] == 1) {
							$z = sql::q1("SELECT name,vsname FROM zayavki WHERE `name`='" . $pers["user"] . "'");
					...
					if ($_SESSION["a_type"] == "group") {
						if ($pers["cfight"] == 0)
							echo '<input class="loc" type="submit" value="Принять вызов на бой"></form>';
						if ($pers["cfight"] == 3) {
							$z = sql::q1("SELECT name FROM zayavki WHERE `name`='" . $pers["user"] . "|' and vsname=''");
							if ($z["name"] <> '')
				...
				if ($pers["cfight"] == 1 and $_POST["delete"] == 1) {
					$z = sql::q1("SELECT name,vsname FROM zayavki WHERE `name`='" . $pers["user"] . "'");
					if (sql::q("DELETE FROM zayavki WHERE name='" . $pers["user"] . "';	
 ") and sql::q("UPDATE `users` SET cfight=0 WHERE uid='" . $pers["uid"] . "' or user='" . $z["vsname"] . "';")) {
						echo "Заявка удачно удалена";
						$pers["cfight"] = 0;
					} else
				...
				if ($pers["cfight"] == 3 and $_POST["delete"] == 2) {
					$z = sql::q1("SELECT name,vsname FROM zayavki WHERE `name`='" . $pers["user"] . "|' and vsname=''");
					if ($z["name"] <> '') {
						if (sql::q("DELETE FROM zayavki WHERE name='" . $pers["user"] . "|';	
 ") and SQL::q("UPDATE `users` SET cfight=0 WHERE uid='" . $pers["uid"] . "';")) {
							echo "Заявка удачно удалена";
							$pers["cfight"] = 0;
						} else
							echo "Заявка не удалена";
			...
			<table width="100%" cellspacing="0" cellpadding="0" style="border-left-width: 1px; border-right-width: 1px; border-bottom-width: 1px">
				<tr>
					<td class="inv" align="center">
						<a href="main.php?goarena=duel"><u>Дуэли</u></a>
					</td>
					<td class="inv" align="center">
						<a href="main.php?goarena=group"><u>Групповые</u></a>
					</td>
					<td class="inv" align="center">
						<a href="main.php?goarena=haot"><u></u></a>Хаоты
					</td>
					<td class="inv" align="center">
						<a href="main.php?goarena=statistic"><u></u></a>Статистика
					</td>
				</tr>
			</table>
			<table border="0" width="100%" cellspacing="0" cellpadding="0">
	...
	<tr>
		<td width="96">
			<img border="0" src="images/design/borders/b1/lb.gif" width="96" height="109">
		</td>
		<td background="images/design/borders/b1/bbg.gif" align="center" width=100%>
			<img border="0" src="images/design/borders/b1/b.gif" width="380" height="106">
		</td>
		<td width="99">
			<img border="0" src="images/design/borders/b1/rb.gif" width="99" height="107">
		</td>
	</tr>
</table>