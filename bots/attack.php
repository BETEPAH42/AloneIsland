...
if ($SPECIAL_pers)
	...
	$pers = SQL::q1("SELECT * FROM users WHERE online=1 and location='out' and cfight=0 and apps_id=0 and curstate<5 and lb_attack<" . (tme() - 2 * $lb) . " LIMIT 0,1;");
...
if ($pers) {
	...
	if ($pers["gain_time"] > (tme() - 1200)) {
		...
		$cell = SQL::q1("SELECT bot,blvlmin,blvlmax,type FROM `nature` WHERE x>=" . ($x - 1) . " and x<=" . ($x + 1) . " and 
y>=" . ($y - 1) . " and y<=" . ($y + 1) . " ORDER BY RAND() LIMIT 0,1");
		$timeout = 50;
		list($a1, $a2) = SQL::q1("SELECT idmin,idmax FROM nature_bots WHERE x>=" . ($x - 1) . " and x<=" . ($x + 1) . " and 
y>=" . ($y - 1) . " and y<=" . ($y + 1) . " ORDER BY RAND()");
		$BTS = rand(intval($a1), intval($a2));
	} else {
		list($a1, $a2) = SQL::q1("SELECT idmin,idmax FROM nature_bots WHERE x=" . ($x) . " and y=" . ($y) . " ORDER BY RAND()");
		$BTS = rand(intval($a1), intval($a2));
	}
	...
	if ($BTS == 0) {
		...
		$bot_id2 = SQL::q1("SELECT id FROM bots WHERE user='" . $user["user"] . "' and level=" . rand($cell["blvlmin"], $cell["blvlmax"]) . "");
		$bot_id3 = SQL::q1("SELECT id FROM bots WHERE user='" . $user["user"] . "' and level=" . rand($cell["blvlmin"], $cell["blvlmax"]) . "");
		$bot_id = SQL::q1("SELECT id FROM bots WHERE user='" . $user["user"] . "' and level=" . rand($cell["blvlmin"], $cell["blvlmax"]) . "");
		$bot_id2 = $bot_id2["id"];
		$bot_id3 = $bot_id3["id"];
		$bot_id = $bot_id["id"];
	} else {
	...
	//$f_type = 0;
	$perstowho = $pers["user"];
	$bots = "SELECT id,rank_i FROM bots WHERE id='" . $bot_id . "' or id='" . $bot_id2 . "' or id='" . $bot_id3 . "' and rank_i>" . ($pers["rank_i"] / 10) . " LIMIT 0,3;";
	$bot = SQL::q1($bots);
	$bot2 = SQL::q1($bots);
	$bot3 = SQL::q1($bots);
	$b = 0;
