...
if ($pers["online"] == 1) {
	if ($pers["location"] <> 'out')
		$location = sql::q1("SELECT name FROM `locations` WHERE `id`='" . $pers["location"] . "'");
	else
		$location = sql::q1("SELECT name FROM `nature` WHERE `x`='" . $pers["x"] . "' and `y`='" . $pers["y"] . "' ;");
	$_ONLINE = "[" . $pers["online"] . ",'" . tp($curtimeonline = (time() - $pers["lastvisits"])) . "','" . $location["name"] . "'," . $pers["x"] . "," . $pers["y"] . "," . $pers["cfight"] . "]";
} else {
	$location["name"] = '';
	if (($you["sign"] == 'c2' or strpos(" " . $you["rank"], "<pv>") > 0)) {
		if ($pers["location"] <> 'out')
			$location = sql::q1("SELECT name FROM `locations` WHERE `id`='" . $pers["location"] . "'");
		else
			$location = sql::q1("SELECT name FROM `nature` WHERE `x`='" . $pers["x"] . "' and `y`='" . $pers["y"] . "' ;");
...
if ($pers["icq"])
	$INFO_TEXT .= "ICQ:<b> " . $pers["icq"] . "</b><img src='http://web.icq.com/whitepages/online?icq=" . $pers["icq"] . "&img=26' height=12><br>";
if ($pers["vkid"])
	$INFO_TEXT .= "ВКонтакте:<b><a class=timef target=_blank href='http://vkontakte.ru/profile.php?id=" . $pers["vkid"] . "'>" . $pers["vkid"] . "</a></b><br>";
if ($pers["pol"] == "male")
	$INFO_TEXT .= "Пол:<b> Мужской</b><br>";
else
	$INFO_TEXT .= "Пол:<b> Женский</b><br>";
if ($chars["about"]) $INFO_TEXT .= "<font class=title>О СЕБЕ</font><br>" . str_replace("
", "<br>", $chars["about"]);

if ($you) $rep = '<a href="javascript:report();" class=bg>Написать отзыв</a>';
else $rep = '';
...
##################
echo "<script>";
$count_prs = sql::q1("SELECT COUNT(*) as count FROM presents_gived WHERE uid=" . $pers["uid"] . "")['count'];
echo "var prs = [" . $count_prs . "";
$prs = sql::q("SELECT * FROM presents_gived WHERE uid=" . $pers["uid"]);
foreach ($prs as $p) {
	$who = $p["who"];
	if (
		!($_SESSION["sign"] == 'c2' or strpos(" " . $_SESSION["rank"], "<pv>") > 0)
		and $p["anonymous"]
	) $who = 'Анонимно';
	$p["name"] = str_replace("\r\n", "", $p["name"]);
...
if (@$_POST["deleterep"]) {
	echo 1;
	$r = sql::q1("SELECT * FROM reports_for_users WHERE uid=" . $pers["uid"] . " and date=" . intval($_POST["deleterep"]) . "");
	if ($r["who"] == $you["user"] or $you["user"] == $pers["user"] or (substr_count($you["rank"], "<pv>") or $you["sign"] == 'c2'))
		sql::q("DELETE FROM reports_for_users WHERE uid=" . $pers["uid"] . " and date=" . intval($_POST["deleterep"]) . "");
}
#Добавить отзыв:
if (@$_POST["report"] and $you["money"] >= 50) {
	sql::q("INSERT INTO `reports_for_users` ( `uid` , `lvl` , `sign` , `date` , `who` , `text` ) VALUES ('" . $pers["uid"] . "', '" . $you["level"] . "', '" . $you["sign"] . "', '" . time() . "', '" . $you["user"] . "', '" . str_replace("'", "", $_POST["report"]) . "');");
...
if (empty($_GET["all_reports"]))
	$rep = sql::q("SELECT * FROM reports_for_users WHERE uid=" . $pers["uid"] . " ORDER BY date DESC LIMIT 7;");
else
	$rep = sql::q("SELECT * FROM reports_for_users WHERE uid=" . $pers["uid"] . " ORDER BY date DESC");
...
foreach ($rep as $r) {
	...
	if (
		$r["who"] == $you["user"] or $you["user"] == $pers["user"] or
		(substr_count($you["rank"], "<pv>") or $you["sign"] == 'c2')
	) $del = $r["date"];
