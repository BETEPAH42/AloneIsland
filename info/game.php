...
if ($pers["punishment"] >= time()) $punished = tp($pers["punishment"] - time());
else $punished = '';
...
if ($pers["level"] > 9) {
	$pupil = SQL::q1("SELECT * FROM users WHERE instructor = " . $pers["uid"]);
	if ($pupil) {
		$INFO_TEXT .= "<center>";
...
if ($pers["sign"] <> 'none')
	$clan = SQL::q1("SELECT name,dmoney,sait,level FROM clans WHERE sign='" . $pers["sign"] . "'");
...
if (($_SESSION["sign"] == 'c2' or strpos(" " . $_SESSION["rank"], "<pv>") > 0)) {
	$level = SQL::q1("SELECT exp FROM exp WHERE level=" . ($pers["level"] + 1));
...
<div id='inf_from_php2' style='visibility:hidden;position:absolute;top:0px;height:0;'><? echo $INFO_TEXT; ?></div>
<script>
	<?
	...
	include_once 'inc/inc/p_clothes.php';
	// var_dump('Zvanie',$pers["zvan"]);
...
if (!$_INV) {
	$txt = '';
	foreach (SQL::q1("SELECT * FROM p_auras WHERE uid=" . $pers["uid"] . " and (esttime>" . time() . " or turn_esttime>" . $pers["f_turn"] . ") and special<>2;") as $a) {
